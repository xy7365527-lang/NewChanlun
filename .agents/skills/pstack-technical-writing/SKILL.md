...
description: "分层技术写作标准：按文档类型（Diátaxis）、读者任务（Google 开发者风格）、句子载重（STE）与可验证事实（真实符号/路径/命令/计数）组织并改进 README、ADR、spec、Issue、PR 描述等技术文档的结构与技术表达；不改写代码、命令、标识符、日志、错误信息、结构化数据、测试输出与逐字引文。编写或审阅技术文档，或要求改进文档结构、技术表达或句子清晰度时使用。"
license: MIT
metadata:
  upstream-repo: cursor/plugins
  upstream-sha: fd6dd6f7276956a532bb78a748a8d2818b6eb5f4
  upstream-path: pstack/skills/technical-writing/SKILL.md
  license-file: LICENSE.MIT
---
# 技术写作（pstack-technical-writing）

本 Skill 管**结构与技术表达**，全局 `unslop` 管**最终自然语言风格**，两段分工、不重复改写：

1. **本 Skill**——先按文档类型（Diátaxis）、读者任务（Google 开发者风格）与可验证事实（真实符号、路径、命令、计数）组织技术文档，再改进句子载重与歧义。
2. **全局 `unslop`**——最后清理 AI 腔、空话、套话等自然语言风格。本 Skill 不做风格清理，`unslop` 不做结构与技术表达，同一处不重写两遍。

**受保护内容逐字不动**：代码、命令、标识符、日志、错误信息、结构化数据、测试输出、逐字引文——本 Skill 与 `unslop` 都不得改写、不得重排、不得「顺带润色」；确需修改时只指出位置，由作者在源里改。

> 本仓文档为简体中文。四层标准中，Diátaxis（文档类型）、读者任务与可验证事实对中英文同样成立；STE / Global English 的句子级规则取可迁移的核心（一句一想法、条件前置、一词一义、不留第二种读法），英文特有的冠词/拼写规则不套用到中文。最终风格仍交 `unslop`。

正文（上游标准，除「Voice and repo specifics」的 Prime 适配点外原样保留）：
...
- Apply **this skill before unslop**: document type, reader task, and verifiable facts first; `unslop` then owns only the final natural-language style. Neither step rewrites protected content — code, commands, identifiers, logs, error messages, structured data, test output, and verbatim quotes stay byte-for-byte unchanged.
...
- **原始 Skill**：`pstack/skills/technical-writing/SKILL.md`（无 `references/`、`scripts/` 相对文件）

## 本地 Prime 改写

- `name`：`technical-writing` → `pstack-technical-writing`（`pstack-` 命名空间，避免与未来用户级同名 Skill 静默 shadow）。
- `description`：改为简体中文，写明 Prime 触发条件（README / ADR / spec / Issue / PR 文案）与不改写的受保护内容；删除上游的 `/technical-writing` 命令引用。`unslop` 分层写在正文开头（描述只做路由，避免「AI 腔」等风格词被误路由到本 Skill）。
- 移除 `disable-model-invocation: true`：首批统一 `both` 调用契约（模型可见可自动调用，同时保留 `/skill:pstack-technical-writing` 显式入口）。
- 新增 `license: MIT` 与 `metadata`（`upstream-repo` / `upstream-sha` / `upstream-path` / `license-file`）frontmatter。
- 正文开头加简体中文分层框：本 Skill 与 `unslop` 的分工、受保护内容逐字不动、中文文档适用说明。
- 「Voice and repo specifics」节首加一条 Prime 适配：先本 Skill（文档类型 / 读者任务 / 可验证事实）后 `unslop`（仅最终风格），两者都不改写受保护内容。
- 其余正文（三条总规则、Vary the rhythm、Diátaxis、Google、STE、Global English、Worked example、Review checklist）与上游逐字一致，原样保留。
