...
## 回测重定位图开张（2026-08-29）
- 「结构判据」正本落盘 v1.0（ff1dc33a4f，#1278 终态件①）。
- **[#1304](https://github.com/xy7365527-lang/NewChanlun/issues/1304)** 图票已开：功能回测（生产回放验收系统）——研发回测冻结归档，回放回测转功能件。
## #1304 frontier scan 派发（2026-08-29）
- replay-feasibility-scan 子代理派出（五域盘点：确定性障碍/feed 适配/快照恢复/对拍点/范围边界）。
## #1304 第一批拆票（2026-08-29）
- scan 完成（可行零阻塞；最大风险=共核不共外围的对拍误报）。第一批三票：**#1305** 回放 driver / **#1306** 对拍 harness v1 / **#1307** 事件坐标系双列。第二批（验收报告器 + tape v3）缓。
## #1305 补验 + 第二批开票（2026-08-29）
- theta_replay 真实数据双跑自检 PASS ×2（OKLO 34 万 bar 33s / BTC 53 万 bar 105s，逐位一致）——#1305 验收补全。
- #1304 第二批票已开（验收报告器 + tape v3，#1279 检查单 B 组执行器）。
## #1282 预注册冻结 + 实验票（2026-08-29）
- 预注册冻结（7/7 全胜判据 + 前后对照 + 随机基线 + 纪律附款）；#1282 grilling 收口关票。
- 实验票 **[#1309](https://github.com/xy7365527-lang/NewChanlun/issues/1309)** 已开（对称扩展实现 + 冻结判据回测 + 原样入档）。
## #1283 paper 链裁定收口（2026-08-29）
- Q1 BTC@Hyperliquid / Q2 nautilus 链（对齐 0.62.0）/ Q3 风控四条 / Q4 旧 runner.py 退役。
- 实装 **[#1310](https://github.com/xy7365527-lang/NewChanlun/issues/1310)**（T1 依赖接入+θ桥，#916 销）+ **[#1311](https://github.com/xy7365527-lang/NewChanlun/issues/1311)**（T2 paper 首跑+风控+feed）已开。
- nautilus 上游更新核查：骨架钉 0.60.0 → 上游 0.62.0（08-21），实装直接对齐。
## M1 第二轮预注册开票（2026-08-29）
- 状态门控第二预注册票已开（门状态 tape v3 导出 + 双腿状态闸 + 两轮对照）。
## 状态机 MECE 票（2026-08-29，编排者令）
- **[#1313](https://github.com/xy7365527-lang/NewChanlun/issues/1313) 待开**：状态机完全分类核验与方向对称化（#1311 前置）。
- 更正：状态机票即 **#1313**（上一行写错编号格式，实际本条即票号）。
