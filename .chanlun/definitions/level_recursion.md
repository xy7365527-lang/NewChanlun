...
> 正本：本文件是**级别**的教义正本（「级别 ＝ 由递归定义，不由数据聚类的簇定义」）。
> 最后裁定：[#815](https://github.com/xy7365527-lang/NewChanlun/issues/815)（2026-08-03，M-3）
> 受影响代码清单：见 [`qushi.md`](qushi.md) 末节「#815 受影响代码清单（四组汇总）」——#815 四组共用一份，本文件不另立。

**版本**: v1.1（#815 M-3 裁定落盘）
**状态**: 已结算
**创建日期**: 2026-02-16
**结算日期**: 2026-02-17
**最后更新**: 2026-08-03
...
## #815 M-3 裁定：级别由**递归**定义，不由数据聚类的簇定义（2026-08-03）

**裁定原文**（[#815](https://github.com/xy7365527-lang/NewChanlun/issues/815) M-3 定案 comment）：

| | 口径 | 判 |
|---|---|---|
| **A** | 本级由次级构成，`level_id = 下级 + 1`（Rust 主链） | **采纳，这是定义** |
| **B** | persistence 取对数找最大断层横切 → 换算日历天数 → 按阈值归名（`src/newchan/a_level_detection.py:282` + `CalendarPeriodNamer:77`） | **不是定义** |

名分 `[旧缠论]`（A 即本文件「核心定义」节已有的 `level_id` 递归口径，本裁定是把它钉死为**唯一**定义，并处置 B）。

**⚠️ 与本文件下一节「两种『级别』口径的区分」不是同一组 A/B**：下一节的 A/B 是「递归级别 vs 时间周期级别（TF）」；本节的 A/B 是「递归 vs persistence 聚类簇」。两组各自独立，不要混读。

### ★ 承重理由经编排者当场订正——承重的**不是**「用了时间」

主控原先给的理由是「级别不许由时间尺度确定」，**编排者订正：那个说法不准。**

> 编排者原话：「**那个底跟级别无所谓，这本身跟时间也没关系。**」

**订正后的理由** `[新缠论:推论]`：B 的病不在它用了日历天数，而在 `CalendarPeriodNamer` 那张阈值表（`>100 日 → 月线级` / `10~40 → 日线级` / …）**给级别装了绝对锚点**。

**推理链**：① 缠论的级别是**纯相对关系**——「本级由次级构成」就是全部内容，没有绝对参照系；② 问「这段是不是日线级」预设了绝对刻度，而理论里没有那东西；③ 时间只是 B 选的刻度单位，**换成别的单位病一样在** ⟹ 承重的是「有没有绝对锚点」，不是「用没用时间」。

**⟹ 这条理由比原来那条强**：按原理由，B 只要把日历日换成别的量纲就"合规"了；按订正后的理由，**只要它给出绝对锚点就不合格**。

### 连带：解释了 [#812](https://github.com/xy7365527-lang/NewChanlun/issues/812) S-5「底可以随便选」 `[新缠论:推论]`

#812 S-5 已裁 `a₀ = 线段中枢`，依据 `084-第84课.md:54`【正文】逐字：「本ID关于中枢等的定义，其实一直没有改变过，因为中枢定义的关键，在于定义的递归性。一般的递归定义，由两部分组成，一、f1(a0)=a1；二、f2(an)=an+1；关于第二条的中枢过程规则，是一直没有任何改变的，而关于第一条，其实，可以随意设置任何的，都不会改变中枢定义的递归性。」

**本裁定解释了缠师为什么能这么说**：级别是相对关系，**底只是编号的原点**，原点平移不改变关系。**⟹ S-5 的「底可以随便选」与本条「级别由递归定义」是同一件事的两面，不是两条独立裁定。**

（S-5 自己登记的那条「不同 `a₀` 得到的**中枢序列**是否一致，**原文未处理**」**不受影响，仍然开着**——那问的是序列内容，不是级别编号。）

### ★ 撤回一个伪问题（照实登记）

主控走票中提出「同一个 `a₀` 定义、喂不同周期的 K 线（1min vs 日线），得到的塔是不是同一个」，并拟登记为 S-5 未覆盖的新欠账。**编排者当场指出该问题不成立，撤回。**

**成因**：它问「两个塔的第 3 级是不是同一个」，**而这句话预设了绝对编号存在**。两塔的递归关系同构，只是编号原点不同——问「哪个第 3 级」本身就是本裁定要否掉的那个绝对刻度思维。

### 裁定二：B 的处置 ＝ 降为**独立对照臂**（编排者选甲）

- **甲（采纳）**：降为**独立对照臂** —— 保留测量能力，但**显式声明不产生交易决策、不参与准入**。判例 [ADR-0005](../../docs/adr/0005-nest-import-ban-clause9-ruling.md)：已声明的独立对照臂不在「生产判定路径」管辖内。**代价**：需在代码侧立门，且此后每次有人想用它定级别都要被挡一次。
- 乙（未取）：完全退场 —— 干净但损失独立视角，与本仓「探针是可复用的测量装置」判例相悖。
- 丙（未取）：留作可影响判定的启发式 —— **会触发 `AGENTS.md` 收敛通则**（同一判断不得有宽严两档），两套定级别口径接在同一判定上正是该条所禁。

**可达性事实（map [#787](https://github.com/xy7365527-lang/NewChanlun/issues/787) N-1 举证）**：`detect_levels` 的调用者只有 Python 侧 PH 链（`a_ph_zhongshu.py:60`、`a_buypoint_score.py:43`），再往上**全是 `scripts/` 研究脚本**（`mos_ratio_ph.py`、`settle_backtest.py`、`tencent_three_gates_ph.py`、`dual_settle_scan.py`），**无一条接到 Rust 主链或 NT 生产路径**。⟹ 当前它事实上已在准入路径之外，**本裁定是把这个事实变成显式声明，不是新增限制**。

### ★ 连带转正：`formal/Origin/Turn.lean` 的 `level : Nat`

[#877](https://github.com/xy7365527-lang/NewChanlun/issues/877)（已关，合入 `fb57014f11`）在 `formal/Origin/Turn.lean` 里把级别编码成 **`TrendInstance.level : Nat`**，把「让位于**新的同级**走势类型」编码成**数字相等**（`T'.level = T.level`）。该编码形态此前名分为 `[新缠论:候选]`，因为它**押注了 M-3 的答案是「递归级别」**（可线性索引、同级 ＝ 数字相等）——若 M-3 裁出「数据涌现的簇」，簇不一定能线性索引，「同一级」也就不再是相等判断。

**⟹ M-3 裁出「递归」而非「涌现簇」，故 `Nat` 层号编码原样成立，名分由 `[新缠论:候选]` 转正为 `[新缠论:选择]`**（编码形态是实现选择，不是原文给的）。它表达的是「**同一个塔里的层号相同**」，是相对判断，不是绝对刻度 ⟹ 不被本条打到。

**⚠️ 但正本在此写明（这一句是转正的附带条件）**：

> **`TrendInstance.level` 是「塔内层号」，不是绝对级别名。** 不得拿它当「日线级／30 分级」一类的绝对刻度用——**那正是 B 犯的错**。代码侧须补上这条注释（见 `qushi.md` 汇总清单 M-3 行）。

### ⚠️ Lean 主链两个核心类型仍缺 `level`（本裁定不填，只登记）

`DivergencePair`（`formal/Origin/Divergence.lean:72-76`）与 `MoveOutcome` 都不带级别字段 ⟹ `Turn.lean` 可以合法造出「外层标签写 `level = 3`、里面装的却是 1min 图上算出的背驰证据」的实例，**类型系统检查不出来**。#877 已按「做不到就写做不到」在两处登记，未以恒真占位命题冒充已补上。

**⚠️ 加字段前先看反面判例**：`BspPoint.level_origin` 从未被塔实际填充、恒为 0（[#434](https://github.com/xy7365527-lang/NewChanlun/issues/434)），污染 Debug 摘要，最后走 [SPEC #455](https://github.com/xy7365527-lang/NewChanlun/issues/455) 删除。**加字段容易，填对难；一个永远为 0 的级别字段比明写「这里没检查」更坏——它看着像已经管住了。**
...
## 纵向造级别 vs 横向读级别（#827 裁定，2026-08-01）

**[新缠论:推论]** 上文「递归构造链」「区间套与级别递归的关系」两节都是**纵向**——用低级别的产出造高级别。第 38 课另有一条**横向**方向：同级别分解（操作分解），选定一层把市场切成一段段该层走势类型。二者不是同一根轴：
...
**依赖单向**：横向消费纵向的产出，纵向从不消费横向——「30 分钟走势类型」得先被纵向递归造出来，横向才有东西可读，反过来不成立（`038-第38课.md:22`）。

**订正**：本仓曾用「横向递归」描述同级别分解，不准确——横向方向上没有递归，同级别分解是**一次性切分**；真正递归的只有纵向，"多"来自纵向铺开的 N 个操作级别（「重」，见 ADR 0010）。

详见 [ADR 0011：操作分解层是一等模块](../../docs/adr/0011-operation-decomposition-layer.md)。
