...
**版本**: v0.3
**状态**: 生成态（#1下跌确立✅ #4盘整背驰✅ 已结算；#2代码落地中/#3/#5仍为生成态）
**最后更新**: 2026-02-16
...
### 2. 买卖点与走势类型完成的映射关系 — ⏳ 代码落地中
- **问题**：买卖点的出现与走势类型完成的关系尚需精确化
  - 第一类买卖点 = 走势类型完成点？还是走势类型完成后的某个时点？
  - 背驰点出现时走势是否已"完成"？（背驰是完成的充分条件还是必要条件？）
- **原文回溯结论**（R21-C, 2026-02-16）：
  1. 第一类买卖点 ≠ 走势类型完成点本身，而是**完成时刻产生的背驰信号** [旧缠论]
  2. 背驰判定时机 = "C段走势类型完成时"（第24课原文精确表述）
  3. `Divergence.confirmed = True` → 该背驰对应走势已完成，可直接生成第一类买卖点
  4. 走势完成是背驰检测的**触发时机**，不是买卖点本身
  5. 背驰→走势完成是充分条件（beichi #3已结算：[旧缠论:选择]）
- **原文依据**：第17课（走势终完美）、第24课L24（"C段走势类型完成时"）、第21课（第二类买点=1B后第二段次级走势低点）
- **代码落地计划**（R22-B, 2026-02-16）：
  - **Type 1**：当前已正确 `confirmed=div.confirmed` ✅
  - **Type 2**：当前 `confirmed=callback_seg.confirmed` ❌ → 改为 `Move.confirmed`
  - **Type 3**：当前 `confirmed=pullback_seg.confirmed` ❌ → 改为 `Move.confirmed`
  - 核心修正：两处精确语义修正 + `[TBD-3]` 注释清理
