...
4. 分型出现 → 两种情况：
   - **第一种**（无缺口）：分型第1、2元素间无缺口 → 分型极值点即线段终点，分型形成即**直接终结**（任何三笔都构成破坏，无须第二特征序列；067:28/194）
   - **第二种**（有缺口）：分型第1、2元素间有缺口 → **须**从分型极值点开始的反向序列出现第二特征序列分型才终结（最不直接；067:38/196）
5. 结算锚验证：新段前三笔必须重叠
...
- 连续三笔无重叠：不成段
- 偶数笔的结构不是线段
- 古怪线段（78课）：第三笔在第一笔范围内、无法构成特征序列时，破坏笔未在该方向发展成线段破坏 → 旧段延续。与第67课"两种情况"正交——第一种(无缺口)是分型一出现即直接终结，古怪线段是另一层（笔破坏≠线段破坏）现象，勿混

## 未结算问题

1. ~~**v0 和 v1 的输出差异多大？**~~ **已验证**：v0贪心段数是v1的2-3倍。v0每3笔重叠即成段，v1等待特征序列分型才断段。（谱系003，`test_segment_v0_vs_v1_comparative.py` 10/10通过）
2. ~~**退化段归因于口径还是笔？**~~ **已回答**：退化段根因在笔层（旧笔漏判短促波动），非口径差异。干净笔序列上两口径都不产生退化段。（谱系001+003联合验证）
3. ~~**v0 是否应该保留？**~~ **已决断**：v0 降级为参考实现（reference implementation），v1 为唯一正式口径。详见下方"v0/v1 口径决断"节。
4. **v1 状态机审计结果**（2026-02-16）：

   | 检验点 | 状态 | 说明 |
   |--------|------|------|
   | 第一种情况（无缺口→分型出现即直接终结） | ✅ | `scan_trigger()` + 主循环正确实现 |
   | 第二种情况（有缺口→第二特征序列出分型） | ✅ 已修复 | `_second_seq_has_fractal()` 从同向笔独立构建第二特征序列 |
   | 结算锚（新段前三笔重叠） | ✅ | `_three_stroke_overlap` 防护有效 |
   | 包含作用域（71课约束） | ✅ | reset机制隐式规避跨段包含，结果正确 |

   **第二种情况修复详情**（v1.2）：
   - **旧实现**：`_has_any_fractal_after(self.std, i+1)` 检查第一特征序列（反向笔）的延续——这是错误的笔集合
   - **新实现**：`_second_seq_has_fractal(strokes, seg_dir, b_stroke)` 从分型中心之后收集**同向笔**（= 新潜在线段的反向笔），独立做包含处理，检查任意分型
   - **原文依据**：第67课 L38 "从该分型最高点开始的向下一笔开始的序列的特征序列"明确指向新的独立序列
   - **测试**：37/37通过（gap分类16 + 尾窗扫描6 + v0/v1对比10 + 第二序列边界条件5）
...
**谱系引用**: 003-segment-concept-separation.md

**含义**:
- v0 不进入正式管线，不作为下游模块的输入源
- v0 代码保留但标记为参考实现（reference implementation），仅供对比/调试
- 上层模块（中枢、走势、买卖点）只对接 v1
