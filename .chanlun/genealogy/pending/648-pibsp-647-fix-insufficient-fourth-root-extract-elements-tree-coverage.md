...
negation_source: "编排者价值判断（chat「子声部.pdf」27页严格推导，权威）+ 本号 L2 源码事实（647 修复后子声部仍=0，host-hit 2-8%/parent_active=0）。裁决 D 超越本号上呈的 A/B/C 三选一。"
...
contradiction:
  description: |
    【原矛盾 + 编排者裁决 D 消解】647 诊断 π^bsp 子声部=0 三根因（pi_bsp_timing.rs）：
    ...
    本号实装 647 修复路径后子声部仍恒=0（OKLO/CL/BTC），坐实**第四根因**：
  ...
  trigger: "task#2 实装 647 修复后子声部仍=0（本号坐实第四根因）→ 上浮 A/B/C → 编排者经「子声部.pdf」27页裁决 D（视图分离），超越三选一。"
...
negated:
  description: "(1) 647 三根因清单完备。(2)【原本号倾向 C】子声部=0 是 P1(单最高级降序树)∧P2(严格右端点命中) 的不可避免定理（结构必然，不可修）。(3)【645 倾向】π^bsp 子声部层是 wrong object（goal#5 多声部对冲是本质错误问题）。(4) 修复必在 A/B/C 三方向中三选一。"
  why_negated: "(1) 本号实装 647 全部修复路径子声部仍=0（第四根因 extract_elements 树覆盖局限）。(2) 编排者裁决 D：第四根因真因是 host^op=host^struct 混用（可修），非 P1∧P2 不可避免——分离 K_i(endpoint-complete) 即可修，C 被否。(3) 裁决 D 推翻 wrong object 倾向：子声部=0 是 host 宇宙不足（K_i 未建），非 goal#5 本质错误；命题A(645)π^cov≠π^bsp 仍成立，但 π^bsp 子声部层是可激活的真对象（待 K_i 实装）。(4) 编排者经「子声部.pdf」27页给出第四方案 D（视图分离），超越三选一——A/B/C 都在『单一 host 宇宙』前提下，D 否定该前提（双视图）。"
...
new_output:
  definitions:
    ...
    - "647 修复路径（按 carrier 匹配 + host-miss 不丢弃）是必要非充分（缺 K_i 视图）；第四根因（extract_elements 树覆盖 36-42%）经裁决 D 归因为 host 宇宙错配（用 T_i 而非 K_i），可修。"
  code_changes: |
    rust/src/bin/pi_bsp_timing.rs（已落 647 修复，128+ / 52−）：carrier 改真嵌套塔，host-miss 退化 ∂ 根，出场证书按 carrier 配对。
...
impact:
  affected_modules:
    - "rust/src/bin/pi_bsp_timing.rs → 647 修复已落；待 task#14 接 K_i 操作 carrier forest（host^op 定义域）。"
  ...
  affected_definitions:
    - "647（生成态）：本号坐实其根因清单不完备（必要非充分）；裁决 D 给出充分解（K_i 视图）。维持生成态待 /ritual（可随 648 一并辨认）。"
    ...
    - "231（settle）：本号是有效域≠定义域活实例（647 修复 L1 fixture 有效，L2 真实数据子声部=0）；裁决 D 修正有效域（K_i endpoint-complete 扩 host-hit 域）。维持 settle。"
...
related_records:
  parent: "647（π^bsp 子声部=0 实装根因；本号坐实不完备 + 编排者裁决 D 给充分解）"
  children: []
  related:
    ...
    - "231（有效域≠定义域——647 修复 L1 有效 L2 失效；裁决 D K_i 修正有效域）"

epistemological_levels:
  - proposition: "647 修复路径（按 carrier 匹配 + host-miss 不丢弃）实装后子声部仍=0（OKLO/CL/BTC）"
    level: "L2（pi_bsp_timing.rs 实测三标的：host-hit 2.4%-7.7%，parent_active 恒 0）"
    increment: "高：647 根因清单不完备的 L2 坐实（修复后行为未翻转）"
...
### 检查范围
同轮（642-648）∪ 1-hop（647/644/645/638/547/231）∪ Hub（645 覆盖/择时对象、231 有效域、638 hostOf）。

### 张力1：vs 647（parent）——裁决 D 给充分解
647 三根因必要非充分；本号坐实第四根因；裁决 D（K_i 视图）给充分解。本号是 647 的充分化。无矛盾。
...
647 修复 L1 fixture 有效、L2 真实失效=231 活实例；裁决 D（K_i endpoint-complete）修正有效域。印证 231。无矛盾。
...
- 第1层：本号 × 647 → 充分解（净新发现高：第四根因从结构必然修正为 host 宇宙错配可修）。
...
- **647（生成态，parent）**：裁决 D 给充分解；维持生成态待 /ritual（可随 648 一并辨认）。
