...
**前置**: 无（首条记录）
**创建时间**: 2026-02-15
**更新时间**: 2026-02-16（第五次修订：全管线集成测试13/13通过，BiEngine缺陷修复）
...
- [x] BiEngine `merged_to_raw` 传参缺陷修复（mode="new" 在引擎中不再是死代码）。
- [x] 全管线集成测试（13/13通过）：`test_new_bi_pipeline.py`
  - 新笔笔数 >= 旧笔（包含压缩不再阻碍成笔）
  - 新笔+v0线段退化段数 <= 旧笔+v0线段（核心假说验证）
  - BiEngine mode="new" 端到端运行
- [x] 在真实市场数据上端到端验证新笔效果（DataBento AAPL 1分钟，退化段率=0%，18线段无一退化）。
- [x] 确认不需要在线段层面增加后置过滤（真实数据退化段率=0%，新笔从根源解决）。
...
本记录已结算（2026-02-16）：
1. ~~假说验证~~ ✅ 已完成
2. ✅ 真实市场数据验证通过（AAPL 1分钟，退化段率=0%，v0/v1比值=4.72）
3. ✅ mode="new" 保留为可选模式，编排者批准结算
