...
- [x] v1 的第一种情况（无缺口→分型形成即直接终结）是否正确实现？→ ✅ `scan_trigger()` + 主循环正确（括号语义对齐博文067:28/194；原"等待线段破坏"系与 xianduan.md:169 同源转写笔误，谱系612 勘误）
...
本条谱系中的"包含"概念特指缠论笔的包含处理（相邻K线的合并规则）。168号谱系引用本条时涉及的是精神分析语境下的 alienation-separation 辩证法中的"包含"概念——两个语境中的"包含"含义不同，引用时必须标注语境区分。
