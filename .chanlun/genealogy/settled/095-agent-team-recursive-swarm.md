...
### 真递归是默认模式（编排者 INTERRUPT 修正）

095号初始版本将 Trampoline 定为默认、真递归定为可选。编排者否定了这个选择。

**修正后**：真递归拓扑异步自指蜂群是默认模式。Trampoline 是退化特例。

这与 056号"递归是存在方式，线性是退化特例"完全同构：
- 056号：线性执行需要理由（必须证明任务间存在严格数据依赖），并行不需要理由
- 095号修正：Trampoline 需要理由（必须证明子任务不需要独立治理），真递归不需要理由

| 模式 | 机制 | 何时使用 |
|------|------|----------|
| **真递归**（默认） | teammate 创建子 team → 子 team 内部自治 → 结果向上回传 | 默认——除非子任务不可分解 |
| **Trampoline**（退化特例） | 任务裂变 → Lead 重新 spawn → 同一层弹跳 | 仅当任务不可分解为需要独立治理的子蜂群时 |
