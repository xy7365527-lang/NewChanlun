...
**原文**：
- "黄白线不能创新高"（第25课）— T6 维度
- "柱子的面积或者伸长的高度不能突破新高"（第25课）— T5 + T7 维度

**选择**：v1 实现三维判断：
- T5: MACD 柱子面积（`macd_area_for_range`）
- T6: DIF 峰值（`dif_peak_for_range`）
- T7: 柱子伸长高度峰值（`histogram_peak_for_range`）
- `_check_three_dim_divergence()` 组合三个维度

**依据**：
- 原文 "面积**或者**伸长的高度" 暗示多维度判断
- T6（黄白线创新高）是独立于柱子面积的维度

**未结算**：T5/T6/T7 的组合方式（"或" vs "且"）— beichi.md #2 记录此问题但实现中选择了三维任一满足即判定背驰
...
### 3. 边界条件
- C2（三维组合方式）未完全结算——"或 vs 且"问题可能影响判断灵敏度
