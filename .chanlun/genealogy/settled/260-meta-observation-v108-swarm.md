...
1. 260-1（team 状态丢失）：定理类。162号推论在 compaction 边界案例下成立。→ resolved
2. 260-2（操作者间收敛）：定理类。221号扩展。→ resolved
3. 260-3（K4 顶点重命名）：选择类。编排者决断已执行（259号+260号：不重命名C→Au，加操作语义标注）。→ resolved
