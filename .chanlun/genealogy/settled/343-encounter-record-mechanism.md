...
1. ceremony 每轮自动检测偶遇，偶遇候选以 P1 工位形式进入工位列表
2. 偶遇候选需要人工审计（确认/拒绝），不自动结算
3. encounter-records.yaml 是穿越基础设施的增量记录，与 cross-gang-refs.json（全量快照）互补
4. 未来新增设计内模式时，更新 classify_cross_gang_edge() 的模式列表
