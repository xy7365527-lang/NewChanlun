...
1. **L2 验证（核心）**：在真实 relations.jsonl 上运行 Cerf 分析，分岔点与编排者标记的"关键转折"对比。这是 B-M 研究线的最终验证 — **已执行**（363号 L2 否定性结果：加权 Cerf Top-20 precision=0.10，未改善均匀 Morse）
2. **加权 vs 均匀对比**：如果 L2 验证成功（加权 Morse 的 precision 显著高于均匀 Morse），加权 Morse 可替代均匀 Morse 作为默认 — **已执行**（363号：前提不成立，加权未优于均匀，14/21 ground truth 中均匀排名更优）
3. **Cerf 分岔作为蜂群监控信号**：如果 L2 验证确认分岔点与关键转折有强相关，Cerf 分岔检测可集成到 ceremony_scan 中作为"拓扑结构质变预警" — **已执行**（363号：前提不成立，分岔点与关键转折无强相关，不集成）
