...
1. **矛盾检测的零参数原则**：编排者否定 K 参数化后确立的原则——拓扑计算引擎的所有判断应来自拓扑内禀结构（如 Morse critical/tree），不依赖外部参数 → `resolved: 方法论记录——零参数原则已在390号2000步长运行中持续生效（Morse critical过滤贯穿全程，encounter密度59-67%稳定）。此为设计原则，不需要实验验证` [audited: v198, consumed]
2. **fold 质量依赖 negate_a 质量**：387号的实验结果（59次fold成功）建立在本修正之上。未修正时的 fold 质量不可信 → `resolved: 被390号数据回答——2000步长运行使用修正后negate_a，116个settled cycles + 484个blocked folds。fold质量在修正后的negate_a上持续稳定，确认依赖关系成立` [audited: v198, consumed]
3. **自引用振荡模式的泛化**：任何在图上做增量修改的操作都可能产生"与自身产物交互"的 false positive。这是拓扑计算引擎的通用设计约束 → `resolved: 被390号数据回答——2000步中syn_/anti_排除机制持续运行无回归。泛化观察成立但无新实例出现，当前为方法论记录` [audited: v198, consumed]
