...
4. Channel adapter 作为事件源之一接入 daemon 事件循环 [covered: v214-swarm, channel_adapter.py — ChannelEvent/ChannelAdapter/WebhookAdapter/EventSourceManager + daemon_loop.py drain_events() 集成]
5. ACP bridge 同上——另一个事件源 [covered: v214-swarm, acp_bridge.py — ACPRequest/ACPResponse/ACPBridge(ChannelAdapter) + stdio NDJSON 协议 + method routing(ping/status/task/feed)]
6. Daemon 作为 systemd/launchd 服务部署（参考 OpenClaw daemon 适配层） [covered: v214-swarm, deploy/ — fengliang.service(systemd) + com.fengliang.daemon.plist(launchd) + fengliang-task.xml(Windows Task Scheduler)]
