...
1. **traversal.py 已修改**：`_check_articulation_encounter()` 从度量阈值改为拓扑不一致判据
   - status: 已完成（v231-swarm/articulate-refactor）
   - [covered: v232-swarm, consumed — traversal.py:1102 已实装 imbalance_type 布尔判据]
2. **daemon.py 已修改**：articulation block 写入格式从 cooc_weight/ta_count/step_gap 改为 imbalance_type/reason
   - status: 已完成（v231-swarm/articulate-refactor）
   - [covered: v232-swarm, consumed — daemon.py:1027 已实装 imbalance_type/reason 字段]
3. **block_topology_persistence.py 已修改**：`write_articulation_block()` 接口同步更新
   - status: 已完成（v231-swarm/articulate-refactor）
   - [covered: v232-swarm, consumed — block_topology_persistence.py:365 接口已包含 imbalance_type 参数]
