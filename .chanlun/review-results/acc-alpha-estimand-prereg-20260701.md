...
| 路径 | 方法 | beta 如何被剥离 | 等级 |
|------|------|----------------|------|
| **A（L2 主路，单标的 BTC）** | **分层内 δ 置换**（stratified permutation）：桶键 `(ℓ,δ,bsp_class)` 内固定 entry/持有期/成本，只 shuffle δ 标签，200 次固定种子 Fisher-Yates（666 方法1）。统计量 `S=Σ_bucket|μ̂|`，`perm_p` = #{S_perm≥S_obs}/200。 | beta 被分层**吸收**（同桶同持有窗共享 beta，置换只动方向标签）——绕开显式减法退化（665：分层内置换 beta 被分层吸收，不需分离） | **L2**（BTC 单标的，可否证但不交叉验证） |
