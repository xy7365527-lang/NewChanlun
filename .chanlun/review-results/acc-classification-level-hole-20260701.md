...
|---|---|---|---|---|---|---|---|---|
| 0 | 2500 | 721 | 0 | 0 | 721 | 721 | 721 | 0 |
| 1 | 756 | 183 | 0 | 77 | 106 | 183 | 18 | 165 |
| 2 | 220 | 61 | 0 | 22 | 39 | 61 | 27 | 34 |
| 3 | 61 | 15 | 0 | 4 | 11 | 15 | 5 | 10 |
| 4 | 14 | 4 | 0 | 2 | 2 | 4 | 2 | 2 |
| 5 | 4 | 1 | 0 | 1 | 0 | 1 | 1 | 0 |

## H1/H2/H3 判定（中间级 = level 1..levels_seen_max-1）
- level1: bsp_pre=183 (一/二/三=0/77/106) Γ非Flat=183 sig_post=18 → 有信号
- level2: bsp_pre=61 (一/二/三=0/22/39) Γ非Flat=61 sig_post=27 → 有信号
- level3: bsp_pre=15 (一/二/三=0/4/11) Γ非Flat=15 sig_post=5 → 有信号
- level4: bsp_pre=4 (一/二/三=0/2/2) Γ非Flat=4 sig_post=2 → 有信号
...
**depth=0（退化 base-case）：478/774；depth≥1（真跨级触达）：267/774**
...
**小转大通过**：29 条（占通过门总数 774 的 3.75%）。level==1 的通过数已隐含 C3(新中枢+突破) 硬门；level>=2 仍为 C2-only（旧 C3 same_side_same_center 字段降为诊断，不参门）。
- 通道分离：区间套（descend=Some，depth 直方图 745 条）与小转大（descend=None，29 条）输入域 Some/None 互斥（codex §6-4 构造同义反复，非经验重叠）。
- **小转大域触达/C2/旧C3 分项（诊断，旧 C3=same_side_same_center 不参门）**：路由到 Xzd=240，其中 C2 成立=97，旧 C3 成立=0，门通过=29。
...
- lvl==1 routed=165
- last_zs_exists=165 (100.00%)：s 跨度内存在次级中枢
- same_side_l0_type3_any=165 (100.00%)：存在同向 L0 Type3（不问 center）
- same_center_any=0 (0.00%)：存在任意侧 Type3 其 center==last_zs
- same_side_causal_ok=165 (100.00%)：同向 Type3 候选中存在 source_index<=confirm_index 者（否则=时间确认问题）
...
- lvl>=2 routed=75，sub_bsp Type3 点总数=3150
- **重封通过**：默认 300K 窗基线锁定 routed=75 / sub_bsp_type3_total=3150（裁定A+#123 后新真值，2026-07-03 测定）。
...
- **配对机制**：next_opp 表跨级别混合（所有 level 信号按 entry_bar 排序）——中间级信号找「下一个反向信号」时不分级别，几乎总配 level0（信号 717/770 是 level0）。decomp.level=入场信号 level（配对出场 level 不影响）。
