...
**窗口**：bars=20000（2026-05-18→2026-05-31，全量=4613599），max_bars=20000。
...
|---|---|---|---|---|---|---|---|---|
| 0 | 173 | 49 | 0 | 0 | 49 | 49 | 49 | 0 |
| 1 | 53 | 1 | 0 | 1 | 0 | 1 | 0 | 1 |
| 2 | 16 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| 3 | 4 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |

## H1/H2/H3 判定（中间级 = level 1..levels_seen_max-1）
- level1: bsp_pre=1 (一/二/三=0/1/0) Γ非Flat=1 sig_post=0 → 门滤空(H1候选)
- level2: bsp_pre=0 (一/二/三=0/0/0) Γ非Flat=0 sig_post=0 → 门前空(H3候选:架构)
...
**depth=0（退化 base-case）：35/49；depth≥1（真跨级触达）：14/49**
...
**小转大通过**：0 条（占通过门总数 49 的 0.00%）。level==1 的通过数已隐含 C3(新中枢+突破) 硬门；level>=2 仍为 C2-only（旧 C3 same_side_same_center 字段降为诊断，不参门）。
- 通道分离：区间套（descend=Some，depth 直方图 49 条）与小转大（descend=None，0 条）输入域 Some/None 互斥（codex §6-4 构造同义反复，非经验重叠）。
- **小转大域触达/C2/旧C3 分项（诊断，旧 C3=same_side_same_center 不参门）**：路由到 Xzd=1，其中 C2 成立=1，旧 C3 成立=0，门通过=0。
...
- lvl==1 routed=1
- last_zs_exists=1 (100.00%)：s 跨度内存在次级中枢
- same_side_l0_type3_any=1 (100.00%)：存在同向 L0 Type3（不问 center）
- same_center_any=0 (0.00%)：存在任意侧 Type3 其 center==last_zs
- same_side_causal_ok=1 (100.00%)：同向 Type3 候选中存在 source_index<=confirm_index 者（否则=时间确认问题）
...
- lvl>=2 routed=0，sub_bsp Type3 点总数=0
- 本窗无 lvl>=2 路由到 Xzd 的信号，真封断言跳过（无样本）。
...
- **配对机制**：next_opp 表跨级别混合（所有 level 信号按 entry_bar 排序）——中间级信号找「下一个反向信号」时不分级别，几乎总配 level0（信号 45/45 是 level0）。decomp.level=入场信号 level（配对出场 level 不影响）。
