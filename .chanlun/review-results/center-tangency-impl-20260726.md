...
| 靶向验证① | v0 新口径 ↔ v1 延伸谓词 **不一致 0 次**（OKLO 2547 次判定 / BZ2024 3505 次）；旧口径不一致 6 / 14 次。**限定：谓词形式等价，非中枢集合等价**——见 §5.1 有效域声明 |
...
- **下游真空照实标注**：单点中枢若成立，其下游（定理二分支/三类买卖点/破坏/092 监视器 Z 值）全落原文真空。
  **输入域实测（ZG==ZD 输入，落码口径）**：OKLO **0 次**、BZ2024 **2 次**（均在 L1）；Python 两版
  ...
  `center_from_segments`（`:162`）/ `center_from_window`（`:192`）两处均为 `if zd > zg { return None }`
  （单点 `zd==zg` 通过），同文件 `:152` doc 明写「`ZD_B=ZG_B` 单点核心合法」。故该分支在 Rust 生产链上
  **可达且判成立**，下游真空是**激活的**。此矛盾（Lean 弱 / Python 严格 / Rust 弱）**超出 #290 裁定 B 的
  有效域**，已另立票 **#321**（成立口径三方矛盾，grilling）。若未来原文/新证据出现，另立票重议。
...
> **有效域声明（照实，防止过度推广）**：本节证明的是 **v0 `_has_overlap` 与 v1 延伸谓词在相同 `(zd, zg, seg)` 输入上的取值等价（谓词形式等价）**。这**不等价于**「v0 链与 v1 链产出的中枢集合相同」——两链的候选起点、延伸/终结规则、扫描推进（v1 `_scan_zhongshu` 跳段口径）均不同，**中枢集合等价未验证、不在本票有效域内**。上面的靶向验证走的是 monkeypatch 记录判定入参、逐点比对，**不是**全链产出对拍。
...
9. **手写 Lean 对齐声明无机器锁**：本票在 docstring / 文档中登记的「对齐 Lean `Origin.CenterStates.CenterExtension`（弱）/ `CenterBroken`（严格）」等对齐声明是**人工读 Lean 源码后手写**的，**无任何机器检查锁住**（无 parity 测试、无 CI 断言）。Lean 侧若改动，这些声明会静默过期。
10. **单点计数（2 次）的轨迹依赖**：§3.2 登记的 BZ2024 单点中枢 **2 次**是在**落码后（弱）轨迹**上统计的。相切改弱后中枢候选起点整体平移，单点输入集合本身随轨迹变化——**该计数不是轨迹无关的常量**，换轨迹（旧严格口径 / 其他 `stroke_mode`）会得到不同数字。与 §5.1 脚注登记的相切计数轨迹依赖（OKLO 6 / BZ2024 14 vs 旧轨迹 6/13）同源。
