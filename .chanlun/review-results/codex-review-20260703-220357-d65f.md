# Codex review — 2026-07-03 22:03:57 UTC

## 元数据

- **mode**: review
- **subject**: Task #145 Q5 复审：A/C 次级别走势区间化 + reentry episode 定界修复。前审 verdict=fail（#1 locate_departure_move_a 无 reentry 边界桥接独立 episode；#2 c_start_cache λ_C 无 reentry 检测且按 c_idx 缓存错误；#3 C 侧单段兼容无独立测试；#4 探针/oracle 平行实现 fork 风险）。本轮修复：共享原语 episode_start_in（边界=窗口内最后回中枢段，判据与 judge_pan_div reenters 相同）+ departure_move_c_start（λ_C 按 (c,dir,seg.start) 逐段计算，partition_point 定界）；生产 extract 循环/type1_funnel_dx 探针/judge_first_orig oracle/judge_pan_div 四处全走同一 helper。请审：修复是否忠实闭合前审 #1/#2；episode 边界判据（反向段端点回核心内侧 Down:end>=zd / Up:end<=zg）是否与 Q5『次级别走势类型区间』语义一致；旧 fixture 更新（中间反向段从回核心改为不回核心）是否合法；单段兼容是否保持 bit 相同
- **model**: codex-cli
- **timestamp**: 2026-07-03 22:03:57 UTC
...
Task #145 Q5 复审：A/C 次级别走势区间化 + reentry episode 定界修复。前审 verdict=fail（#1 locate_departure_move_a 无 reentry 边界桥接独立 episode；#2 c_start_cache λ_C 无 reentry 检测且按 c_idx 缓存错误；#3 C 侧单段兼容无独立测试；#4 探针/oracle 平行实现 fork 风险）。本轮修复：共享原语 episode_start_in（边界=窗口内最后回中枢段，判据与 judge_pan_div reenters 相同）+ departure_move_c_start（λ_C 按 (c,dir,seg.start) 逐段计算，partition_point 定界）；生产 extract 循环/type1_funnel_dx 探针/judge_first_orig oracle/judge_pan_div 四处全走同一 helper。请审：修复是否忠实闭合前审 #1/#2；episode 边界判据（反向段端点回核心内侧 Down:end>=zd / Up:end<=zg）是否与 Q5『次级别走势类型区间』语义一致；旧 fixture 更新（中间反向段从回核心改为不回核心）是否合法；单段兼容是否保持 bit 相同
...
**问题**：`judge_pan_div` 的 A 侧仍是“前一次同向离开末段”，没有升级为前一次离开 episode 区间。C 侧已经用 `departure_move_c_start` 得到 `[lambda_c, seg.end]`，但 A 侧只取 `a.start_index..a.end_index`。如果前一次离开也是多段走势，中间反向段未回中枢，MACD 面积会被低估，可能误拒盘整背驰证书；这不忠实于 Q5“次级别走势类型区间”口径。

**位置**：`rust/src/theta_v0/classifier/signal.rs:560`

**严重性**：重要

**修复建议**：抽一个返回完整 episode 的 helper，例如 `departure_episode_interval(segments, c, dir, until_start) -> Option<(usize, usize)>`。`departure_move_c_start` 可以只投影 `.0`。`judge_pan_div` 中 A 应取“分隔回中枢段之前的上一 episode 区间”，而不是上一根破核心同向段。补一个 PanDiv A 多段、未 reentry 的 fixture。

已接受点：趋势一类路径上，`locate_departure_move_a` 已用最后 reentry 切 episode，前审 #1 闭合；`departure_move_c_start` 按 `until_start` 逐段算 λ_C，前审 #2 闭合；C 单段兼容已有独立测试；旧 fixture 把中间反向段改成未回核心是合法的，用来覆盖“同一 episode 多段离开”。

验证限制：本地 workspace 为空，无法运行测试；以上基于你提供的 diff 静态复审。

```yaml
---stance-declaration---
verdict: conditional
stances:
  trend_locate_departure_move_a_reentry: accept
  trend_departure_move_c_start_reentry_and_cache_fix: accept
  q5_episode_boundary_predicate: accept
  old_fixture_reentry_to_non_reentry_update: accept
  c_side_single_segment_compatibility: accept
  pan_div_a_side_intervalization: needs_work
concessions:
  - previous_q5_issue_1_locate_departure_move_a_reentry_bridge
  - previous_q5_issue_2_c_start_cache_lambda_c_reentry
  - previous_q5_issue_3_c_single_segment_test_missing
---end-stance---
```
