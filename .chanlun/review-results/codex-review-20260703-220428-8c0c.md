# Codex review — 2026-07-03 22:04:28 UTC

## 元数据

- **mode**: review
- **subject**: Task #145 Q4 审计：盘整背驰承接路由。裁决：τ=Consolidation 的背驰不冒充同级 B1/S1 也不消失——PanDiv 证书路由 N^δ（PanDiv⟹∃e<ℓ Conf_e）或 XZD（PanDiv⟹XZD_{ℓ↓e}），两门皆闭诚实丢弃。实装：signal.rs judge_pan_div（同一中枢两次同向离开：破核心+回中枢段存在+前次离开 A+C<A 背驰，AbcDivergence is_trend=false）产 PanDivCert（零 six-bit）；decompose.rs center_block_kind（ownership 分区：C_i 归 R(i-1,i) 块、C_0 归 B₁）；mod.rs LevelState.pan_div 与 cached_bsp 同 key memo；econ_positive.rs collect_signals 消费 pan_div 走 pan_div_gate_pass（通道1 descend_type1_anchor_depth 次级别 Type1 锚 / 通道2 xiaozhuanda_confirm gate_pass），trigger=NestTrigger::PanDivConsolidation 独立归因桶，dx 镜像 bit-exact 复制。请审：判据链是否忠实于裁决（不冒充/不消失/承接二通道）；memo 同批缓存是否与 bsp 锁步无失效缺口；econ 层 z/MuClass 口径（i_class=0/parent_dir=Root/nest_depth=None）是否诚实；BTC 牛市窗实测 pan_div L0=45 L1=13 L2=1
- **model**: codex-cli
- **timestamp**: 2026-07-03 22:04:28 UTC
...
Task #145 Q4 审计：盘整背驰承接路由。裁决：τ=Consolidation 的背驰不冒充同级 B1/S1 也不消失——PanDiv 证书路由 N^δ（PanDiv⟹∃e<ℓ Conf_e）或 XZD（PanDiv⟹XZD_{ℓ↓e}），两门皆闭诚实丢弃。实装：signal.rs judge_pan_div（同一中枢两次同向离开：破核心+回中枢段存在+前次离开 A+C<A 背驰，AbcDivergence is_trend=false）产 PanDivCert（零 six-bit）；decompose.rs center_block_kind（ownership 分区：C_i 归 R(i-1,i) 块、C_0 归 B₁）；mod.rs LevelState.pan_div 与 cached_bsp 同 key memo；econ_positive.rs collect_signals 消费 pan_div 走 pan_div_gate_pass（通道1 descend_type1_anchor_depth 次级别 Type1 锚 / 通道2 xiaozhuanda_confirm gate_pass），trigger=NestTrigger::PanDivConsolidation 独立归因桶，dx 镜像 bit-exact 复制。请审：判据链是否忠实于裁决（不冒充/不消失/承接二通道）；memo 同批缓存是否与 bsp 锁步无失效缺口；econ 层 z/MuClass 口径（i_class=0/parent_dir=Root/nest_depth=None）是否诚实；BTC 牛市窗实测 pan_div L0=45 L1=13 L2=1
...
- **问题**：`judge_pan_div` 的 A 侧只取“前次同向离开末段”，没有取完整前次离开 episode。Q5 已把 A/C 定义升级为走势区间，PanDiv 这里却只对 C 做区间化、A 仍是单段，定义不对称；多段前次离开会被低估，导致本应承接的盘整背驰被漏掉。
- **位置**：`rust/src/theta_v0/classifier/signal.rs:560`
- **严重性**：重要
- **修复建议**：为前次离开也定位 episode 起点，`seg_a` 应为前次离开区间 `[lambda_a, a.end_index]`，不要只用末段 `(a.start_index, a.end_index)`。

- **问题**：`collect_signals` 在 `pan_div_gate_pass` 之前写入 `seen_pan`。如果 PanDiv 证书首见时两门未开、后续 bar 下级 Type1/XZD 承接证据才出现，该证书会被永久跳过。这会把“暂未承接”实现成“永不重试”，存在“诚实丢弃”时点过早的问题。
- **位置**：`rust/src/theta_v0/backtest/econ_positive.rs:430`
- **严重性**：重要
- **修复建议**：只在 gate pass 后写入 `seen_pan`；如果确实要求首见即终局，需要把这个时序作为裁决前提写入规格并加反例测试。

- **问题**：PanDiv 的增量缓存复用完全绑定 `cached_bsp_key=(centers.len, upper_moves.len, struct_len)`。PanDiv 额外依赖 `center_block_kind/decompose` 的块类别；如果中心内容或块归属在同长度下变化，`cached_pan_div` 会随 `cached_bsp` 误命中。现有 diff 没看到覆盖“长度不变但 ownership/kind 改变”的失效测试。
- **位置**：`rust/src/theta_v0/classifier/mod.rs:1281`
- **严重性**：建议
- **修复建议**：要么证明该增量层只缓存 sealed 前缀且同长度内容不可变；要么把能代表 centers/block frontier 的版本号或尾部签名并入 key，至少补一个同长度归属翻转测试。

```yaml
---stance-declaration---
verdict: fail
stances:
  pan_div_a_interval_definition: reject
  seen_pan_before_gate_pass: needs_work
  pan_div_memo_key_invalidation: needs_work
concessions: []
---end-stance---
```
