...
- 本次窗口：bars=300000（2025-11-04→2026-05-31，max_bars=300000）
- train_frac=0.6，切分 bar=180000（2026-03-09），train=[0,180000) / holdout=[180000,300000) 无重叠
- 全窗 n_signals=722

## acc-multilevel-sample：level0-2 全历史信号数分布
...
train_frac=0.6，切分日 2026-03-09。每个 (level,δ) 独立评估（不从 train-only 挑类——无跨级别选择偏差）。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2 | -1 | 1 | 6.8140e2 | 0 | 0.0000e0 | 0.0000e0 | 0.0000e0 | **0.0000e0** | — | -0.0000e0 | 1.000 | ✗ |
  胜率=0/0

**acc-highlevel-mu 判定**：
- level2+ holdout LCB>0 占比 = 0/1 (0%)
...
## 结果包六要素
1. **结论**：
   - acc-multilevel-sample：level0-2 全历史信号数分布见上表。level3+ 全部 <30（Le Cam 硬墙，结构性稀疏）。
   - acc-highlevel-mu：level2+ holdout LCB>0 占比=0/1（0%）。全否证，高级别无稳健 OOS alpha（L2）。
...
3. **边界条件**：单标的 BTC L2——结论翻转条件：(a) max_bars=300000（若<461万则为截断窗结论）；(b) train_frac=0.6 改变切分点结论可能变化；(c) 高级别 n 过少（Le Cam 约束）时 LCB 由 se 主导而非 μ；(d) 不同 bootstrap 种子理论上可影响 p（本实现固定 LCG 种子确定性）。
