...
**编排者假说被 L3 证实并精化：穿仓 = 强牛里持有「冻结空腿」（开仓后 switches≈0–1、bars_short=10⁵⁺、做空骑整段反转）无界失血。根因 = 556（最高级别走势完成稀疏）使一条腿一旦开仓就无法在趋势反转时关闭/翻转 + reading_b 缺升跌完备性/父级门控。这是双刃：腿方向与最终主趋势一致则冻结=骑牛巨利（ES 顶层 L3 @1131 开多→7368，+694k；BTC 顶层 L2 @4237 开多→63086，+75k），与反转后主趋势相反则冻结=穿仓。**

**失血空腿的级别因标的而异（编排者「最高级别 vs 次级别」判据）：**
- **OKLO（top=L2）= 最高级别冻结空腿**：L2 @bar25451 价 7.29 开空——OKLO 从 17.80 跌到 7.29（−59% 真实下跌，开空合法），但随后反转涨到 72.46（+894%），空腿 switches=0 从不关闭 → −91k~−102k 穿仓。**不是塔矮误标，是 556 冻结使空腿无法在反转向上时关闭**（21:40 违反在反转后）。
- **BTC（top=L2）= 次级别 L1 冻结空腿**：顶层 L2 正确 Long(+75k)，但 L1 冻结空腿 −98k~−132k > 顶层 → 穿仓。
- **ES（不穿仓）= 顶层 L3 正确 Long 主导**（+694k），sub 空腿被碾压，liq=0。

**穿仓+杠杆同根 = YES**（多腿无父级门控，多空同在场既造杠杆又造穿仓）。**修法 = honor 第21课:40 升跌完备性 + 解 556 冻结：确认上涨趋势（最高级别 Up）下，所有级别既有空腿必须关闭/翻多（不冻结骑反转），新空腿只能是有界回调（完成即平）**——即给 reading_b 补 instances/route_bsp 已有的父级门控 + 解最高级别完成稀疏。不是 ANCHOR（方向由结构涌现），不是摩擦。**

> **关键 nuance（resolve 开放轴#1）**：OKLO L2 开空不是「假翻空」（开仓时跟随真实 −59% 下跌，方向正确）——bug 是**冻结**（556 使其在 +894% 反转中无法关闭）。所以编排者「假空腿」更精确是「**冻结空腿**」：开仓可能合法，致命的是反转时关不掉。route-bsp-complete #36（强牛 type1 卖点 flip short 吃熊）的真价值在此：**升跌完备性的正面用途是在最高级别走势完成时及时翻转（关空/开空），而 556 冻结正是它失效的根**。
...
### 2.1 OKLO（top=L2）：最高级别冻结空腿（开仓合法 → 反转中冻结失血）
...
- 穿仓时 3 条腿全是 Short（DTOP: L0/L1/L2 全空）。

**★trend-label 时序（resolve 开放轴#1）**：OKLO 起 17.80 → 末 72.46（BH+307%）。L2 腿在 **bar25451 价 7.29 开空**——此时 OKLO 已从 17.80 **跌到 7.29（−59% 真实下跌结构）**。即 **L2 开空不是塔矮误标，是跟随真实大级别下跌（17.80→7.29）正确开空**。BUG 在**之后**：价从 7.29 **反转上涨到 72.46（+894%）**，而 L2 空腿 **switches=0 从不关闭** → 骑整段反转做空 → 穿仓。**根因 = 556（最高级别走势完成稀疏）使空腿在趋势反转向上时无法关闭/翻转**，而非开仓方向错。
**第21课:40 违反在反转后**：OKLO 从 7.29 起新上涨趋势确认后，应无最高级别卖点 ⟹ 该空腿应关闭/翻多；但 556 冻结使其持续做空整段 +894% 反转。
...
### 4.3 残留开放轴（诚实）
- ~~为何 OKLO 顶层 L2 标 Down~~ **已 resolve（§2.1 trend-label 时序）**：L2 @price 7.29 开空跟随真实 −59% 下跌（17.80→7.29），开仓方向正确；bug 是 556 冻结使其在 +894% 反转中无法关闭。**非塔矮误标。**
- **556 冻结是核心根（已升级为主因）**：冻结空腿关不掉的根 = d_top[k] 稀疏（556）。**单加升跌完备性门控不足——须同时解 556**（最高级别完成信号稀疏致腿冻结）。门控防「开不该开的空」，解 556 防「开了关不掉」。两者缺一：门控解 OKLO 顶层开空（但反转关闭仍需 556 解），556 解使反转能关闭（但 BTC sub 冻结空腿仍需门控约束有界）。两者相对贡献 = 开放轴（route-bsp-complete #36 + 556 解的联合回测未做）。
- **次级别冻结空腿 vs 有界回调的边界**：BTC L1 冻结空（switches=1）vs L0 有界回调（switches 多、+pnl）的判别阈值（switches/bars 比）= 经验阈值，未跨标的交叉验证。
...
| 修法 = 升跌完备性父级门控 + 解 556（非 ANCHOR 非摩擦）| **L1**（架构对照：instances 有门控 vs reading_b 无）|
| OKLO L2 开空跟随真实 −59% 下跌（17.80→7.29），bug=556 冻结非误标 | **L3**（leg-event：bar25451 价 7.29 开空，反转 +894% 不关闭）|
| 冻结空腿双刃（方向对=骑牛巨利 ES/BTC top Long，方向反=穿仓 OKLO）| **L3**（ES L3 @1131 开多/OKLO L2 @7.29 开空 leg-event 对照）|
| 门控 vs 556 解的相对贡献 | **未测（开放轴；route-bsp-complete #36 + 556 联合回测）**|
