...
| 6. 细分优势定理 V(Z)≥V(Y)（p5-6） | oracle 单调性（数学命题） | 无需代码；桶键扩维（#83 role/σ_p）是其操作化 | **已实装（作为设计原则）**（L0 定理，非可执行物） |
...
| 9. 交易选择器 χ=1[LCB>θ∧RiskOK∧ConflictOK], LCB=μ̂−c·se（p8-9） | 置信下界门 | `runner::ChiFilterCtx{theta,z_alpha}` + `mu_estimator` LCB；RiskOK=`k_theta_risk_gate`；ConflictOK=`interp::interpret` ≺_Θ 冲突互斥 | **已实装**（三项齐；见 §缺口对 RiskOK μ_t 退化） |
...
| 十一. 细化优势定理 V(Z)≥V(Y)（p23 / p10） | oracle 单调（同 A-6） | 同 A-6 | **已实装（原则）** |
| 十二. 鞅不可能定理（同 A-12）（p24 / p11） | 数学限制 | 同 A-12 | **已实装（原则）** |
| 十三. 元素数量 \|E\|=O(n) + Eat(e) 命题（结构层，非 alpha 层）（p24-25 / p11-12） | 复杂度 + 覆盖 | `coverage`（Eat/element-coverage）、`persistent`(Eat(e) 祖先命期不变量 I1-I5) | **已实装（结构层）** |
...
| 11. 更本质解：限制有效维数 φ:Z→U 压缩映射（p29-30） | 维数控制 | `MuClass` 桶键=有限低维投影（level/δ/type/role/σ_p/H/ω，非全 z）——正是维数控制 | **已实装**（当前桶键本身就是 φ 压缩） |
...
1. **结论**：完整策略 π（声部/AncOK/净账本/LexArgmin/K_Θ/χ 选择器）**已实装且被回测消费**（`run_theta_v0_pi_inner`→`coverage::pi_theta_step`）；订正 goal 前提「只有逐信号测量」。逐节判定（共 56 节/行）：PDF1 前半 14 节（13 已实装/1 部分）、后半 14 节（12 已实装/2 部分）、PDF2 背驰强度 9 节（6 已实装/1 部分/2 缺口P1）、估计层 19 行（18 已实装/1 部分）。合计 **49 已实装 / 5 部分 / 2 缺口**；跨认识论轴另计 **2 个 P0 + 3 个 P1**（P0 落在「部分」行内：P0-1=后半四、P0-2=A-9/K_Θ 输入退化）。
