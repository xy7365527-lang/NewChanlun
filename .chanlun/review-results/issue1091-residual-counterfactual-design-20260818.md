...
- 失败桶 = **链终止 C 条件号**（cond1..=4，`prod_div_cand_why` 直调生产 `div_cand_fail`，parity 由构造保证）。**对表口径订正**：方向残 13 是 #1028 三轮终验（锚迁 #1052 后）读数；D-3 取段 59 / Extreme 36 / Weak 15 是 #870 二轮（迁前）读数——迁后三桶正是本探针要首次测出的量（ADR 0013 三轮订正只给定性「残余 27.5% = D-3/Extreme/Weak + base_none 54」，未给迁后三桶计数）。
...
1. **H1（最强）cond2 主因 = 取段边界/锚残余**——（迁前二轮）D-3 取段 59 例中应有相当比例是「departure 子段未跨界、但窗口极值 bar 所在子段跨界」型：② 臂会把这些翻为通过或 cond3/4。
