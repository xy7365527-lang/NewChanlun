> 链接转换阅读版；正文语义与原稿一致。签署原字节：[原稿](../../../../payload/sources/newchanlun-1339-architecture-r2-20260909/author/SCHEME-COMPARISON-R2.md)，SHA256 `61536618576684c33f72fb4e0cde6ad99c32ab12a64be3a1b762e048d23363d8`。当前批准状态见归档根APPROVAL.json；原稿中的待审状态保留为发生时记录。
...
当前推荐是以[本轮R2协议](PROTOCOL-R2.md)及[20项明确替代/继承关系](CONTRACT-REPLACEMENTS.md)组成的完整修订架构作为图内SPEC基础，取得本轮独立关项后再请用户采纳。它保留此前从需求独立推导的“逐重权威账＋共同责任”组织，按RF-02将正式结构从经济日志、经营轮和全局观察等待中分离。组织与A2趋同有明确需求依据，但不直接采纳原A2或旧R1全稿；新协议由本体接回编写，独立复核只按其实际阅读射程签署。
...
[分类目录 r2](../../newchanlun-1339-architecture-20260908/author/CLASSIFICATION-CATALOG.md)列出62个轴或公式族、10组跨轴合法性合同，以及32个证明、同步、工程实例或具体待定事项的定位。它承接全部44条结构需求；35条经营需求另接[经营与工程状态合同](../../newchanlun-1339-architecture-20260908/author/independent-design-state-contracts.md)中的16个软件状态轴和各自数值、集合域；其中ES-01/02/03/05/15等涉及发送、未决/未知和观察闭包的条款按R2的CR-14–16替代，不能将旧单M位与新两域协议同时适用。**这些数量是交付目录规模，不是已证的市场状态总数。**
...
下表评价原稿设计合同及本轮所需改造，不评价已经运行的系统。完整原文证据和80项矩阵见[原案重评](../../newchanlun-1339-architecture-20260908/author/EXISTING-REEVALUATION.md)；当前分类接入与旧措辞订正见[分类整合附录](../../newchanlun-1339-architecture-20260908/author/EXISTING-CLASSIFICATION-INTEGRATION.md)。
...
上述P4/P7经济规则继承旧稿已读结论；P4原来同时排序结构修订的部分由R2明确撤销，旧签署不覆盖新协议。R2初评另发现条件阶段不完整、busy拒绝可使交易一直得不到核验、同收据可重复触发调用和未决/未知混淆。作者已分别补完整清单与Unsupported、有限稳定服务、X同ID持久调用占有及知识状态分层，当前待独立关项。最新状态见[本轮交付](../DELIVERY.md)。文档反例闭合不是模型检查、运行活性或场所能力实证。

## 5. 后端处理到前端显示的验收路径

[完整观察底图](../../newchanlun-1339-architecture-20260908/author/FULL-STATE-OBSERVATION.md)提供逐步合同。小转大必须能从必要条件出现追到实际转折、不同级别背驰及当时获知证据；中枢扩展必须能看到参与中枢、核心和外缘、重切成员及实际构造的高级对象。关系成立与高级对象已构造是两个事实，尚缺构件时准确显示缺件。
...
[旧入口终态提案](../../newchanlun-1339-architecture-20260908/author/OLD-ENTRY-DISPOSITIONS.md)按已封存扫描登记绑定真实入口、callee和preset，给保留迁移、只读、独立对照或退役条件。只列家族名不算旧实现收敛；也不能把大量import关系当成独立生产入口。入口去向是设计提案，当前没有停止服务或移走发送权，未结头寸、订单、费用与更正必须按原责任保存。
