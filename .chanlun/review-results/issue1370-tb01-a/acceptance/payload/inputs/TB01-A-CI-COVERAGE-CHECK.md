...
基线：`f6354d3e7fb7fc863ca569e8786bd62807e85c0e`。所有 Cargo、CI、wrapper 和旧测试载体都用 `git show <完整SHA>:<path>` 读取，未读取作者正在修改的 Q 工作文件。`f635` 与 tree/blob 短号有歧义，已解析唯一 commit 后使用完整 SHA。逐文件 SHA 见[机器报告](TB01-A-CI-COVERAGE-CHECK.json)。
