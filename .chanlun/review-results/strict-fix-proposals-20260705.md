...
## §R2 编排者裁决订正（2026-07-05）：非 bug → 有效域缺口（真 bug）

**编排者原话**：「这个属于次级别的回调，然而，难道次级别就不需要背驰了吗？...这个不需要核实，肯定是这样的，因为区间套和递归是默认的。」

**订正**：§R2 此前判定"非 bug（Type2/3 不是 divergence 候选，§4.3 明文）"**只对了一半**——它在高级别 Cand^δ 层面对（Type2/3 不进高级别背驰候选）。但漏了**级别递归**：

- Type2/3 是**次级别走势**（二类=一类后回调，三类=突破后回踩）
- 次级别走势的**结束确认**按缠论级别递归 + 区间套 N^δ 终条件，**必须有次级别背驰**（除非小转大特殊情形）
- 这是缠论核心语法，区间套和递归是默认的——不需要再核实

**有效域缺口确认**：若实装只对 Type2/3 做结构确认（中枢突破/回踩不破 ZG-ZD）而无次级别 divergence 确认，则 97.8% 的 Type2/3 交易确实可能进在次级别走势**未结束**的位置——这是亏损的重大信号侧根因候选。

**R2 修复方向**（新预注册级，135 号冻结先于跑数）：
- Type2/3 entry 路径（econ_positive.rs/cand_predicate.rs/interp.rs）接入次级别 divergence 确认
- 次级别走势结束（回调/回踩完成）须有次级别背驰（N^δ 递归终条件），否则不入场
- 改信号集准入 ⟹ 新预注册 ⟹ bit-exact 不保留（既有 Type2/3 交易集合会缩小）

**与 R5 的关系**：R5 是诊断插桩（env gated 不改语义），R2 是信号集改动（改语义）。串行：R5 完成后立 R2 goal。
