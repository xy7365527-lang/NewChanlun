...
### 4.2 lvl>=2 C3 命中量化探针（实测，2026-07-04 补测）

`acc_classification_level_hole_dx`（300K 窗 L2 探针，`--ignored`）实测（#166 修复编译阻塞、commit 6af9b5fd1a 后）：

```
[c3-breakout-dx] lvl1  routed=212 new_center_exists=13 breakout_ok=3
[c3-breakout-dx] lge2  routed=217 new_center_exists=6  breakout_ok=1
[deadgate-reseal] lvl>=2 routed=217 sub_bsp_type3_total=433
```

| level 子集 | routed | c3_new_center_exists | c3_new_center_breakout_ok | breakout 命中率 |
|-----------|--------|----------------------|---------------------------|-----------------|
| level==1  | 212    | 13                   | 3                         | 3/212 = **1.42%** |
| level>=2  | 217    | 6                    | 1                         | 1/217 = **0.46%** |

**量化裁断输入（关键）**：level>=2 的 C3 硬门参门项 `breakout_ok` 命中率仅 **0.46%（1/217）**。若把 lvl>=2 并入 C3 硬门（`gate_pass` 改为全 level 读 `c3_new_center_breakout_ok`），lvl>=2 的 XZD 吞吐将从 217 塌缩到 1——几乎全拒。level==1 命中率（1.42%）也很低但非零，是现行 level==1 硬门收紧吞吐的实证依据（3 个信号通过硬门）。

探针字段来源：`c3_new_center_exists`/`c3_new_center_breakout_ok` 是 `XzdEvidence` 恒有字段（`econ_positive.rs:1124-1127`），`build_gate_certificate` 对所有 level 都计算；lvl>=2 分支读取（`econ_positive.rs:3851-3852`）与 level==1 分支（3828-3829）同源，**不参门、零行为影响**（gate_pass 在 level!=1 不读这两字段）。全库 `cargo test --release --lib` 全绿（1478 passed, 0 failed）。
...
- 现行分级源自 codex #41/#44 裁定，其前提「level>=2 旧 C3 判据结构性死门」在 #148 升级重切后**在 level==1 已实证失效**（C3 脱 0）；level>=2 由 §4.2 实测证明**未同等失效**（命中率仅 0.46%，接近死门）。
- `gate_pass()` 注释（`econ_positive.rs:1131-1134`）与死门断言注释（`econ_positive.rs:4225-4226`）均已声明「下游口径待裁（上浮 Lead，非本层自决）」——本工位继承此声明。

**codex 裁定所需证据包**（5 项全备齐）：
1. PDF 原文摘录：`一类买卖点.pdf` p.7 Q4（XZD 是承接通道，未定义内部分级）；`区间套.pdf` p.3-8（XZD ∉ 区间套 rung 语义）。
2. 现行分级判据源：codex #41 + #44（异质裁决，非 PDF）。
3. level==1 实证：C3 脱 0，exists=13/breakout_ok=3，命中率 1.42%（#148 默认窗基线，已锁死门断言 `econ_positive.rs:4229-4235`）。
4. level>=2 实证：exists=6/breakout_ok=1，**命中率 0.46%（1/217，§4.2 实测）**——接近死门，若并入 C3 硬门吞吐从 217 塌缩到 1。
5. 现行 dx 探针已内置分级翻转的裁断逻辑（`econ_positive.rs:4106-4116`）：`n_l1_c3==0` ⟹ 全域退化处置成立；`>0` ⟹ 应翻转为分级。此逻辑「只报数，是否翻转由后续工位/编排者裁定」。

**本工位倾向（不裁断，供 codex 参考）**：实测数据支持**维持现状**（level==1 硬门 / level>=2 C2-only）——level>=2 的 C3 breakout 命中率 0.46% 接近死门，把它并入硬门等于用一个几乎全 false 的条件全拒 lvl>=2 XZD 吞吐（217→1），与 codex #41「level>=2 C2-only」的原意一致（只是当时归因为「判据死门」，现在归因为「命中率极低」）。但翻转与否仍需 codex 权衡「第43课『背驰后新中枢+反向突破』语义在高级别的适用性」（概念），这是价值判断，非逻辑必然。
...
3. **边界条件**：lvl>=2 C3 breakout 命中率实测 0.46%（1/217，§4.2）——接近死门，支持维持现行 level==1-only 硬门。若未来窗口/数据变化使 lvl>=2 命中率显著上升，则 codex 可重新裁定翻转为「全 level C3 硬门」，届时改 `gate_pass()` 的 `self.level != 1` 单一条件即可。
...
6. **影响声明**：本复审生产代码改动 0 行（gate_pass 逻辑无修改）；新增 lvl>=2 C3 命中探针（诊断报数，不参门，零行为影响）+ 复审报告 1 份；上浮 codex 裁定议题 1 项（XZD 全 level C3 硬门口径，本工位倾向维持现状）；全库 `cargo test --release --lib` 全绿（1478 passed）。

---

## 附：#166 阻塞已解除

初次复审时（2026-07-03）§4.2 探针与全库测试被并发工位 #166（A9-posnode）在飞半成品破坏全库编译（E0063）而阻塞。#166 完成后（commit 6af9b5fd1a，删除 interp.rs ActiveLeg 死字段，四元组正确落账本层）编译阻塞消除，本工位按计划三步完成：①全库 `cargo test --release --lib` 全绿（1478 passed, 0 failed）；②§4.2 探针实测（lvl>=2 命中率 0.46%）；③§五证据包 5 项全备，交 codex 裁定。lvl>=2 探针 hunk（诊断报数，与 #175 eta_bucket 分离后）单独入库。
