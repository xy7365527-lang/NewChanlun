...
- 遗留（均为登记态诚实缺口，非半成品）：①ηBucket——#158 专项终裁（6e2a8adbab，`a5-gamma4-confirm-20260703.md`）与本包核验互相独立佐证：γ_t 即 ηBucket 同一对象、**确认性缺口非存疑**（底层连续量 η_t/L^wc/Q/κ 已由 ledger 真承载且精确对应 PDF §10 公式；离散四态 Deficit/Zero/PositiveUnsafe/PositiveSafe 分类变量全仓库零承载）；实装枚举+路由后经 ZExt 通路接入（与 t_stage 同一装配点）；②SubMovePower 独立工位（前置/验收已冻结于裁定文件）；③`ForceStateA5→ForceState` 升名以 ② 完成为前提。
