...
- 遗留（均为登记态诚实缺口，非半成品）：①ηBucket——#158 专项终裁（6e2a8adbab，`a5-gamma4-confirm-20260703.md`）与本包核验互相独立佐证：γ_t 即 ηBucket 同一对象、**确认性缺口非存疑**（底层连续量 η_t/L^wc/Q/κ 已由 ledger 真承载且精确对应 PDF §10 公式；离散四态 Deficit/Zero/PositiveUnsafe/PositiveSafe 分类变量全仓库零承载）；实装枚举+路由后经 ZExt 通路接入（与 t_stage 同一装配点）
  **【①已关闭，#175】**：终裁 `a5-etabucket-stance-ruling-20260704.md`（立场B成立——本包 §1/§2「无生产者/不装」判定被推翻：η_t 生产者=`TwState::tw()` 即 `enter_ready` 判据左操作数，早已存在）后实装落地——`EtaBucket` 枚举+`RiskPolicy::eta_bucket` 分类函数（ledger.rs）+MuClass 第 15 维经 ZExt 同装配点接入，见 `etabucket-impl-20260704.md`；②SubMovePower 独立工位（前置/验收已冻结于裁定文件）；③`ForceStateA5→ForceState` 升名以 ② 完成为前提。
- 收口补丁（2026-07-03 收口 agent）：`mu_estimator.rs` 模块文档缺口清单同步——TStage 从「生产路径无数据源」清单移出（已第 14 维实装，留着=声明失真），ηBucket 条目改注 #158 终裁口径。全量测试于收口时点复跑确认 1466/0/110。
