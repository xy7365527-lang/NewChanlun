...
编排者澄清**概率不是缠论的一个"层"**：
- 走势理论本体（分型/笔/线段/中枢/走势类型/买卖点）= 分段函数 = **不需要概率**（第68课："这种预测，不需要任何概率化的无聊玩意"）
- 操作程序设计（选股/甄别）可以用概率乘法（第9课：30%×40%×30%=3.6%），但这是理论的**应用层工具**，不是理论的组成部分
- 缠师"不测而测，让市场自己去选择"（第68课原文）= 分段函数给出边界条件，市场当下选择落入哪个分段
- ~~"语法+概率分层"~~——此前 CC 错误表述，已作废。概率不构成独立层
