...
编排者确立**概率的三层架构**（第四次修正，终稿）：

缠师关于概率说了两件不同的事，在不同层次上，不矛盾：

| 层 | 角色 | 原文出处 | 概率态度 |
|----|------|---------|---------|
| **底层** | 概率原则 = 数学有效性保证 | 第19课："最基础的是三个独立系统所依据的概率原则所保证的数学上的系统有效性" | 正面：为什么系统管用 |
| **中间** | 语法 = 完全分类/分段函数（笔/线段/中枢/走势/买卖点） | 第68课 | 界面：概率从正翻负的分界线 |
| **顶层** | 操作 = 不测而测 | 第68课："不需要任何概率化的无聊玩意" | 负面：排除分类 → 概率乘法惩罚你 |

- 概率不在语法**之上**（CC 最初的错误），也不是**不存在**（CC 第三次的过度修正）
- 概率在语法**之下**——是语法有效性的数学地基
- 语法是概率从正翻负的分界线：语法之下概率为你工作，语法之上你试图用概率排除分类就被惩罚
- 第9课的 30%×40%×30% 是概率原则在操作程序设计中的具体应用（属于底层原则的体现）

CC 归因修正历史：
1. "概率为你服务" → 非缠师原话，已删
2. "不测而测"可能非原文 → 确认**是**原文（第68课，两次）
3. "概率不是一层" → 过度修正，概率**是**底层
4. **终稿**：概率是最底层（第19课），语法是中间层，操作层拒斥概率排除（第68课）
...
- **概率底层研究**: 概率是语法之下的地基（第19课），不是语法之上的评估层。可研究方向：为什么完全分类在数学上保证了正期望（定量分析）。操作层面不做概率排除（第68课）
