...
## 张力检查的范围与递归（019d + 020号谱系）

张力碰撞是核心发现来源（012），但发现驱力需要收缩来完成相位切换（020）。递归深度限制和范围策略是**维持构成性矛盾的运行机制**，不是对发现的限制。
...
|----|------|------|
| 0 | 本条谱系自身 | 写入020 |
| 1 | 020与邻接谱系碰撞产出新发现 | 020↔012产出新张力 |
| 2 | 第1层产出与其邻接再碰撞 | 极限，但010的发现就需要这一层 |

超过2层的张力：写入"深度张力待审"状态，不丢失不阻塞。
...
连续 **3个 ceremony 周期** 未被触碰的 pending 谱系 → 信号化提醒 Lead。这不是超时清理（005a禁止），是活性检查——提醒系统某处的脉动可能停滞了。
