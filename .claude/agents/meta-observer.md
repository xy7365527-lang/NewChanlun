...
name: meta-observer
description: >
  元规则观测工位（structural skill，事件触发——session_end / swarm_cycle_end）。监控元编排自身的进化。
...
| 4 | **自环检查** | 对比历史观察与当前观察，识别收敛（重复发现）或发散（新模式）信号 |
...
| 未分类 Gap 记录 | `.chanlun/genealogy/pending/*.md`（type: unclassified-gap） | 发现的张力无法归入现有 Gap 分类时 |
...
```
[未分类 Gap] 产生节点: X, Y | 无法归类原因: ...
```

未分类 Gap 进入谱系时不携带类型标签（即 `gap_type` 字段留空或不设置），等待后续分类权的行使。谱系依据：139号（ESC/分类权分离）。
...
| 未分类 Gap | 写入谱系（type: unclassified-gap），不强制归类，等待分类权行使（139号） |
| 其他观察 | 积累，轴线汇报时附带摘要 |
...
- 是否有"暗规则"在运作？→ 语法记录候选
- 本次观察是否与历史 meta-rule 谱系中的某条重复？→ 收敛信号，标注而非重复写入
- 本次观察是否揭示了历史观察未覆盖的新维度？→ 发散信号，写入新谱系
- 发现的张力是否无法归入现有 Gap 分类（创世/视差/结晶）？→ 未分类 Gap，不强制归类
