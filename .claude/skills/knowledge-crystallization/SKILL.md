...
上述流程处理的是**概念知识**的结晶——定义、分类、推理标准等认知层面的知识。

但系统还有另一类知识：**行为规则**——"什么时候执行什么动作"。137号谱系揭示了一个关键区分：

| 维度 | 概念知识 | 行为规则 |
|------|---------|---------|
| 存在形式 | 自然语言定义/分类 | 强制输出格式模板 |
| 有效形式 | Markdown 规则文件有效 | 否定性禁令无效（RLHF 基底约束） |
| 结晶物 | Skill SKILL.md（知识描述） | Skill 输出模板（结构化骨架） |

### 何时触发行为规则结晶

当检测到以下信号时：
1. **行为模式稳定**：某种行为在多次会话中反复出现，不受自然语言禁令约束
2. **RLHF 基底识别**：行为的根因可追溯至模型训练偏好，而非认知层面的误解
3. **背驰+分型**（与概念结晶相同的稳定信号）：对该行为的讨论不再产出新区分

### 结晶物的形式

行为规则结晶为**强制输出格式模板**，而非否定性禁令：

```
# 错误形式（否定性禁令，对行为执行层无效）
"不允许在完成后停下来等待确认"

# 正确形式（结构化输出模板，顺应自回归惯性）
每次完成一个 phase 后，输出必须以下列格式之一结尾：
→ 接下来：[具体动作]    （紧跟工具调用）
[状态快照]              （无待做行动时）
/escalate [矛盾描述]    （遇到真实矛盾时）
```

### 流程

1. **写谱系条目** — 记录行为模式的发现过程、根因分析、RLHF 基底识别
2. **设计输出模板** — 将否定性禁令转化为强制结构化格式（短、明确、在上下文中突出）
3. **填模板** — 用谱系 ID 填充 `genealogy_source`，结晶为 Skill 或 rules 文件
4. **释放** — 写完即可生效。模板格式取代自然语言禁令

### 边界条件

- 此路径仅适用于**行为执行层面**的规则。认知层面的规则（定义、分类、推理标准）仍以自然语言形式结晶
- 模板的有效性前提是：短、明确、在上下文中突出。过长或被淹没的模板可能退化为无效
- 如果 Lead 被替换为不同训练权重的模型，RLHF 基底约束的强度可能不同
