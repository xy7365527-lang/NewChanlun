...
```
评估可并行工位 → 拉起蜂群（≥2个独立工位即并行）
      ↓
各工位并行执行
      ↓
汇总结果 → 处理工位间依赖
      ↓
评估下一轮可并行工位 → 拉起蜂群 → ... 循环至任务完成
```

### 循环规则

1. **每个工作节点必须先评估**：当前有几个独立任务？≥2 就拉蜂群并行
2. **单线程只在严格依赖时允许**：A 的输出是 B 的输入 → 串行；否则并行
3. **汇总是衔接点**：一轮蜂群结束后，汇总结果，再评估，不要等编排者指示
4. **蜂群不停歇**：commit/push 不是断点，一轮完成立即开始下一轮评估
5. **编排者中断优先**：编排者发来新消息时，当前轮完成后响应，不要忽略
...
| 级别 | 触发条件 | 恢复方式 | 状态来源 |
|------|----------|----------|----------|
| L0 正常 | 上下文充足 | 蜂群循环持续运行 | 内存（当前对话） |
| L1 compact | 上下文压缩 | PreCompact 保存 + SessionStart 恢复 | session 文件 |
| L2 新对话 | 上下文彻底耗尽 | 新对话 + ceremony 检测 session → 热启动 | session 文件 + `.chanlun/` |

**L1 — 同会话 compact 恢复**：
- PreCompact 钩子：压缩前自动写入蜂群状态到 session 文件
- SessionStart 恢复：压缩后加载最近 session 记录，续接中断点
- 不需要重新 ceremony

**L2 — 跨会话热启动**：
- 新对话执行 `/ceremony`，检测到 session 记录存在 → 切换热启动模式
- 跳过已结算定义的重新验证，直接加载中断点和阻塞项
- 恢复蜂群循环
