...
- **异质否定**：通过 `/challenge` 调用外部模型（Gemini）质询关键产出。异质否定发现的是同质质询无法覆盖的系统性盲区。质询协议包含两个必选阶段：
  1. **广度轮**（多模型横向收敛）：识别否定方向集合
  2. **深度轮**（单模型 thinking 模式纵向精确化）：对每个否定方向构造精确反例和边界条件。必须使用 thinking 模式（258号结算：从"X 有问题"到"X 在 Y 条件下以 Z 方式失效"的信息增量是显著且稳定的）
...
2. 每累积 10 条新谱系，genealogist 触发形式类型审计：
   - `unclassified` 能否归类
   - 已归类是否需修正
3. 无法归入三种已知类型的否定标注 `unclassified`，上浮编排者——这可能是新的否定结构类型（新的不可消除拓扑修改，即139号重定义的"亏格"）的信号
