...
**蜂群的否定能力不限于同质 agent。不同推理模式的模型提供结构性不同的否定。**

### 为什么需要异质否定

Claude agent 之间的质询是同质的——相同的训练数据、相同的推理模式、相同的盲区。同质质询能发现逻辑错误和定义不一致，但无法发现**系统性盲区**——整个模型家族共享的认知偏差。

异质否定源（如 Gemini）通过不同的推理路径审视同一代码库，能发现 Claude 看不到的问题。031号谱系（Move 价格范围语义）就是实证：Gemini 通过 MCP/Serena 自主导航代码库，发现了 Level 1 和 Level 2+ 之间的自同构性破缺——这个问题在 Claude 的多轮质询中从未被发现。

### 异质否定的本体论位置

异质否定源不是"工具"也不是"成员"——它是**否定的另一种显现形式**。

| 维度 | 同质否定（Claude 内部质询） | 异质否定（外部模型质询） |
|------|---------------------------|------------------------|
| 否定来源 | 同一推理模式的不同展开 | 不同推理模式的碰撞 |
...
外部模型不能直接参与蜂群协议（SendMessage/TaskUpdate）。解决方案：**代理工位** — 一个 Claude agent 作为外部模型的代理，负责：

1. 构建上下文（读谱系 + 定义 + spec）
2. 调用外部模型（通过 CLI/API）
3. 提取完整推理链（不只是结论，包括中间推理和工具调用）
4. 以结果包格式向团队汇报
5. 判定否定是否成立 → 成立则写谱系，不成立则报告误判

代理工位是结构工位的一种——能力常驻，按需激活。

### 推理链透明性

异质否定的价值不只在结论，更在推理路径。代理工位必须提取并汇报外部模型的完整推理链：

```
[1] 🔍 tool_call: find_symbol(RecursiveOrchestrator)
[2] 💭 thought: "需要看 L* 裁决逻辑的实现..."
[3] 🔍 tool_call: read_file(recursive.py)
[4] 💭 thought: "发现 overlap() 在不同级别语义不同..."
[5] 📋 conclusion: "自同构性被打破"
```
...
[质量守卫] 检查结果包六要素
      ↓
[异质质询工位] 编排者/lead 指定时，调用外部模型质询关键产出
      ↓
汇总结果 → 处理工位间依赖
      ↓
...
| **异质质询工位** | 编排者/lead 指定质询目标时 | 代理调用外部模型（Gemini），提取推理链，产出异质否定 |
...
| 030a/031 | 异质否定源 + 代理工位模式 | 结构工位清单、蜂群循环、谱系模板 |
...
- **蜂群工位分两类：结构工位（常设）和任务工位（临时）。** 结构工位（谱系/元编排进化/Serena同步/质量守卫/异质质询）必须在蜂群实例化时先挂载，不随任务销毁。agent team不按流程走→先查结构工位是否遗漏。
- **异质否定源是结构性能力。** 同质质询发现逻辑错误，异质质询发现系统性盲区。代理工位模式让非 Claude 模型参与蜂群协作，推理链必须完全透明。
