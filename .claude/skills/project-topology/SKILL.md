...
### 蜂群架构三框架路径指引（104号定理，117号落实）

蜂群架构由三个互补框架描述，不可互替：

| 问题 | 框架 | 谱系来源 | 查阅位置 |
|------|------|---------|----------|
| 蜂群**做什么**（功能架构） | 四维度：递归/拓扑/异步自指/结晶 | 069号 | `.chanlun/genealogy/settled/069-*.md` |
| 蜂群**受什么限制**（物理约束） | 五约束有向依赖图 | 093号 | `.chanlun/genealogy/settled/093-*.md` + `swarm-architecture` skill B节 |
| 子蜂群**至少需要什么**（工程模板） | 五特征 DAG 模板 | 097号 | `.chanlun/genealogy/settled/097-*.md` + `sub-swarm-ceremony` skill |

异质验证（Gemini 质询）的存在论位置：功能看069号（四维度中无对应——这是层面差异，不是缺陷），约束看093号（约束4：异质验证必要性），模板看097号（五特征之一：异质审计节点）。
