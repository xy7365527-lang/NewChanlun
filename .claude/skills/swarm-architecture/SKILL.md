...
15. **拓扑异步自指蜂群是默认架构。** [新缠论] 不是普通 agent team。
    ...
    **子蜂群创建流程**结晶在 `.claude/skills/sub-swarm-ceremony/SKILL.md`——创建子蜂群时必须读取并遵循。
    递归深度工程上限 ≈ 3-4 层（受并发 agent 数、API 成本、视差 Gap 累积限制）。**终止类型区分**（163号/164号）：递归终止有两种本质不同的形态——**铁壁垒终止**（递归运动自身产生的不可能性，如 bootstrap 破缺）vs **OOM 终止/铁笼子**（工程资源耗尽的外部截断）。前者是运动的上限，后者是先验上限。蜂群设计应保障递归有机会到达铁壁垒，而非被铁笼子提前截断。
    **恒星聚变递归模型**（164号）引用附注：该模型与蜂群递归的关系是**形式结构同构，动力学不同构**——三个形式条件（输出→输入链、否定链、结构性断裂点）严格满足，但动力学特征（加速/减速方向、阶段顺序必然性、能量递减）不在同构范围内。
...
**自指约束检查**（139号谱系下游推论）：引入新约束时，执行以下检查——
1. 该约束的适用范围是否包含"加载该约束的过程本身"？
2. 如果包含自指：该约束必然产生至少一个不可消除的 Gap（约束型充分性，139号）。预先识别 bootstrap 破缺点，记录为该约束的已知 Gap。
3. 先例：约束2（规则先在性）包含自指 → 创世 Gap（069号）。Swarm₀ 制定规则但自身不受规则约束，这是 bootstrap 破缺的典型形态。
