...
## 卡点即报（2026-08-18 #1066 增补）

外部依赖缺失（无 key / 无数据 / 网络不可达）或工作无法推进时：**不空转、不硬凑**——
在票面留卡点报告（复现证据 + 解除条件 + 续跑命令），然后发 `<promise>COMPLETE</promise>` 收工。
无进展空转到预算耗尽是最贵的失败形态（#1044 第一次空转 24 分钟实录）。
