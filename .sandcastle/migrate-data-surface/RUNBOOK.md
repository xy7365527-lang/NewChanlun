...
`mv`（同目录 rename）与 `ln -s` 各自都是原子元数据操作，但两者之间有一个极短窗口：旧路径暂不存在
（既非目录也非 symlink）。该窗口内任何写者/读者都会得到「不存在」而失败、不会写错位置，故不产生
双写或数据破坏。切换后旧数据完整留在 `<旧>.migrate-backup.<ts>`，只读保留到 #1237 退役步骤批准。
