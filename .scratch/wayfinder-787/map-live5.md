...
## Decisions so far
- [task 普查：总缝规则过筛——40 套口径四分判定](https://github.com/xy7365527-lang/NewChanlun/issues/809) — **三个数：违规 6 / 例外候选 1（上限 3，未触顶，余量 2）/ 真待裁 34 组（7 概念）/ 不适用 19。****★ 6 条违规里 4 条落在「力度」上**（V1 `div_cand`↔`sublevel_diverges`+C1 降级、V3 `recursive_t/divergence.rs:72` `nest>0?嵌套深度:几何振幅`、V4 `rust/src/divergence.rs:188` 无 MACD 时 fallback 振幅×时长、V6 `a_nested_divergence.py:118` level1 走 MACD/level≥2 走振幅×组件数）——**同一个病：正经判据取不到数据就换个便宜的顶上、照常出信号**；另 V2 `Cand^δ` per-rung 恒真档、V5 `a_trendtype_v0.py:143` 源码自陈「§8.2 弱化版本」兜底。**订正 [#789](https://github.com/xy7365527-lang/NewChanlun/issues/789) 两条**：`classifier/mod.rs:547` 的一/三类 `is_l0` 分派**不是分歧**（两侧都落到 `signal.rs:1526` 同一函数，总缝规则通过）；`segments_diverge_or` 是**生产件**非只在编译树。**★ 编排者当场裁定：Lean 分支算「真待裁」⟹ 待裁面 34 组**（理由：`AGENTS.md` 权威分层里 Lean 管边界、是正本一部分，「不适用」那格是给对照臂留的；实证＝[#811](https://github.com/xy7365527-lang/NewChanlun/issues/811) 两条 Lean↔Rust 一类买点对拍在主线上**为红**，分歧真会打架）。**090 照实：全部总缝判定是源码语义推断非对拍实测**（当时 CI 不跑 `cargo test`，且后查出 CI 分支落后主线 545 提交，即便跑也在旧树上）。**毕业**：第二批 6 张概念票已开出并接边——[中枢 #812](https://github.com/xy7365527-lang/NewChanlun/issues/812)（7 组+E1 退场条件）/ [笔与线段 #813](https://github.com/xy7365527-lang/NewChanlun/issues/813)（9 组，合并不拆）/ [背驰 #814](https://github.com/xy7365527-lang/NewChanlun/issues/814)（5 组）/ [走势类型与级别 #815](https://github.com/xy7365527-lang/NewChanlun/issues/815)（4 组，← #812）/ [买卖点 #816](https://github.com/xy7365527-lang/NewChanlun/issues/816)（3 组+#811 必答，← #814）/ [区间套 #817](https://github.com/xy7365527-lang/NewChanlun/issues/817)（6 组，← #812 #814 #816）。报告 `cc1ddd7f4f`
...
<!-- 「第二批概念票的范围与次序」已于 2026-07-30 完全毕业：清点成 [#809](https://github.com/xy7365527-lang/NewChanlun/issues/809)，其结果切出 6 张概念票 [#812](https://github.com/xy7365527-lang/NewChanlun/issues/812)–[#817](https://github.com/xy7365527-lang/NewChanlun/issues/817) 并接完 blocking 边，故此处清除。 -->
