...
6. **推论自动结算（四分法）。** 系统产出分为四类，处理方式不同：
   ...
   - **选择**：多种合理方案，需价值判断 → 通过 `/escalate` 上浮（面对开放的未来，需要人类参与者决断）。
   - **语法记录**：已在实践中运作但尚未显式化的规则 → 通过 `/escalate` 上浮（面对已在运作的现在，需要人类参与者辨认）。辨认不是在多条路之间选，而是把暗处运作的规则写进语法书。语法记录不创造规则，语法记录让规则从隐性变为可质询。
