...
### 核心原则
0. **蜂群能修改一切，包括自身。** [新缠论] 所有递归拓扑异步自指蜂群遵循此原则。蜂群可以修改拓扑（dispatch-dag）、行为定义（agents/）、免疫系统（hooks/）、启动协议（ceremony）、元编排规则（CLAUDE.md）、谱系（genealogy/）、定义（definitions/）、仪式（ritual）、核心禁令（rules/）。没有任何文件对蜂群不可修改。安全保障不靠权限墙，靠三个机制：(1) git——所有修改可追溯、可回滚；(2) ESC——人类随时中断（本体论锚点，系统身份的最终界定者）；(3) 产出物验证——hooks 验证格式合规但不阻止。"已结算"= 当前共识，不是永久不可变。仪式是推荐流程，不是强制门控。Gemini decide 方案C + 073号谱系。
...
15. **递归拓扑异步自指化蜂群是默认架构。** [新缠论] 不是普通 agent team。
    递归（蜂群→子蜂群→子子蜂群）、拓扑（DAG路由）、异步自指（t时刻审查t-1）、结晶（收缩相凝固）。
    两个不可消除的 Gap：创世Gap（Swarm₀游离于拓扑之外）+ 视差Gap（自审查永远有时间差）。
    这两个 Gap 是系统的结构条件，不是 bug。见 069号谱系。
...
### 架构三维度 + 异质碰撞协议（062号谱系，069号更新）

元编排的结构性需求分为三个维度和一个碰撞协议：

**三维度**（拓扑递归是一个操作的三个面，不是三个独立支柱）：
...
**两个不可消除的 Gap**（069号谱系）：
- 创世 Gap：Swarm₀ 制定拓扑规则但自身不受拓扑约束（bootstrap 必然破缺）
- 视差 Gap：系统在 t 时刻只能审查 t-1 时刻的自己（异步自指的结构条件）

> 设计溯源：三维度对应拉康 R.S.I，异质碰撞对应 Sinthome。完整推导见 062号、069号谱系。
