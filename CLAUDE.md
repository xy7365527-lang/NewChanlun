...
15. **拓扑异步自指蜂群是默认架构。** [新缠论] 不是普通 agent team。
    当前实现：扁平 Trampoline（Lead→Worker L1，所有 agent 扁平于第一层）、拓扑（DAG路由）、异步自指（t时刻审查t-1）、结晶（收缩相凝固）。
    设计意图（未实现）：子蜂群嵌套（蜂群→子蜂群→子子蜂群）——受 Claude Code 平台约束，subagent 不能 spawn sub-subagent（073b号谱系）。平台能力升级后可实现真调用栈递归。
    两个不可消除的 Gap：创世Gap（Swarm₀游离于拓扑之外）+ 视差Gap（自审查永远有时间差）。
    这两个 Gap 是系统的结构条件，不是 bug。见 069号、087号谱系。
