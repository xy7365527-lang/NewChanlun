...
| **拓扑** | DAG 路由，定义连接模式 | dispatch-dag.yaml（9 节点偏序集） |
...
三个 dominator 节点（genealogist + quality-guard + meta-observer）是蜂群存在的拓扑前提。Stop hook 从 dispatch-dag 动态读取 mandatory 节点并强制——缺少任何一个，蜂群无法停止。
...
| dag-validation-guard | — | DAG 不变量验证 |
| lead-permissions | — | Lead 权限管理 |
...
| 指标 | 数值 |
|------|------|
| 源文件 | 54 个 Python 模块 + 5 个 Gemini 模块 |
| 测试文件 | 98 个 |
...
│   ├── settled/           #     82 条已结算
│   ├── pending/           #     生成态矛盾
│   └── dag.yaml           #     谱系 DAG（82 节点 / 358 边）
├── dispatch-dag.yaml      #   蜂群拓扑（9 节点 + fractal_template）
