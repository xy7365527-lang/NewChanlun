...
| **Trampoline**（退化特例） | 任务裂变 → Lead 重新 spawn → 同一层弹跳 | 仅当任务不需要独立治理时 |

这与 056号"递归是存在方式，线性是退化特例"完全同构——Trampoline 需要理由（必须证明子任务不需要独立治理），真递归不需要理由。递归深度上限 ≈ 3-4 层，受并发 agent 数、API 成本、视差 Gap 累积约束。
...
| 095 | Agent Team 真递归蜂群（Trampoline 退化为特例） |
