...
蜂群的定义基底一致性不是本体论给定的，是运行时生产的。同一模型的不同实例也会偏差（已有实证：六次违反 no-unnecessary-escalation）。一致性必须被检查，不能被假设。

### 校验时机

- **任务启动前**：每个 agent 对本次任务涉及的关键定义做复述，Lead 对照 `.chanlun/definitions/` 检查。偏差则纠正后再开工。
...
| agent 类型 | 频率 | 理由 |
|-----------|------|------|
| Claude teammates | 抽检（每 3 次质询 1 次） | 同模型偏差概率低 |
| Gemini 工位 | 每次校验 | 异质模型，不共享语法解析 |
| 未来新模型 | 根据历史偏差率调整 | 谱系可学到实际偏差率 |
