"""38课循环 voice 买回判据消融 — RevCycleClose 四臂 × OKLO+BRN。

═══════════════════════════════════════════════════════════════════════
任务（2026-06-11 编排者任务：38课循环买回判据消融）
═══════════════════════════════════════════════════════════════════════
背景：38课循环 voice（V2oa25C38）实装成功但回测否证（OKLO −472.8pp /
BRN −70.4pp），死因定位在买回端——"次级别任意买点"买回太快（胜率 33-36%）。

编排者原则：**区间套无论正反都存在。**
  卖出端：大级别定方向（宿主趋势）+ 本级别卖点定时机 + 次级别确认精度；
  买回端：本级别买点定时机（同锚 Buy1 / ZD 触线）+ 次级别确认精度。
C38base 的买回用"次级别任意买点"跳过了本级别判据——违反区间套。

矩阵：OKLO + BRN × floor=2 ×
  { V2oa25        = 单次 REV 基线（在册默认门）
    V2oa25C38     = C38base：次级别任意买点买回（在册，已否证对照臂）
    V2oa25C38buy1 = C38buy1：同锚 confirmed Buy1 买回
    V2oa25C38zd   = C38zd：ZD 触线买回
    V2oa25C38pair = C38pair：V2oa25 完整配对闭腿集（循环版 V2oa25）}

判决预注册：
  C38pair > V2oa25 ⇒ 循环结构贡献正 alpha（反复操作优于单次操作）；
  C38pair ≤ V2oa25 ⇒ 循环结构本身无增量价值（38课循环假设在事件流
                      近似下整体关闭，位置约束分支除外）。

伴随差异声明（090号）：Buy1/Zd/Paired 三臂的闭腿判据需要锚中枢（同锚
比较基准 / ZD 线），开腿端因此多一个锚捕获准入条件（无存活中枢 ⇒ 保守
拒绝，n_c38_nocenter_rejects 可观测）——这是闭腿判据的数据依赖，不是
独立消融轴。SubAny 开腿路径逐位不变（在册零接触）。

守卫：
  1. 零接触守卫：同一磁带含/不含 trend_flips 行，O0 与 V2oa25 trades
     逐位相同（cargo 114 项测试 + 本守卫合取）。
  2. 磁带指纹守卫（tape_fp，含 trend 翻转数）：禁止跨引擎语义混表。
  3. O0≡P5 守卫：organic_fugue_rust_check.py（独立执行）。

认识论等级：守卫 L1（管线等价）；OKLO/BRN 单 floor 回测 L2→两标的 L3。

输出：analysis/data_cache/cycle38_close_ablation.json（增量续跑）
用法：PYTHONPATH=src .venv/bin/python analysis/cycle38_close_ablation.py
...
OUT_JSON = DATA_DIR / "cycle38_close_ablation.json"
...
def run_cell(rtape, variant: str, years: float, bh: float) -> dict:
    ...
    c = res["counters"]
    # 循环/REV 腿逐对 payoff 分布（diag rev 腿 trace：profit 列）
    rev_profits = []
...
def process_symbol(symbol: str, prev: dict | None = None) -> dict:
    print(f"\n{'=' * 64}\n  {symbol} — 38课循环买回判据消融（floor={FLOOR}）"
          f"\n{'=' * 64}", flush=True)
    ...
    for variant in VARIANTS:
        ...
        m = pack["metrics"]
        c = pack["counters"]
        print(f"  [{variant:13s}] 复利={m['total_compound']:+10.2f}%"
              f" Δ(BH)={pack['delta_vs_bh']:+8.1f}pp"
              f" Δ(base)={pack['delta_vs_v2oa25']:+7.1f}pp 笔={m['n']:4d}"
              f" | open/close={c['n_c38_open']}/{c['n_c38_close']}"
              f" forced={c['n_c38_forced_close']}"
              f" 闭因 t7/t6/b1/zd={c['n_c38_close_t7']}/{c['n_c38_close_t6']}"
              f"/{c['n_c38_close_buy1']}/{c['n_c38_close_zd']}"
              f" 无锚拒={c['n_c38_nocenter_rejects']}"
