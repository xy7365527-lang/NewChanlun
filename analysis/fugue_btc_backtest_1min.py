"""BTC 1min 赋格回测（做多+做空）— Binance 743K bars。

数据源：Binance BTCUSDT 1m archive（17个月，2025-01 → 2026-05）
回测：复用 fugue_with_short_backtest_1min.py 的完整 38 课 FSM
方向：both（向上段做多 + 向下段做空）

认识论等级：L2（真实数据，单标的 1min；可产生否定性结果）。
"""
...
OUTPUT_MD = ROOT / "analysis" / "fugue_btc_backtest_1min.md"
BTC_FILE = DATA_DIR / "btc_1m_full.json"
...
def write_report(
    ...
    if event_records:
        ...
        L.append("|-----|------|------|------|")
        for r in event_records[:200]:
            L.append(
                f"| {r.bar_idx:,} | {r.price:,.2f}"
                f" | {r.state} | {r.event} |"
            )
        if len(event_records) > 200:
            L.append(
                f"| ... | ... | ..."
                f" | （共{len(event_records)}条，显示前200） |"
            )
        L.append("</details>\n")

    L.append("## 结果包六要素\n")
    L.append(
        f"**结论**：BTC 1min 赋格回测（做多+做空），17个月真实数据。"
    ...
    L.append(f"- 测试期 {dates[0][:10]} → {dates[-1][:10]}（{n_bars/1440:.0f}天）\n")
    L.append("**下游推论**：")
    L.append("- BTC 波动率远高于股票，对 FSM 参数敏感度不同")
    L.append("- 24/7 市场使 bar 数远多于股票（同时段）")
    L.append("- 做空结果直接反映 crypto 下跌段的可操作性\n")
    L.append("**谱系引用**：无直接相关谱系（BTC 首次长历史回测）。\n")
    L.append(
        "**影响声明**：重写 BTC 回测脚本（数据源从 yfinance 30天"
        " 升级为 Binance archive 17个月），不修改引擎代码。\n"
    )
    L.append(
        "**认识论等级**：L2（真实数据，单标的 1min 17个月；"
        "可产生否定性结果）。"
    )
