"""完整版缠论+PH多重赋格量化交易回测系统 — 阶段一（纯做多+降成本）。

基于 docs/architecture/complete_fugue_design.md 设计文档实现。
核心改进（相比 longonly 版）：

1. 完整 38 课操作程式 FSM（6 状态）：
   WAIT_ENTRY → ENTRY → HOLDING → EVAL → EXIT/WAIT_DIP → OBSERVE
   替代旧版简单的 SCANNING/POSITION_OPEN/COST_REDUCING/PRINCIPAL_WITHDRAWN

2. 走势段比较（38 课 S7-S9）：
   - 不创新高判断（U3.high < U1.high → 最弱，必须退出）
   - 盘整背驰判断（U3 对 U1 的力度衰竭 → 退出）
   - 替代旧版依赖 L2 rank-1 flip 退出

3. 区间套定位（37 课 + 29 课）：
   - 仅在操作级别出现背驰信号时启动递归
   - L0 PH settle 精确定位入场/出场价位

4. 降成本子状态机：
   FULL_POS ↔ REDUCED → PRINCIPAL_RECOVERED
   持有期间用短差级别 PH 做循环减仓/回补
...
OUTPUT_MD = ROOT / "analysis" / "fugue_complete_backtest_1min.md"
PENDING_EXPIRY = 390
                    ...
                    for settled_mv in new_settled_up_moves:
                        ...
                        if not up_seg_records:
                            ...
                            if _check_no_new_high(
                                ...
                                _close_trade(i, c, "consolidation_divergence")
                                break
...
def write_report(all_results: list[dict]) -> None:
    L: list[str] = []
    L.append("# 完整版缠论+PH多重赋格回测 — 阶段一（纯做多+降成本）\n")
    L.append("## 架构\n")
    L.append("基于 docs/architecture/complete_fugue_design.md 实现。\n")
    L.append("核心改进（相比 longonly 版）：\n")
    L.append("1. **完整38课操作程式FSM**（6状态）：")
    L.append("   WAIT_ENTRY→ENTRY→HOLDING→EVAL→EXIT/WAIT_DIP→OBSERVE")
    L.append("2. **走势段比较**（S7-S9）：不创新高/盘整背驰判断")
    ...
    complete_row = "| 6. **完整版（本版）** |"
    for res in all_results:
        m = res["metrics"]
        complete_row += f" **{m['total_compound']:+.1f}%** |"
    ...
    L.append("## 结果包六要素\n")
    L.append(
        "**结论**：完整版38课操作程式FSM在1分钟级别3标的上的表现。"
        "阶段一实现（纯做多+降成本+走势段比较）。\n"
    )
    L.append("**定义依据**：")
    L.append("- 38课第36行：同级别分解操作程式逐句形式化")
    L.append("- S7: 不创新高判断 + 盘整背驰力度比较")
    L.append("- S5: 回调段不跌破前低/跌破+盘整背驰 判断")
    L.append("- 37课第22行 + 29课第52行：区间套精确定位")
    L.append("- 38课第30行：分区卷钱降成本\n")
    L.append("**边界条件**：")
    L.append(f"- PENDING_EXPIRY = {PENDING_EXPIRY} bars")
    L.append("- 力度比较使用价格振幅（无MACD），可能低估背驰")
    L.append("- 阶段一：纯做多，向下段空仓等待")
    L.append("- 无滑点/手续费建模\n")
    L.append("**下游推论**：")
    L.append("- 走势段比较 vs PH flip 退出的效果对比")
    L.append("- 若完整版 > 纯做多 → 段间比较改善了退出时机")
    L.append("- 若完整版 < 纯做多 → 段间比较引入了额外摩擦\n")
    L.append("**谱系引用**：")
    L.append("- 525号：组件局部完成性")
    L.append("- 526号：a0递归存在论区分")
    L.append("- 521号：PH纯拓扑无动量\n")
    L.append(
        "**影响声明**：新建独立回测脚本和strategy模块，"
        "不修改引擎代码或旧版回测。\n"
    )
    L.append(
        "**认识论等级**：L2"
...
def main() -> None:
    ...
    for symbol in symbols:
        print(f"\n{'=' * 60}")
        print(f"  {symbol} — 完整版赋格回测（阶段一：纯做多+降成本）")
        print(f"{'=' * 60}")
