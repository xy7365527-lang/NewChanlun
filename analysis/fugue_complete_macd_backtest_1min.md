...
基于 persistence 过滤版，盘整背驰从价格振幅升级为 MACD 三维度：

1. **persistence过滤**：走势 persistence >= alive median 才触发段间比较
2. **MACD三维度背驰**（T5/T6/T7 OR逻辑）：
   - T5: MACD hist面积 C < A
   - T6: DIF峰值 C < A
   - T7: hist柱子峰值 C < A
3. 增量 OnlineMacdState（O(1)/bar）
4. 完整38课FSM + 降成本子状态机
...
| 14 | 439.04@236273 | 439.10@243197 | 6,924 | +1.18 | 1 | 4 | 2 | macd_consolidation_divergence | ENTRY→EVAL→HOLDING→WAIT_DIP |
...
| 93,829 | 54.11 | WAIT_DIP | →EVAL(move_settle) high=59.11 force=21.28 p=21.28/med=4.49 → WAIT_DIP(创新高无MACD背驰 noDiv(A=34.9578,C=97.1345)) |
...
| # | 入场 | 出场 | 持仓bars | PnL% | 加仓 | 短差 | 向上段数 | 退出原因 | 状态路径 |
|---|------|------|---------|------|------|------|---------|---------|---------|
| 1 | 50.50@289429 | 46.20@324400 | 34,971 | -1.07 | 0 | 1 | 2 | macd_consolidation_divergence | ENTRY→EVAL→HOLDING→WAIT_DIP |
...
| 状态 | Bars | 占比 |
|------|------|------|
| WAIT_ENTRY | 1,134,961 | 80.6% |
| ENTRY | 15 | 0.0% |
| HOLDING | 253,531 | 18.0% |
| EVAL | 0 | 0.0% |
| WAIT_DIP | 20,375 | 1.4% |
| OBSERVE | 0 | 0.0% |
| EXIT | 0 | 0.0% |

<details><summary>事件流（260条）</summary>
...
| 324,400 | 46.20 | WAIT_ENTRY | →EVAL(move_settle) high=52.70 force=8.60 p=8.60/med=7.10 → EXIT(MACD背驰 T6(0.264034<0.265800)) |
...
| 375,182 | 80.70 | WAIT_DIP | →EVAL(move_settle) high=83.90 force=19.20 p=19.20/med=10.30 → WAIT_DIP(创新高无MACD背驰 noDiv(A=104.4476,C=120.0063)) |
...
| 614,542 | 192.30 | WAIT_DIP | →EVAL(move_settle) high=203.50 force=13.20 p=13.20/med=13.10 → WAIT_DIP(U1完成 high=203.50) |
| 619,998 | 176.05 | HOLDING | DIP_REBUY_DIVERGE@176.05 跌破+盘整背驰 |
| 620,092 | 175.95 | HOLDING | TRIM@175.95 shares=570.3 |
| 620,707 | 175.95 | HOLDING | MOVE_SETTLE_SKIP(p=16.05<med=22.75) |
| 621,249 | 176.80 | HOLDING | CLOSE_DIFF@176.80 profit=-484.73 |
| 621,672 | 173.00 | HOLDING | TRIM@173.00 shares=546.9 |
| 623,255 | 169.35 | HOLDING | CLOSE_DIFF@169.35 profit=1996.28 |
| ... | ... | ... | （共260条，显示前150） |
</details>
...
| 8. **persistence+MACD版（本版）** | **+58.7%** | **+194.4%** | **+191.6%** |

## 汇总

| 标的 | Bars | BH% | 复利% | 胜率 | 交易数 | L2翻转 | 加仓 | 降成本 | PW | 段比较 | 耗时 |
|------|------|-----|------|------|--------|--------|------|--------|-----|--------|------|
| QQQ | 728,030 | +174.6 | +58.74 | 70% | 30 | 14 | 20/30 | 24/30 | 0/30 | 5/30 | 405s |
| OKLO | 333,613 | +251.3 | +194.37 | 56% | 9 | 7 | 7/9 | 7/9 | 0/9 | 1/9 | 74s |
| HK700 | 1,408,882 | +3933.5 | +191.61 | 67% | 15 | 5 | 14/15 | 15/15 | 0/15 | 2/15 | 564s |

## 结果包六要素

**结论**：persistence过滤+MACD三维度背驰版，在正确粒度的走势段比较上使用真实MACD背驰判断。

**定义依据**：
- 38课段间比较适用于操作级别走势类型
- persistence门槛 = PH alive median（从数据涌现）
- 盘整背驰 = MACD三维度（T5面积/T6 DIF峰值/T7 hist峰值）OR逻辑
- 第24/25课：MACD面积+黄白线+柱子三维度缺一不可

**边界条件**：
- PENDING_EXPIRY = 390 bars
- MACD参数：(12,26,9)标准参数
- T5/T6/T7任一成立=背驰（OR逻辑，可能过于宽松）
- 阶段一：纯做多，向下段空仓等待
- 无滑点/手续费建模

**下游推论**：
- 若MACD版 > persistence过滤版 → MACD背驰比价格振幅更精确
- 若MACD版 < persistence过滤版 → MACD 3D OR逻辑过于宽松
- 若MACD版 > 纯做多 → 完整38课程式+MACD在正确粒度上超越简单PH flip退出
