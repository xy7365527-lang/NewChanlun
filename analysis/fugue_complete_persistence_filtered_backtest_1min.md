# 完整版缠论+PH赋格回测 — persistence过滤版

## 架构

基于完整版阶段一，增加 persistence 门槛过滤：

- 走势 settle 的 persistence >= 操作级别 PH alive median → 触发38课段间比较
- 走势 settle 的 persistence < median → 只触发降成本，不触发退出判断
- persistence 阈值从数据涌现（median of alive），不是人工参数

其余架构与完整版阶段一相同：

1. 完整38课操作程式FSM（6状态）
2. 走势段比较（S7-S9，仅高persistence走势）
...
| 14 | 439.04@236273 | 439.10@243197 | 6,924 | +1.18 | 1 | 4 | 2 | consolidation_divergence | ENTRY→EVAL→HOLDING→WAIT_DIP |
...
| 21 | 544.24@520848 | 558.20@532487 | 11,639 | +3.99 | 0 | 12 | 2 | consolidation_divergence | ENTRY→EVAL→HOLDING→WAIT_DIP |
...
| 28 | 650.00@699682 | 703.02@717431 | 17,749 | +10.44 | 0 | 19 | 3 | consolidation_divergence | ENTRY→EVAL→HOLDING→WAIT_DIP |
...
| 状态 | Bars | 占比 |
|------|------|------|
| WAIT_ENTRY | 527,113 | 72.4% |
| ENTRY | 30 | 0.0% |
| HOLDING | 194,700 | 26.7% |
| EVAL | 0 | 0.0% |
| WAIT_DIP | 6,187 | 0.8% |
| OBSERVE | 0 | 0.0% |
| EXIT | 0 | 0.0% |

<details><summary>事件流（568条）</summary>
...
| 2 | 87.50@378892 | 90.50@409439 | 30,547 | +17.70 | 2 | 10 | 0 | bsp_invalidate | ENTRY→HOLDING |
| 3 | 119.10@428315 | 111.25@438789 | 10,474 | -6.27 | 1 | 2 | 1 | bsp_invalidate | ENTRY→EVAL→HOLDING→WAIT_DIP |
| 4 | 109.60@441287 | 105.25@443385 | 2,098 | -0.18 | 1 | 1 | 0 | bsp_invalidate | ENTRY→HOLDING |
| 5 | 119.55@453755 | 109.95@457850 | 4,095 | -2.50 | 1 | 1 | 0 | bsp_invalidate | ENTRY→HOLDING |
| 6 | 124.65@468634 | 140.55@502832 | 34,198 | +13.88 | 6 | 15 | 1 | bsp_invalidate | ENTRY→EVAL→HOLDING→WAIT_DIP |
| 7 | 158.80@576774 | 167.75@624082 | 47,308 | +8.08 | 1 | 13 | 1 | bsp_invalidate | ENTRY→EVAL→HOLDING→WAIT_DIP |
| 8 | 222.70@652086 | 263.40@670541 | 18,455 | +25.41 | 1 | 8 | 0 | bsp_invalidate | ENTRY→HOLDING |
| 9 | 276.90@672228 | 299.90@682404 | 10,176 | +7.68 | 1 | 3 | 1 | bsp_invalidate | ENTRY→EVAL→HOLDING→WAIT_DIP |
| 10 | 312.90@685236 | 351.40@704079 | 18,843 | +11.17 | 1 | 6 | 1 | bsp_invalidate | ENTRY→EVAL→HOLDING→WAIT_DIP |
| 11 | 403.40@710944 | 384.60@719629 | 8,685 | +7.93 | 1 | 5 | 0 | bsp_invalidate | ENTRY→HOLDING |
| 12 | 487.60@917775 | 490.00@922761 | 4,986 | +0.29 | 1 | 5 | 0 | bsp_invalidate | ENTRY→HOLDING |
| 13 | 552.80@944408 | 540.80@954533 | 10,125 | -1.58 | 1 | 2 | 1 | bsp_invalidate | ENTRY→EVAL→HOLDING→WAIT_DIP |
| 14 | 583.20@962508 | 641.60@972802 | 10,294 | +10.55 | 2 | 6 | 0 | bsp_invalidate | ENTRY→HOLDING |

### FSM 状态分布（按 bar 数）

| 状态 | Bars | 占比 |
|------|------|------|
| WAIT_ENTRY | 1,112,831 | 79.0% |
| ENTRY | 14 | 0.0% |
| HOLDING | 274,095 | 19.5% |
| EVAL | 0 | 0.0% |
| WAIT_DIP | 21,942 | 1.6% |
| OBSERVE | 0 | 0.0% |
| EXIT | 0 | 0.0% |

<details><summary>事件流（267条）</summary>
...
| 375,182 | 80.70 | WAIT_ENTRY | →EVAL(move_settle) high=83.90 force=19.20 p=19.20/med=10.30 → EXIT(盘整背驰 force 19.20<20.40) |
...
| 606,575 | 200.00 | HOLDING | CLOSE_DIFF@200.00 profit=-125.69 |
| 614,145 | 185.65 | HOLDING | TRIM@185.65 shares=570.7 |
| ... | ... | ... | （共267条，显示前150） |
</details>
...
| 7. **persistence过滤版（本版）** | **+58.7%** | **+194.4%** | **+313.0%** |

## 汇总

| 标的 | Bars | BH% | 复利% | 胜率 | 交易数 | L2翻转 | 加仓 | 降成本 | PW | 段比较 | 耗时 |
|------|------|-----|------|------|--------|--------|------|--------|-----|--------|------|
| QQQ | 728,030 | +174.6 | +58.74 | 70% | 30 | 14 | 20/30 | 24/30 | 0/30 | 5/30 | 368s |
| OKLO | 333,613 | +251.3 | +194.37 | 56% | 9 | 7 | 7/9 | 7/9 | 0/9 | 1/9 | 73s |
| HK700 | 1,408,882 | +3933.5 | +313.04 | 71% | 14 | 5 | 14/14 | 14/14 | 0/14 | 1/14 | 525s |

## 结果包六要素

**结论**：persistence过滤版在完整版基础上，通过PH alive median门槛过滤低persistence走势settle，减少虚假退出。

**定义依据**：
- 38课段间比较适用于操作级别走势类型
- 低persistence的走势 = 结构性影响小的小走势
- persistence门槛 = PH alive组件的median（从数据涌现）
- 高persistence settle → 段间比较（S7退出判断）
- 低persistence settle → 只降成本（不退出）

**边界条件**：
- PENDING_EXPIRY = 390 bars
- median threshold可能过高或过低，取决于数据特征
- 阶段一：纯做多，向下段空仓等待
- 无滑点/手续费建模

**下游推论**：
- 若persistence过滤版 > 完整版 → 低persistence settle是噪音源
- 若persistence过滤版 ≈ 纯做多 → 过滤后的段间比较接近PH flip退出效果
- 若persistence过滤版 > 纯做多 → 段间比较在正确粒度上改善了退出
