...
基于 fugue_complete_backtest_1min.py（完整版阶段一），增加 persistence 门槛过滤：

核心改动（仅一处）：
- 走势 settle 触发 EVAL 前，检查该走势的 persistence 是否 >= 操作级别 PH 树
  alive 组件的 median persistence
- 高 persistence settle → 触发38课段间比较（S7不创新高/盘整背驰）
- 低 persistence settle → 只触发降成本（减仓/回补），不触发退出判断

设计原理：
38课的段间比较是操作级别走势类型之间的比较。低 persistence 的走势是
"小走势"（结构性影响小），不应该触发"不创新高"的退出判断。只有高
persistence 的走势（对价格结构有持久影响）才是38课意义上的"向上段走势"。
...
OUTPUT_MD = ROOT / "analysis" / "fugue_complete_persistence_filtered_backtest_1min.md"
PENDING_EXPIRY = 390
...
def write_report(all_results: list[dict]) -> None:
    L: list[str] = []
    L.append("# 完整版缠论+PH赋格回测 — persistence过滤版\n")
    L.append("## 架构\n")
    L.append("基于完整版阶段一，增加 persistence 门槛过滤：\n")
    L.append("- 走势 settle 的 persistence >= 操作级别 PH alive median → 触发38课段间比较")
    L.append("- 走势 settle 的 persistence < median → 只触发降成本，不触发退出判断")
    L.append("- persistence 阈值从数据涌现（median of alive），不是人工参数\n")
    L.append("其余架构与完整版阶段一相同：\n")
    L.append("1. 完整38课操作程式FSM（6状态）")
    L.append("2. 走势段比较（S7-S9，仅高persistence走势）")
    L.append("3. 区间套定位：L0 PH settle精确入场")
    L.append("4. 降成本子状态机：FULL_POS↔REDUCED→PRINCIPAL_RECOVERED\n")
    ...
    filtered_row = "| 7. **persistence过滤版（本版）** |"
    for res in all_results:
        m = res["metrics"]
        filtered_row += f" **{m['total_compound']:+.1f}%** |"
    ...
    L.append("## 结果包六要素\n")
    L.append(
        "**结论**：persistence过滤版在完整版基础上，"
        "通过PH alive median门槛过滤低persistence走势settle，"
        "减少虚假退出。\n"
    )
    L.append("**定义依据**：")
    L.append("- 38课段间比较适用于操作级别走势类型")
    L.append("- 低persistence的走势 = 结构性影响小的小走势")
    L.append("- persistence门槛 = PH alive组件的median（从数据涌现）")
    L.append("- 高persistence settle → 段间比较（S7退出判断）")
    L.append("- 低persistence settle → 只降成本（不退出）\n")
    L.append("**边界条件**：")
    L.append(f"- PENDING_EXPIRY = {PENDING_EXPIRY} bars")
    L.append("- median threshold可能过高或过低，取决于数据特征")
    L.append("- 阶段一：纯做多，向下段空仓等待")
    L.append("- 无滑点/手续费建模\n")
    L.append("**下游推论**：")
    L.append("- 若persistence过滤版 > 完整版 → 低persistence settle是噪音源")
    L.append("- 若persistence过滤版 ≈ 纯做多 → 过滤后的段间比较接近PH flip退出效果")
    L.append("- 若persistence过滤版 > 纯做多 → 段间比较在正确粒度上改善了退出\n")
    L.append("**谱系引用**：")
...
def main() -> None:
    ...
    for symbol in symbols:
        print(f"\n{'=' * 60}")
        print(f"  {symbol} — persistence过滤版赋格回测（纯做多+降成本）")
        print(f"{'=' * 60}")
