...
基线：区间套版（QQQ+60.7%, OKLO+192.4%, HK700+325.3%）

改动（仅加仓逻辑）：
1. S7b-ii"创新高且不背驰"→ 除继续持有外，触发趋势追加仓位
   - 加仓比例 = persistence_ratio × 0.5（追涨比1买更保守）
2. S5a"回调不跌破前低"→ 加仓不要求同时有 buy BSP（结构信号已充分）

不改的：FSM主流程、退出判断、降成本、区间套进出场、persistence过滤。
...
OUTPUT_MD = ROOT / "analysis" / "fugue_enhanced_addon_backtest_1min.md"
        ...
        if state == St.WAIT_ENTRY:
                    ...
                    for settled_mv in new_settled_up_moves:
                        ...
                        if not up_seg_records:
                            ...
                            if _check_no_new_high(
                                ...
                                # S7b-ii: 创新高且不背驰 → 持有 + 趋势加仓
                                up_seg_records.append(new_seg)
                                state = St.WAIT_DIP
                                states_in_trade.add(St.WAIT_DIP.name)
                                current_down_low = float("inf")

                                # 趋势追加仓位（ratio×0.5，追涨更保守）
                                ratio = _persistence_ratio(
                                    l1_down.alive, 1,
                                )
                                if ratio > 0.05 and total_shares > 0:
                                    add_r = ratio * 0.5
                                    add_shares = total_shares * add_r
                                    old_val = total_shares * cost_basis
                                    new_total = total_shares + add_shares
                                    cost_basis = (
                                        (old_val + add_shares * c)
                                        / new_total
                                    )
                                    total_shares = new_total
                                    add_count += 1
                                    ev += (
                                        f" → WAIT_DIP+ADD(创新高无背驰"
                                        f" +{add_shares:.1f}"
                                        f" r={add_r:.3f})"
                                    )
                                else:
            ...
            # BSP invalidate
                ...
                if dip_ended and up_seg_records:
                    ...
                    # S5a: 不跌破前低 → 重新买入（结构信号充分，不要求buy BSP）
                    if current_down_low >= last_up.low:
                        state = St.HOLDING
                        cost_state = CostSt.FULL_POS
                        has_active_trim = False

                        # 加仓（结构信号：回调不破前低 = 趋势延续确认）
                        ratio = _persistence_ratio(l1_down.alive, 1)
                        if ratio > 0.05 and total_shares > 0:
                            add_shares = total_shares * ratio
                            old_val = total_shares * cost_basis
...
def write_report(all_results: list[dict]) -> None:
    L: list[str] = []
    L.append("# 维度3：加仓逻辑完善 — 在区间套版基础上叠加\n")
    L.append("## 架构\n")
    L.append("基线：区间套版。改动仅加仓逻辑：\n")
    L.append("1. S7b-ii 创新高且不背驰 → 趋势追加仓位（ratio×0.5）")
    L.append("2. S5a 回调不跌破前低 → 加仓不要求同时有buy BSP\n")
    L.append("不改的：FSM主流程、退出判断、降成本、区间套、persistence过滤。\n")
    ...
    L.append("| 7+D2. +区间套定位 | +60.7% | +192.4% | +325.3% |")

    addon_row = "| 7+D2+D3. **+加仓完善（本版）** |"
    for res in all_results:
        m = res["metrics"]
        addon_row += f" **{m['total_compound']:+.1f}%** |"
    L.append(addon_row)
    L.append("")
    ...
    L.append("**定义依据**：")
    L.append("- 38课S7b-ii：创新高且不背驰 = 趋势力度强劲")
    L.append("- 38课第30行：分区卷钱 = 不同级别筹码各做各的")
    L.append("- 趋势追加 ratio×0.5（追涨比1买更保守）")
    L.append("- S5a 回调不破前低 = 结构性趋势延续确认，不需要额外BSP\n")
    L.append("**边界条件**：")
    L.append(f"- PENDING_EXPIRY = {PENDING_EXPIRY} bars")
    L.append("- 趋势加仓系数 0.5（可调）")
    L.append("- 加仓在追涨点，风险高于1买入场\n")
    L.append("**下游推论**：")
    L.append("- 若加仓版 > 区间套版 → 趋势追加在正确时机产生了额外收益")
    L.append("- 若加仓版 < 区间套版 → 追涨加仓拉高了成本，被回调吃掉")
    L.append("- 加仓次数/每笔交易的加仓占比可验证分区卷钱效果\n")
    L.append("**谱系引用**：")
