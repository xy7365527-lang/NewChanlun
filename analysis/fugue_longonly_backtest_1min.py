"""纯做多版多级别 PH 分层赋格 FSM 回测 — 1分钟数据版本。

基于 fugue_multilevel_ph_backtest_1min.py（表现最好的版本），去掉所有做空逻辑：
- FSM 只响应 buy 方向的 BSP candidate
- sell BSP 只用于降成本平仓触发，不开空
- L2 PH rank-1 settle 向下 = 清仓退出（不是翻空）
...
| 信号         | PH级别 | 触发条件              |
|-------------|--------|----------------------|
| 方向裁决     | L2     | rank-1 settle 向上    |
| 清仓退出     | L2     | rank-1 settle 向下    |
...
OUTPUT_MD = ROOT / "analysis" / "fugue_longonly_backtest_1min.md"
PENDING_EXPIRY = 390
...
# ════════════════════════════════════════════════════════════
# FSM 状态（纯做多：去掉 STOPPED_OUT，因为不做空不存在空头止损）
# ════════════════════════════════════════════════════════════
...
def run_backtest(
    ...
    def _close_trade(bar_idx: int, price: float, reason: str) -> None:
        nonlocal state, total_shares, cost_basis, entry_price
    ...
    for i in range(n):
        ...
        # ══════ 5. BSP 事件提取（纯做多：只关注 buy 候选 + sell 用于降成本） ══════
        buy_cands: list[int] = []
        sell_cands: list[int] = []
        ...
        if state == St.SCANNING:
            ...
            if pending_entry:
                if (i - pending_entry["bar"]) > PENDING_EXPIRY:
                    ...
                    if any(bid in pending_entry["bsp_ids"] for bid in buy_invalidates):
                        pending_entry = None

            # 进场：pending buy BSP + L0 rank-1 settle 确认
            ...
            if should_stop:
                ...
                ev = f"EXIT:{stop_reason}@{c:.2f} pnl={pnl:+.2f}%"
                _close_trade(i, c, stop_reason)

            else:
                # ── 加仓逻辑（buy BSP + L1 persistence ratio）──
                side_match_cands = buy_cands + buy_confirms
                ...
                # 多头降成本：L1 up non-rank-1 settle → 减仓卖出
                # 然后 L1 down non-rank-1 settle 或 buy BSP → 买回
...
def write_report(all_results: list[dict]) -> None:
    L: list[str] = []
    L.append("# 纯做多版多级别 PH 分层赋格 FSM 回测 — 1分钟数据\n")
    L.append("## 架构\n")
    L.append("基于多级别 PH 版本（表现最好），去掉所有做空逻辑：\n")
    L.append("- FSM 只响应 buy 方向的 BSP candidate")
    L.append("- sell BSP 只用于降成本平仓触发，不开空")
    L.append("- L2 PH rank-1 settle 向下 = 清仓退出（不翻空）")
    L.append("- 降成本保留：持有多头时，次级别卖点减仓，再买回")
    L.append("- 清仓后回到 SCANNING 等待下一个买点\n")
    L.append("4状态 FSM + 三级别 PH 分层：\n")
    ...
    L.append("| L2 | L1走势端点价格 | ~数百次 | 方向裁决（rank-1 settle 向上）/ 清仓（向下） |\n")
    ...
    for res in all_results:
        ...
        L.append("### 交易明细\n")
        L.append("| # | 入场 | 出场 | 持仓bars | PnL% | 加仓 | 短差 | 本金回收 | 退出原因 | 状态路径 |")
        L.append("|---|------|------|---------|------|------|------|---------|---------|---------|")
        for idx, t in enumerate(res["trades"]):
            ...
            L.append(f"| {idx+1} | {t.entry_price:.2f}@{t.entry_bar} | "
        ...
        for st in ["SCANNING", "POSITION_OPEN", "COST_REDUCING", "PRINCIPAL_WITHDRAWN"]:
    ...
    L.append("|------|-----|------|-------|")
    L.append("| 1. 单层PH | -19.8% | -99% | -84% |")
    L.append("| 2. 多级别PH（双向） | +58.3% | +72.3% | +222.8% |")
    L.append("| 3. Regime过滤器 | +58.3% | +23.7% | +222.8% |")
    L.append("| 4. 走势类型过滤 | +18% | -86% | +6% |")

    longonly_row = "| 5. **纯做多（本版）** |"
    for res in all_results:
        m = res["metrics"]
        longonly_row += f" **{m['total_compound']:+.1f}%** |"
    L.append(longonly_row)
    L.append("")
    ...
    for res in all_results:
        ...
        m = res["metrics"]
        bh = res["bh"]
        L.append(f"**{sym}**：纯做多 {m['total_compound']:+.1f}% vs 双向 "
                 f"{'(见上表)'} vs BH {bh:+.1f}%")
    L.append("")
    ...
    L.append("## 结果包六要素\n")
    L.append("**结论**：纯做多版多级别 PH 分层 FSM 在 1 分钟级别 3 标的上的表现。"
             "去掉所有做空逻辑，L2 翻空只清仓不翻空。\n")
    L.append("**定义依据**：")
    L.append("- 与多级别PH版本相同的三级别 PH 架构")
    L.append("- 区别：SCANNING 状态只监听 buy BSP candidate")
    L.append("- sell BSP 降级为降成本触发条件，不具有开仓能力")
    L.append("- L2 rank-1 settle 向下 = 清仓退出，不开空头\n")
    L.append("**边界条件**：")
    L.append(f"- PENDING_EXPIRY = {PENDING_EXPIRY} bars")
    L.append("- 纯做多 → 下跌行情中完全空仓（vs 双向版可做空获利）")
    L.append("- 适合长期上涨标的（如 QQQ），不适合震荡/下跌标的")
    L.append("- 无滑点/手续费建模\n")
    L.append("**下游推论**：")
    L.append("- 若纯做多 > 双向 → 做空信号是噪音，损害整体表现")
    L.append("- 若纯做多 < 双向 → 做空信号确有 alpha")
    L.append("- 若纯做多 ≈ BH → FSM 的买卖择时不如持有\n")
    L.append("**谱系引用**：")
...
def main():
    ...
    for symbol in symbols:
        print(f"\n{'='*60}")
        print(f"  {symbol} — 纯做多版多级别 PH 分层 FSM 回测 (1min)")
        print(f"{'='*60}")
