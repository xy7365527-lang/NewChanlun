"""persistence过滤版 — 控制变量：盘整背驰度量从价格振幅改为MACD面积。

与 fugue_complete_persistence_filtered_backtest_1min.py 的唯一差异：
- 旧：盘整背驰 = abs(C.high - C.low) < abs(A.high - A.low)（价格振幅）
- 新：盘整背驰 = MACD_hist_area(C段) < MACD_hist_area(A段)（MACD面积）

其他所有逻辑完全不变：
- persistence过滤阈值（median of alive）
- FSM状态机（6状态）
- 降成本子状态机
- 进场逻辑（buy BSP + L2方向 + L0确认）
- L2方向裁决
- 不创新高判断（价格直接比较，不涉及MACD）

控制变量实验：只有"盘整背驰度量"这一个变量改变。
...
OUTPUT_MD = ROOT / "analysis" / "fugue_persistence_macd_backtest_1min.md"
PENDING_EXPIRY = 390
...
def _macd_area_from_hist(
    hist_list: list[float],
    raw_i0: int,
    raw_i1: int,
    direction: str,
) -> float:
    """从 OnlineMacdState._hist_hist 直接计算 MACD hist 面积。O(range)。"""
    i0 = max(0, raw_i0)
    i1 = min(len(hist_list) - 1, raw_i1)
    if i0 > i1:
        return 0.0
    if direction == "up":
        return sum(max(0.0, hist_list[k]) for k in range(i0, i1 + 1))
    return sum(abs(min(0.0, hist_list[k])) for k in range(i0, i1 + 1))
...
def _check_consolidation_divergence_macd(
    ...
    """38课S7b-i: 盘整背驰 — MACD面积版。
    创新高但MACD面积减弱 = 盘整背驰。
    返回 (is_divergent, area_prev, area_curr)。
    """
    if curr_high <= prev_high:
        return False, 0.0, 0.0

    n = len(merged_to_raw)
    if prev_mi0 >= n or prev_mi1 >= n or curr_mi0 >= n or curr_mi1 >= n:
        return False, 0.0, 0.0

    raw_a0 = merged_to_raw[prev_mi0][0]
    raw_a1 = merged_to_raw[prev_mi1][1]
    raw_c0 = merged_to_raw[curr_mi0][0]
    raw_c1 = merged_to_raw[curr_mi1][1]

    hist = macd_state._hist_hist
    area_a = _macd_area_from_hist(hist, raw_a0, raw_a1, "up")
    area_c = _macd_area_from_hist(hist, raw_c0, raw_c1, "up")

    return (area_a > 0 and area_c < area_a), area_a, area_c
                    ...
                    for settled_mv in new_settled_up_moves:
                        ...
                        if not up_seg_records:
                            ...
                            if _check_no_new_high(
                                ...
                                is_div, area_a, area_c = (
                                    _check_consolidation_divergence_macd(
                                        macd_state, m2r,
                                        prev_seg.merged_i0,
                                        prev_seg.merged_i1,
                                        new_seg.merged_i0,
...
def write_report(all_results: list[dict]) -> None:
    L: list[str] = []
    L.append("# persistence过滤版 — 控制变量：MACD面积盘整背驰\n")
    L.append("## 架构\n")
    L.append("与 persistence 过滤版完全相同，唯一差异：\n")
    L.append("- **旧**（价格振幅）：`|C.high - C.low| < |A.high - A.low|` → 盘整背驰")
    L.append("- **新**（MACD面积）：`MACD_hist_area(C) < MACD_hist_area(A)` → 盘整背驰\n")
    L.append("MACD通过增量OnlineMacdState计算（O(1)/bar），面积查询O(段长度)。\n")
    L.append("控制变量：只有盘整背驰度量改变。其余所有逻辑不变。\n")
    ...
    L.append("| 7a. persistence过滤（价格振幅） | +58.7% | +194.4% | +313.0% |")

    macd_row = "| 7b. **persistence过滤（MACD面积）** |"
    for res in all_results:
        m = res["metrics"]
    ...
    L.append("**定义依据**：")
    L.append("- 38课盘整背驰 = A段力度 vs C段力度")
    L.append("- 价格振幅：|high-low|（几何度量）")
    L.append("- MACD面积：sum(hist>0)（动量度量）")
    L.append("- 两者测量不同的东西——振幅测波动范围，MACD测动量累积\n")
    L.append("**边界条件**：")
    L.append(f"- PENDING_EXPIRY = {PENDING_EXPIRY} bars")
    L.append("- MACD参数(12,26,9)标准值")
    L.append("- 只影响「创新高+背驰」分支，不影响「不创新高」分支\n")
    L.append("**下游推论**：")
    L.append("- 若MACD面积版 > 价格振幅版 → MACD动量度量更精确识别力度衰竭")
    L.append("- 若MACD面积版 < 价格振幅版 → MACD面积在走势级别上更宽松（更多假背驰）")
    L.append("- 若MACD面积版 ≈ 价格振幅版 → 度量选择不是关键变量\n")
    L.append("**谱系引用**：")
...
def main() -> None:
    ...
    for symbol in symbols:
        print(f"\n{'=' * 60}")
        print(f"  {symbol} — persistence过滤+MACD面积版赋格回测")
        print(f"{'=' * 60}")
