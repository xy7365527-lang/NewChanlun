"""维度4：做空模块 — 在D2无加仓版基线上叠加。

基线：D2无加仓版（QQQ+138.9%, OKLO+652.3%, HK700+1644.6%）

改动：
- 向上段结束 + L2 PH确认翻空 → 进入向下段做空
- 向下段操作是38课程式镜像：先卖后买，比较"不创新低"/"盘整背驰"
- 向下段用persistence过滤（同向上段）
- 向下段不加仓、不用区间套（保守）
- 向下段有降成本（买低卖高）
- L2未翻空 → 不做空，空仓等待（同基线）

不改的：向上段全部逻辑（FSM/persistence过滤/区间套/无加仓/降成本）。
...
OUTPUT_MD = ROOT / "analysis" / "fugue_with_short_backtest_1min.md"
...
class St(Enum):
    ...
    EXIT = auto()
    # 向下段（做空）
    SHORT_WAIT_ENTRY = auto()
    SHORT_HOLDING = auto()
    SHORT_EVAL = auto()
    SHORT_WAIT_RALLY = auto()
    SHORT_OBSERVE = auto()
...
def run_backtest(
    ...
    short_cumulative_recovered = 0.0
    short_has_trim = False
    short_trim_buy_price = 0.0
    short_trim_shares = 0.0
    short_diff_count = 0
    short_states: set[str] = set()
    down_seg_records: list[DownSegRecord] = []
    current_rally_high = 0.0
    short_pending: dict | None = None
    ...
    def _close_short(bar_idx: int, price: float, reason: str) -> None:
        nonlocal state, short_entry_price, short_entry_bar, short_shares
        nonlocal short_cost_basis, short_cumulative_recovered, own_capital
        nonlocal short_has_trim, short_diff_count, short_states
        nonlocal down_seg_records, current_rally_high, short_pending

        if short_shares <= 0 or short_entry_price <= 0:
            state = St.WAIT_ENTRY
            return

        pnl_pct = (short_cost_basis - price) / short_entry_price * 100
        trades.append(CompletedTrade(
            entry_bar=short_entry_bar, entry_price=short_entry_price,
            exit_bar=bar_idx, exit_price=price,
            pnl_pct=round(pnl_pct, 4),
            states_visited=sorted(short_states),
            n_add_positions=0,
            n_short_diffs=short_diff_count,
            cumulative_recovered=short_cumulative_recovered,
            principal_withdrawn=False,
            exit_reason=reason, cost_basis_at_exit=short_cost_basis,
            n_up_segs=len(down_seg_records),
            side="short",
        ))
        remaining = own_capital * (1 + pnl_pct / 100)
        state = St.WAIT_ENTRY
        short_entry_price = 0.0
        short_entry_bar = -1
        short_shares = 0.0
    ...
    for i in range(n):
                ...
                for m in moves:
                    if (m.seg_start == e.seg_start
                        ...
                        if e.direction == "up":
                            ...
                            new_settled_down_moves.append(m)
                        break
        ...
        # ══════ 向上段退出后 → 检查是否进入向下段做空 ══════
        if state == St.WAIT_ENTRY and l2_direction == -1 and l2_flip_short:
            state = St.SHORT_WAIT_ENTRY
            short_pending = None
            if not ev:
                ev = f"→SHORT_WAIT_ENTRY@{c:.2f} L2翻空"
            else:
                ev += f" → SHORT_WAIT_ENTRY"

        # ══════ 8. 向下段 FSM（做空，38课程式镜像） ══════

        elif state == St.SHORT_WAIT_ENTRY:
            # 等待操作级别上涨背驰（sell BSP + L2方向为空）
            if sell_cands and l2_direction == -1:
                short_pending = {"bar": i, "bsp_ids": set(sell_cands)}

            if short_pending:
                if (i - short_pending["bar"]) > PENDING_EXPIRY:
                    short_pending = None

            # 入场：sell BSP + L0 rank-1 up settle
            if short_pending and l0_r1_up:
                state = St.SHORT_HOLDING
                short_entry_price = c
                short_entry_bar = i
                short_cost_basis = c
                short_shares = own_capital / c
                short_cumulative_recovered = 0.0
                short_has_trim = False
                short_diff_count = 0
                short_states = {St.SHORT_HOLDING.name}
                down_seg_records = []
                current_rally_high = 0.0
                short_pending = None
                ev = f"SHORT_ENTRY@{c:.2f} L2dir={l2_direction}"

            # L2翻多 → 放弃做空
            if state == St.SHORT_WAIT_ENTRY and l2_flip_long:
                state = St.WAIT_ENTRY
                short_pending = None
                if not ev:
                    ev = f"SHORT_ABORT:L2_FLIP_LONG@{c:.2f}"

        elif state == St.SHORT_HOLDING:
            short_states.add(St.SHORT_HOLDING.name)

            # L2翻多 → 平空退出
            if l2_flip_long:
                ev = f"SHORT:L2_FLIP_LONG@{c:.2f}"
                _close_short(i, c, "l2_direction_flip_long")

            else:
                # 走势段比较（镜像：down-move settle）
                if new_settled_down_moves:
                    med_p = _median_alive_persistence(l1_up.alive)

                    for settled_mv in new_settled_down_moves:
                        mv_p = settled_mv.persistence
                        new_dseg = DownSegRecord(
                            seg_start=settled_mv.seg_start,
                            high=settled_mv.high,
                            low=settled_mv.low,
                            bar_start=settled_mv.first_seg_s0,
                        ...
                        short_states.add(St.SHORT_EVAL.name)
                        ev = (
                            f"SHORT→EVAL low={settled_mv.low:.2f}"
                            f" force={new_dseg.force:.2f}"
                        )

                        if not down_seg_records:
                            down_seg_records.append(new_dseg)
                            state = St.SHORT_WAIT_RALLY
                            short_states.add(St.SHORT_WAIT_RALLY.name)
                            current_rally_high = 0.0
                            ev += f" → SHORT_WAIT_RALLY(D1完成)"
                        else:
                            prev_dseg = down_seg_records[-1]

                            # 不创新低 → 平空
                            if new_dseg.low > prev_dseg.low:
                                down_seg_records.append(new_dseg)
                                ev += (
                                    f" → SHORT_EXIT(不创新低"
                                    f" {new_dseg.low:.2f}"
                                ...
                                _close_short(i, c, "no_new_low")
                                break

                            # 盘整背驰 → 平空
                            elif (new_dseg.low < prev_dseg.low
                                  and new_dseg.force < prev_dseg.force):
                                down_seg_records.append(new_dseg)
                                ev += (
                                    f" → SHORT_EXIT(盘整背驰"
                                    f" {new_dseg.force:.2f}"
                                    f"<{prev_dseg.force:.2f})"
                                )
                                _close_short(
                                    i, c, "short_consolidation_divergence",
                                )
                                break
                                ...
                                current_rally_high = 0.0
                                ev += (
                                    f" → SHORT_WAIT_RALLY(创新低无背驰)"
                                )

                # 做空降成本（L1 down non-rank-1 settle → 买低回补）
                if state == St.SHORT_HOLDING:
                    if not short_has_trim and l1_nr1_down:
                        ratio = _persistence_ratio(l1_down.alive, 1)
                        if ratio > 0.05:
                            short_trim_buy_price = c
                            short_trim_shares = short_shares * ratio
                            short_has_trim = True
                            if not ev:
                                ev = f"SHORT_TRIM_BUY@{c:.2f}"
                    elif short_has_trim and l1_nr1_up:
                        profit = (c - short_trim_buy_price) * short_trim_shares
                        if short_shares > 0:
                            short_cost_basis += profit / short_shares
                            short_cumulative_recovered += profit
                        short_diff_count += 1
                        short_has_trim = False
                        if not ev:
                            ev = f"SHORT_TRIM_SELL@{c:.2f} profit={profit:.2f}"

        elif state == St.SHORT_WAIT_RALLY:
            short_states.add(St.SHORT_WAIT_RALLY.name)

            if c > current_rally_high:
                current_rally_high = c

            # L2翻多 → 平空
            if l2_flip_long:
                ev = f"SHORT_RALLY:L2_FLIP@{c:.2f}"
                _close_short(i, c, "l2_direction_flip_long_in_rally")

            else:
                # 反弹结束信号
                rally_ended = bool(sell_cands) or l0_r1_up
                if rally_ended and down_seg_records:
                    last_down = down_seg_records[-1]

                    if current_rally_high <= last_down.high:
                        state = St.SHORT_HOLDING
                        short_has_trim = False
                        ev = (
                            f"SHORT_RALLY_RESELL@{c:.2f} 不突破前高"
                        )
                    else:
                        state = St.SHORT_OBSERVE
                        short_states.add(St.SHORT_OBSERVE.name)
                        ev = f"SHORT→OBSERVE@{c:.2f} 突破前高"

        elif state == St.SHORT_OBSERVE:
            short_states.add(St.SHORT_OBSERVE.name)

            if l2_flip_long:
                ev = f"SHORT_OBS:L2_FLIP@{c:.2f}"
                _close_short(i, c, "l2_direction_flip_long_in_observe")

            else:
                if sell_cands and l0_r1_up:
                    state = St.SHORT_HOLDING
                    short_has_trim = False
                    ev = f"SHORT_OBS_REENTRY@{c:.2f} 新上涨背驰"
    ...
    if state in (
        St.SHORT_HOLDING, St.SHORT_WAIT_RALLY, St.SHORT_OBSERVE,
    ) and short_shares > 0:
        _close_short(n - 1, closes[-1], "eod_close_short")
...
def write_report(all_results: list[dict]) -> None:
    L: list[str] = []
    L.append("# 维度4：做空模块 — D2无加仓版基线\n")
    L.append("## 架构\n")
    L.append("在D2无加仓版基线上增加向下段做空操作：\n")
    L.append("- 向上段结束 + L2翻空 → 进入向下段做空")
    L.append("- 向下段：38课程式镜像（先卖后买，不创新低/盘整背驰判断）")
    L.append("- 做空用persistence过滤，不加仓，不用区间套，有降成本")
    L.append("- L2未翻空 → 不做空，空仓等待\n")
    ...
    L.append("| D2无加仓版（基线） | +138.9% | +652.3% | +1644.6% |")

    short_row = "| D2无加仓+做空（**本版**） |"
    for res in all_results:
        m = res["metrics"]
        short_row += f" **{m['total_compound']:+.1f}%** |"
    L.append(short_row)
    L.append("")
    ...
    for res in all_results:
        ...
        L.append(
            f"**{res['symbol']}**: "
            f"多头{len(longs)}笔({(l_eq-1)*100:+.1f}%), "
            f"空头{len(shorts)}笔({(s_eq-1)*100:+.1f}%)"
        )
    L.append("")
    ...
    L.append("**定义依据**：")
    L.append("- 38课S12-S14：向下段先卖后买，镜像操作")
    L.append("- 做空前提：L2 PH方向确认翻空（不是向上段结束就做空）")
    L.append("- 向下段段间比较：不创新低/盘整背驰\n")
    L.append("**边界条件**：")
    L.append(f"- PENDING_EXPIRY = {PENDING_EXPIRY} bars")
    L.append("- 做空不用区间套（保守）")
    L.append("- 做空不加仓")
    L.append("- QQQ/OKLO/HK700全是牛市标的，做空信号预期负贡献\n")
    L.append("**下游推论**：")
    L.append("- 若做空版 < 纯多头 → 做空在牛市标的上是噪音")
    L.append("- 若做空版 > 纯多头 → 做空在熊市段产生了正收益")
    L.append("- 多空拆分可独立评估各自alpha\n")
    L.append("**谱系引用**：")
