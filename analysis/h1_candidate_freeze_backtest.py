"""H1 candidate 冻结五标的回测：V2oa25_ht / _scco / _h1 同磁带三变体。

═══════════════════════════════════════════════════════════════════════
任务（2026-06-12 编排者任务：H1 candidate 冻结——osc 白名单的零参数消除）
═══════════════════════════════════════════════════════════════════════
h1 = osc_candidate_freeze（49课禁令窗口，行52/60/68）：candidate type3
     离开段未决期间 osc 不开腿；价格回边界内（买侧 c<ZG / 卖侧 c>ZD）
     解冻。在册缺口：is_frozen 挂 confirmed Buy3，单边趋势中回抽不发生
     ⇒ candidate 永不确认 ⇒ 禁令窗口内开腿门恒开（僵尸腿全在此窗口开出）。
     只挡开腿，永不挡闭腿（僵尸腿教训）。纯结构零参数零标的依赖。

对照：V2oa25_ht（基线）+ V2oa25_ht_scco（在册白名单形态最优组合）。

预注册判据（osc_whitelist_elimination_research.md §4 H1；任一不满足即否证）：
1. OKLO Δosc ≈ +105K（僵尸尾部 3 腿占总亏 62% 被窗口拦截）；
2. 正域标的（OKLO/BRN）不恶化（对照：co 在 OKLO 上恶化）；
3. 负域标的（BTC/CL/ES）osc 亏损大幅缩减。

口径：三变体同磁带（同一 compute_organic_signals pass），osc 腿盈亏从
diag 腿 trace 分解（leg_kind == "osc"，sc/co/scco 任务同一口径）。
tape_fp 守卫：与在册 osc_scco_{SYM}.json 指纹对账，不一致即 fail-fast。

输出：analysis/data_cache/h1_cf_{SYM}.json（每标的独立文件，规避双实例
竞态陷阱——多级别赋格任务在册教训）。
用法：BT_SYMBOLS=OKLO,BRN,BTC,CL,ES PYTHONPATH=src .venv/bin/python \
      analysis/h1_candidate_freeze_backtest.py
"""
...
BASELINE = "V2oa25_ht"
VARIANTS = [BASELINE, "V2oa25_ht_scco", "V2oa25_ht_h1"]


def osc_leg_stats(diag: list) -> dict:
    """diag 腿 trace → osc 域腿盈亏分解（sc/co/scco 任务同一口径）。"""
    n_legs = 0
    n_wins = 0
    profit_total = 0.0
...
def guard_tape_fp(symbol: str, fp: dict) -> None:
    ...
    H1 判决要与在册 sc/co/scco 判决同表比较，前提是同一磁带。指纹漂移 =
    数据重拉或信号层变更 ⇒ 不可与在册数字混表（REV腿配对任务在册教训）。
    """
...
def process_symbol(symbol: str) -> None:
    ...
    for variant in VARIANTS:
        ...
        print(f"  [{variant:16s}] 复利={m['total_compound']:+10.2f}%"
              ...
              f" 窗={co['n_cf_windows']}/否定={co['n_cf_negations']}"
