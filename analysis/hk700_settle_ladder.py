"""腾讯 700.HK（0700.HK）持续同调(PH) settle 阶梯分析 —— 与 OKLO 完全同一套引擎。

700 是 PH 研究的**基准标的**（persistence_theory.md §5-§7 全在 700 上验证）。
本脚本对 0700.HK 最近 2 年日线做 OKLO 同款分析：

  PH 七模块管线（a_persistence_barcode / a_level_detection / a_ph_zhongshu /
  a_online_persistence / a_settle_trigger / a_buypoint_score）
  + settle 阶梯时间演化（近端低点 → 当前，区间套逐级确认）

关键诚实点（formalization-validity-domain / no-patch-mentality）
---------------------------------------------------------------
1. **因果屏障定级别**：级别一律用 settle_persistence = settle价 − valley（"未来一个
   反弹要多高才吸收这条下跌腿"，纯局部纯因果），**绝不用 cap−valley**（cap=历史最高
   价，会把近端小腿误标大级别）。全局最低分量 settle_price=None，永不反弹 settle，隔离。

2. **级别名不过度声明**：级别映射直接复用引擎 detect_levels_adaptive 在**本数据上实际
   分辨出的簇**（与 §2.2 同源），不硬贴日线数据分辨不出的"30分/60分级"——日线 K 线的
   级别有效域到"日线级"为止，硬贴更细级别名=声明膨胀。band_stat 按 level_id 统计（不
   按级别名，避免重名级别 dict 覆盖丢数据）。

认识论：数据=真实(yfinance 0700.HK 日线，L2 底材)；PH 算法 L0；
...
def build_engine_level_mapper(levels):
    ...
    级别按 persistence_range 上界降序（粗→细）。相邻级别的边界 = (上级下界, 下级上界)
    的几何中点。任一 persistence 落到从粗到细第一个 ≥边界 的级别；比最细级还小则归
    最细级。这样级别数 = 引擎实际分辨出的簇数，级别名直接来自引擎（与 §2.2 一致），
    不引入日线数据分辨不出的更细级别名（formalization-validity-domain）。
    """
...
def settle_ladder(data: dict, anchor_idx: int, anchor_label: str) -> dict:
    """settle 阶梯时间演化（与 oklo_settle_ladder 同逻辑，因果屏障定级别）。

    anchor_idx : 从该索引起累积 "锚点后 newly settled" 的下跌腿。
        - 全局底锚点：min(closes) 索引（OKLO 同款，700 上会混入大涨段，仅对照）。
        - 见顶回撤锚点：max(closes) 索引（当前操盘相关——回撤段内 settle 的腿）。
    级别名复用引擎 detect_levels_adaptive 实际分级（build_engine_level_mapper）。
    band_stat 按 level_id 统计（避免重名级别 dict 覆盖）。
    """
    closes, highs, lows, dates = data["closes"], data["highs"], data["lows"], data["dates"]
    n = len(closes)
    ...
    # 当前活树（settle_triggers 必须在 finalize 前）
    cur_tree = OnlineMergeTree()
    for p in closes:
        cur_tree.update(p)
    trigs = settle_triggers(cur_tree)
    final_all = cur_tree.finalize()  # 封顶，trigs 已取；用于全量级别检测

    # 级别映射：用引擎 detect_levels_adaptive 在全窗口 mbars 上的实际分级
    ...
    for t in trigs:
        # alive 腿：用因果屏障 settle_persistence 定级别；全局腿用 current_persistence
...
def main() -> None:
    ...
    (CACHE / "HK700_1d_2y.json").write_text(json.dumps(data, ensure_ascii=False, indent=2))

    ph = analyze_ph(data)
    closes = data["closes"]
    gmin_idx = closes.index(min(closes))
    gmax_idx = closes.index(max(closes))
    ladder_global = settle_ladder(data, gmin_idx, "全局底锚点(2年前, 对照)")
    ladder_decline = settle_ladder(data, gmax_idx, "见顶后回撤锚点(当前操盘相关)")
    result = {"ph": ph, "ladder_global": ladder_global, "ladder_decline": ladder_decline}
    (CACHE / "hk700_settle_ladder.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2))
    ladder = ladder_decline

    m = ph["meta"]
    print("=" * 78)
    print(f"腾讯 700.HK PH settle 阶梯  ({m['date_start']} → {m['date_end']}, n={m['n_bars']})")
    print("=" * 78)
    ...
    print(f"全局最高 {m['price_max']} → 当前 {ladder['last']}（回撤段）")
    ...
    print(f"[对照] 全局底锚点后已 settle {ladder_global['n_settled_after_anchor']} 腿"
          f"（{ladder_global['n_above_tau']} 个 >τ）—— 含 351→677 大涨段污染")
    ...
    print(f"\n已存 analysis/data_cache/hk700_settle_ladder.json + HK700_1d_2y.json")
