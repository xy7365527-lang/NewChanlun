"""美团 3690.HK 持续同调(PH) settle 阶梯分析 —— 与 OKLO/700/MOS 完全同一套引擎。

本脚本对 3690.HK 最近 2 年日线做 OKLO/700 同款分析：
...
--------------------------------------------------------------
period="2y" 窗口（2024-05 → 2026-05）**不含** 2021 年 $460 历史高点，也不含
2024 年初的深跌。窗口内实际形态：起 112.7 → 2024-10-04 见顶 213.4（9·24 刺激
行情顶）→ 一路跌到当前 73.45（2026-05-28 创 2 年最低 73.3）。即美团当前**不在
反弹途中**，而是在见顶后持续下跌 ~19 个月、位于 2 年最低位。任务前提"反弹到 180+"
与 2 年窗口数据不符，本脚本如实分析窗口内真实走势，不弯曲方法贴叙事。
...
认识论：数据=真实(yfinance 3690.HK 日线，L2 底材)；PH 算法 L0；
...
def build_engine_level_mapper(levels):
    ...
    最细级。这样级别数 = 引擎实际分辨出的簇数，级别名直接来自引擎，
...
def analyze_ph(data: dict) -> dict:
    """PH 七模块（与 oklo_ph_driver / hk700 同结构）。"""
...
def settle_ladder(data: dict, anchor_idx: int, anchor_label: str) -> dict:
    """settle 阶梯时间演化（与 oklo/hk700 同逻辑，因果屏障定级别）。
...
def main() -> None:
    ...
    (CACHE / "MEITUAN_1d_2y.json").write_text(json.dumps(data, ensure_ascii=False, indent=2))
    ...
    ladder_decline = settle_ladder(data, gmax_idx, "见顶后回撤锚点(2024-10顶, 当前操盘相关)")
    result = {"ph": ph, "ladder_global": ladder_global, "ladder_decline": ladder_decline}
    (CACHE / "meituan_settle_ladder.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2))
    ladder = ladder_decline

    m = ph["meta"]
    print("=" * 78)
    print(f"美团 3690.HK PH settle 阶梯  ({m['date_start']} → {m['date_end']}, n={m['n_bars']})")
    print("=" * 78)
    ...
    print(f"\n已存 analysis/data_cache/meituan_settle_ladder.json + MEITUAN_1d_2y.json")
