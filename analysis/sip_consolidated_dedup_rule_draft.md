...
| # | 决策 | 草案口径 | 状态 |
|---|---|---|---|
| 1 | 主所判定 | 丢 `trf_id` 非空（TRF/暗池打印），保留主所打印 | **已对拍（#1051）**：与条件码过滤合计去除 ~52%（7 日实测）；跨所同笔多报仍有残留，见 §6 |
| 2 | 条件码过滤 | 黑名单（19 码）+ correction 黑名单（4 值），其余全保留 | **已对拍（#1051）**：合判据一实测去除 ~52% |
| 3 | 消歧键 | `(sip_timestamp, sequence_number)` vs `participant_timestamp` 二选一 | **已定稿（#1051 实测）**：A `(sip_timestamp, sequence_number)`，见 §4 |
...
## 4. 消歧键（决策 3）——已定稿（#1051 实测，2026-08-18）
...
**定稿读数（#1051 本体实测，AAPL 7 交易日 / 2,722,118 行样本，2023-09 与 2026-08 两窗）**：

| 键 | 唯一键数 | 重复率 | 组内价格一致率 |
|---|---|---|---|
| **A `(sip_timestamp, sequence_number)`** | 2,722,118 / 2,722,118 | **0.0%** | **1.0** |
| B `participant_timestamp` | 1,855,835 / 2,722,118 | 31.8% | 0.970（3% 撞键组内价格不一致 = 误并不同价成交） |

**裁定：A `(sip_timestamp, sequence_number)`。** B 键 31.8% 碰撞且 3% 异价误并，不合格；
A 全唯一且价格一致率 1.0。标定记录：`analysis/sip_dedup_key_calibration_20260818.md`。
...
## 6. 跨所同笔多报残留（#1051 实测发现，第四判据候选）

三判据全开 + A 键后，正本/raw 行数比实测 **2.11**（7 日：5,734,269 → 2,722,118），
而 #1030 基线 ≈23 是对标 Databento EQUS consolidated（SIP 全合并）。**已裁（#1064 拍 a，2026-08-18）：不加第四判据**——#1069 对拍探针否定：官方盘 recall≈1 但 precision 0.11-0.21，Massive 流 80-90% 打印系 tape-eligibility 语义（非重复打印），模糊聚类仅压 10% 且无法自证。**正本口径定稿 = 三判据**（条件码黑名单 + 丢 TRF + sip_seq 去重），体积 ≈250-380GB 照实；「consolidated」一词在本规格中意为三判据口径，非官方 tape 语义。
