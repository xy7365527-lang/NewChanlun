...
而 #1030 基线 ≈23 是对标 Databento EQUS consolidated（SIP 全合并）。差 11 倍 = **同一笔
成交经多个交易所打印**（路由/内化跨所多报），现判据不聚类跨所同笔。是否加第四判据
（同笔跨所聚类：price+sip_timestamp 邻域归并）另票裁（#1053）；此前的「正本 ≈30-50GB」
体积预期照实测上修为 ≈250-380GB（若不做第四判据）。
