...
3. ~~四 provider ↔ GG/DD 判据映射核对~~ **已完成 2026-07-14**（见 D3 映射核对栏）：无现成合规实现，D3 可结裁，结裁建议已附。
4. ~~等待结裁~~ **D3 已结裁 2026-07-14（选项 A，见 `d3-direction-ruling-20260714.md`）**；D4 确认维持 `GREEDY_V1`（事实已判）。
5. 执行队列（已入任务系统）：#73 D1 投影+version tuple、#74 D2 自动配对、#75 D5 事件持久化、#76 D7 重开复核（阻塞于 #73/#74）。
6. 遗留人工项：/tmp worktree 清理（D6 已解除阻塞，由裁决人确认后自行执行，不代删）。
