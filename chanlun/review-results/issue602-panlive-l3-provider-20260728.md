...
> **订正（评审 #629 S2，2026-07-28 补记，不静默改写）**：上一段把三者并列登记为「能力已具备、
> 本窗未触发」，口径不精确——`scan_units_absent` / `lower_legs_unprojectable` 是**数据性**
> 未触发（能力已具备，本窗数据未命中，其它 BTC 窗口或标的下可能触发）；`lower_leg_dirs_out_of_sync`
> 在**唯一调用点**（`p123_fast_replay.rs` L3 分支）是**结构性不可达**——该调用点对
> `l1_leg_dirs`/`scan_units` 做 `min()` 静默截齐，且 `lower_legs_from`/`project_to_units` 均为
> 对 `tower[1]` 的 1:1 map（长度天然相等），加上前置的 `scan_units_out_of_sync` 已拦同长破坏，
> 三重之下该守卫在生产路径上恒为 0，不是「未触发」而是「不会触发」。守卫本身对 API 的其它潜在
> 调用方仍然有效，未删除；代码侧订正见 `nest_lifecycle.rs` 调用点注释（同一票登记）。
...
| L1 逐级终局/寿命 | 身份 **238**（原文误记 239，见下方订正） / flash 45 / nonflash 191 / 1‑80‑423 | **逐值相同** |
| L2 逐级终局/寿命 | 身份 55 / flash 21 / nonflash 34 / 14‑139‑358 | **逐值相同** |
| `P421_LIFECYCLE_SUMMARY live_windows` | 50702 | **55950** |
| `P421_LIFECYCLE_SUMMARY extension_suppressed` | 26040 | **31121** |

> **补登（评审 #629 S5，2026-07-28 补记）**：上两行原文缺失，属登记完备性缺口而非发现的新回归
> ——两者均是本票本体面（L3 活窗行新增使该 bar 的活窗计数与「延伸被抑制」计数随之变化）的直接
> 产物，不构成回归；口径与本节其它行一致（pre = `d40607d805`，post = 本票 HEAD）。

> **订正（评审 #629 S4，2026-07-28 补记，不静默改写）**：L1 逐级终局/寿命身份数原文记
> **239**，评审复算（键 = `(level, side, seg_a, λ_C, b)`）为 **238**；自洽性锚：
> `entries=301`（本表首行，pre/post 不变）= L1 238 + L2 55（本表 L2 逐级终局/寿命行）+
> L3 8（§4.1/§验收3）；239+55+8=302 与 `entries=301` 差 1，238+55+8=301 自洽。pre/post 同口径
> ⟹ **不影响零回归结论**（该行「逐值相同」判定本身不变），仅原文记录的绝对数值需订正。
