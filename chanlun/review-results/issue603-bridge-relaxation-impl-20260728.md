...
> **订正（2026-07-28，锚 #618 Resolution，编排者裁：结论接受）**：本报告 §0 表格「占槽期 B 恒异于
> 完成 B」17 只与 §7 遗留 2(b) 关于「活窗与完成两路径的 centers 集合来自不同投影」的归因**系误诊**，
> 已被 #618 逐行代码核对推翻——两路径 centers 构造算法逐字相同、同读 `tower[level]`（#591 结论在
> #603 加入队列化之后依然成立）。**真机制**：目标中枢常在完成信号**同 bar** 首次被确认，覆盖率判据
> 严格 `<`（活窗观测须早于完成）天然排除同 bar 命中——是判据边界产物，不是路径分裂；旗舰案例
> `c=17170` 实为同 C 位置三个不同 `seg_a` 身份先后出现（粗键歧义，诊断行已在 #618 小修包补
> `seg_a` 字段消歧）。下文正文**保留原表述、不做静默改写**，17 只桶的归因以 #618 报告
> （`chanlun/review-results/issue618-center-projection-divergence-20260728.md`）为准。
>
> **订正（2026-07-28，锚 #619 评审，编排侧簿记）**：档2 已经编排者裁定回退（`0e0d011da2`）。
> 本报告 §4/§8-4 发布的读数（304/31/229/95.0 等含档2 贡献的口径）**作废**；当前真值 = 回退报告
> §3 档1-only 列（`entries=301 first_provable=207 refuted=28 flash=74 nonflash 226/88.0
> force_overtake=40 claimed=2`，#619 评审逐位复现坐实），覆盖率以档1-only 实测 78.7% 在案。
> 正文保留原表述、不做静默改写。
>
