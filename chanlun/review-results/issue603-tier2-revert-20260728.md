...
> **订正（2026-07-28，锚 #618 Resolution，票 #619 条件 6 收口）**：本报告 §6 遗留 1(b)
> 「活窗与完成两路径 centers 投影不同源（17 只，与 #591/#592 根因同族）」的归因**系误诊**，
> 与 impl 报告（`issue603-bridge-relaxation-impl-20260728.md` §0/§7-2(b)）同源同述，已被 #618
> 逐行代码核对推翻——两路径 centers 构造算法逐字相同、同读 `tower[level]`。**真机制**：目标
> 中枢常在完成信号**同 bar** 首次被确认，而覆盖率判据严格 `<`（活窗观测须早于完成）天然排除
> 同 bar 命中——是判据边界产物，不是路径分裂；旗舰案例 `c=17170` 实为同 C 位置三个不同
> `seg_a` 身份先后出现（粗键歧义，诊断行已在 #618 小修包 `dc2b7dd48b` 补 `seg_a` 字段消歧）。
> 下文正文**保留原表述、不做静默改写**（体例同 impl 报告注记），17 只桶的归因以 #618 报告
> （`chanlun/review-results/issue618-center-projection-divergence-20260728.md`）为准。
>
> **另注（同上，锚影子评审 #619 §3.3 / M4）**：§6 遗留 2 的候选口径 78.7% 当时是**派生量推断**
> （自陈"未重新跑候选口径脚本"）；影子评审已用独立探针实测坐实（`L1=202`、身份键口径
> `covered=156 (77.2%)`、候选口径 `covered=159 (78.7%)`，新增三条 `(15984,15789)/(66981,66923)/
> (71448,71367)`），**该遗留可结项**。
>
