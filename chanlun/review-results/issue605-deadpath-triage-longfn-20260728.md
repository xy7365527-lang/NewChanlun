...
## 修订补记（2026-07-29，影子评审 HIGH-1/MEDIUM-2 触发，编排侧落）

**HIGH-1 · §1 定档表连带面漏计补正**（影子评审 `shadow-605-review-20260728.md` 实锤）：

1. **真实消费者不是「零散测试」而是验收测试喂书通道**：三件的唯一调用点区域
   `nest_lifecycle.rs:3880` 即测试助手 `feed_prefix_phases`——**25 个调用点，覆盖 F1–F9c 共 12 个
   #421/#426 验收测试**。删除三件 = 这 12 个验收测试失去喂书通道。
2. **不能 1:1 换夹具**：该通道喂书走 `provide_pan_live_windows`（**已完成 legs** 回放窗），与生产
   `provide_active_pan_live_windows`（**active frontier**）在 #523「分水岭」维度语义不同——验收测试
   的构造依赖「已完成 legs 全量可见」这一前提，迁移须逐测适配，非机械替换。
3. **定档建议修订**：三件仍判「生产零消费」，但「5 删」须附**前置条件**——12 个验收测试的喂书
   迁移方案先行设计并落地（属独立工作量）；迁移落地前，删除的真实成本 = **测试套迁移项目**，
   非低成本清理。备选档位 = **4 归档**（维持「仅测试可达」标注，挂「测试专用通道」名分，待迁移
   方案落地后随迁移票一并删除）。终审请按修订后的连带面裁。

**MEDIUM-2 · §2.1 基线 SHA 订正**：拆分映射表标「HEAD `9e3cd25fd6`」有误（该 SHA 与真正父提交
`02ef5f93b1` 的 p123 差 +1700/−96 行，照标签 checkout 对不上）——映射表「原行段」应读作
**开工核面时点工作树**行号（开工基线实为 `02ef5f93b1` 及其前驱序列），订正于此，不影响映射内容本身。

**MEDIUM-1 · 指纹物证缺口说明**：开工态 `lib-test-baseline.log` 实为编译失败日志（前半段物证缺）；
收口指纹 2130/1/137 已经影子评审独立复现（`cargo test --lib` 逐数一致），验证结论不受影响。
