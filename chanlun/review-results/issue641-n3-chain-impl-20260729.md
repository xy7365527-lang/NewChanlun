...
>
> **订正回合登记（#667 C-1，2026-07-29）**：本份与 `-ticket641b-` 份在合流时走 add/add 并存，
> #653 影子 MED-2 的订正（头部收窄 + §五 5.4 删除线）当时只落在另存名下；两份 429 行仅 53 行
> 不同且全是这次订正，实为同源副本的订正前/后两版。现按尾部影子评审把同一订正回合到本份，
> 代码 `chain_cert/mod.rs:72` 的读数指针继续指向本份；§9.6 收口节留在 `-ticket641b-` 份。
