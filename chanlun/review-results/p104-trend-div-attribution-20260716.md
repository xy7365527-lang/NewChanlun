...
## 修复实装与复跑（#105，2026-07-16）

裁定通过后实装：`level_view.rs:691` `iter().rev().find` → `iter().find`
（离开最后中枢后的**第一个**同向段）。

p104 探针复跑（同数据、同 as_of）：

- Trend：total=521，**confirmed=429（82.3%）**，diverge_false=92，map failures=0
  - 剩余 unconfirmed 面积比：min=1.007，p50=1.95 —— 正常「差一点」分布，无口径异常
  - 段长：a_len_p50=165，**c_len_p50=267**（修复前 905,921）
- Pan（对照，路径未动）：806/3,162，与修复前一致 ✅
- 连带变化：**Trend 事件总数 995→521**（events_total 4,157→3,683；C 段不再全域延伸，
  事件定界收敛）。下游以事件计数/区间为基线的对账（#100）须按新口径重建。
- 单测：level_view 10/10；`theta_v0::classifier` 全树 317 passed / 0 failed，
  无旧语义断言残留。

## 状态

归因完成；修复已实装并通过探针复跑与单测。全量回归（p92/p93/nest 证书重跑）进行中，
#100 对账基线以修复后口径重建。
