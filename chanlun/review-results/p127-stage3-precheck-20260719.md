...
| 4 | §5.2 m8_e2e η 列口径裁定 (i)/(ii)（agent-42 注记的待确认项） | **已拍板 (i)（2026-07-19）** | 裁定采 (i)：m8_e2e 层3 η_T 列改打 η_corrected = tw() − cum_holding_cost（= r_decomp.tw_holding_cost_bridge）为唯一合法判读口径，η_T/η_* 原列保留对照（与 M7 witness 增打两行同源，修正只降不升）。**实装入库 commit `640609071d`**，锚点 `rust/src/theta_v0/backtest/wverify_run.rs:1244-1245`（表头 η_corrected 列）/ `1289-1295`（eta_t/eta_star/cum_holding_cost/eta_corrected 计算）/ `1317-1323`（report.push_str 输出含 cum_holding_cost + eta_corrected）。选项 (ii) 不采。 |
