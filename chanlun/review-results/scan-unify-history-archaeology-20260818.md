...
| `opsem_dump`（:199-231） | cand_delta/confirm_src/side | P1 计数 + strict_nest 对照 |
...
- **对照/诊断 nest（CandDeltaEvent 面）**：`assemble_certificates*`（nest.rs:1143-1336）消费 CandDeltaEvent；全部调用点 = bin/测试（strict_nest_check:1584、p116:407/983、p123、opsem_dump:224、runner_tests:121）。**→ 无库内生产调用。**
...
- P1 出处：`chanlun/review-results/strict-nesting-divergence-plan-20260708.md`（P1/P2 规格与验收门）+ `chanlun/escalate/strict-nesting-rulings-20260708.md`（三裁决：Cand^δ≔背驰段谓词、盘整背驰不入链、确认时点=完成时）。
...
4. P2 证据（c_p 生命周期）已在生产 `LevelState.cp_ownership` 产出 ⟹ 投影载体可行，不必另造扫描。
