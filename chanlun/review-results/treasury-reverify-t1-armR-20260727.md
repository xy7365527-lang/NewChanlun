...
- **订正状态**：本报告经 **#401 影子评审打回（MED×4 + LOW×5）**，已由 **#408 订正**（2026-07-27）。
  正文中形如〔**#408 订正**〕的方括号段即订正处，逐条对照与扩散面见 **§11 订正记录**、
  LOW 登记见 **§12**。订正后新增 §4.2b（臂R §2.2 归因）。
...
2. **但本批读数确实大幅偏离 v4 发表表**，发散源**不在**该修复链，而是两处：
   - **臂A（门关）**：§2.2 全部读数由**单个提交 `2fd5ee08ba`（#309 LEE M3 事件门控）**翻转；
     ...
     `4cde104869` 与之相隔 6 个提交。二者读数相同，结论不变，标签订正。〕#309 提交自述明文
     「订单流分叉……digest 不等，不作优劣判断，**未借 bit-exact**」——它从未声明 bit-exact，
     故**不是** bit-exact 声明链的反例，而是母 SPEC #385 前提枚举的**遗漏项**。
   - **臂R（门开）**：发散**可分解为两段**〔**#408 订正**，原文只登记了不可二分那段〕：
     **§2.2 读数由 #309 翻转（可归因，§4.2b）**；**§2.1/§2.3/§2.4 的发散早于 #309**，落在 v4 之后
     的头 10 个提交（`ec728bf6cf`..`12aa929d44`）内，而其中 **9/10 不可独立构建** ⟹ 该段**不可二分**，
     无法归因到单个 commit（§4.2）。
...
> `git rev-parse 2fd5ee08ba^` = `57a50d939f`，`4cde104869` 与 `2fd5ee08ba` 之间隔 **6 个提交**
> （`37a56deab1`(#308) / `38cbe04fa7` / `8dce346588` / `1cac0783f2` / `57a50d939f` 等）。
...
§2.1（trades 聚合）/§2.3（门控拒绝率）/§2.4（链深）的发散仍落在不可二分段（§4.2）。
...
- **§4.2 的诚实缺席（#408 收窄）**：**臂R §2.1/§2.3/§2.4** 的发散无 commit 级归因——不是「未做」，
  是**在册提交序列上不可做**（9/10 不可独立构建）。不以「大致归到某票」冒充归因。
  **臂R §2.2 不属此列**——它可归因到 #309（§4.2b）；原报告把不可归因的范围写成整个臂R，本票已收窄。
...
2. **母 SPEC #385 的修复链枚举缺 #308/#309（LEE M2/M3）**——正是 #309 改变了 treasury 层执行读数。
   建议 T3 报告采本报告 §4.1/§4.2b/§4.3 的分离口径：「**被测八项**枚举链复合 bit-exact 成立
   （#246 在区间外，另引二分 GOOD 链）；发散归 #309（臂A §2.2 + 臂R §2.2）+ 不可二分区间
   （臂R §2.1/§2.3/§2.4）」。〔**#408 订正**：原文写「枚举链」未限定项数，T3 引用时须改述为八项。〕
3. **票体要求「臂A/臂B 读数与 v4 发表表逐项比对」已完成**（§2）；「逐条归因到具体开关/commit」
   ...
   - **臂R §2.1/§2.3/§2.4**：落在 `ec728bf6cf`..`12aa929d44` 不可二分段（9/10 不可独立构建），
...
| 原断言 | 订正后 | 证据 |
|---|---|---|
| §0.2 / §4.1 / §4.2「`4cde104869`（#309 之父）」 | `2fd5ee08ba` 的父提交实为 **`57a50d939f`**，`4cde104869` 与之相隔 **6 个提交**（含 `37a56deab1` = #308 LEE M2）。真父 `57a50d939f` 独立跑批亦为 GOOD（三窗 execR = +923705 / -1414690 / +2508870）⟹ **§4.1 结论不变且更稳**（GOOD/BAD 分界直接落在父子相邻处），订正的是提交谱系标签 | `git rev-parse 2fd5ee08ba^` = `57a50d939f`；`git log --oneline 4cde104869..2fd5ee08ba` = 6 条；`/tmp/bisect_log/57a50d939f.out` |

**#401 评审报告本身亦承此误**（其 MED-2 写「`4cde104869` …恰是区间起点 `2fd5ee08ba` 的父」）——
但该误述不影响其 MED-2 的结论（#246 四提交全在区间外，独立成立）。

### 11.3 扩散面（历史不改写，以本节覆盖）

| 载体 | 受影响断言 | 覆盖方式 |
|---|---|---|
| commit `0dc5bc785d` message | 「48 提交」 | 不改写历史；以本节 §11.1 MED-1 覆盖（正确值 36/37/38 按口径） |
| #387 resolution 评论 | 「48 提交」「整条修复链…零影响」（九项）「臂R 侧不可归因」 | 不改写；以 §11.1 MED-1/2/3 覆盖 |
| #401 票体验收标准 | 沿用「48 提交」 | 以本节覆盖；#401 已关，不追改 |
| **#385 Lead 红线裁定评论**（[issuecomment-5090766856](https://github.com/xy7365527-lang/NewChanlun/issues/385#issuecomment-5090766856)，2026-07-27） | 背景段写「48 提交 × 19 列 × 三窗 × 双臂逐字段相同」（→ 36，八项）；「臂R 早期发散区间 9/10 提交不可构建，无法归因到单 commit」（→ 仅 §2.1/§2.3/§2.4，§2.2 可归因 #309） | 见 §11.4 |
...
> （§2.2 归 #309），不可归因的部分相应缩小到 §2.1/§2.3/§2.4。裁定的两条支撑
...
「臂R 发散 = #309 段（可归因）+ `ec728bf6cf`..`12aa929d44` 段（不可二分）」，
