...
description: >
  蜂群元编排方法论。在使用Agent Teams开发形式化/量化系统时激活。
...
- 第四步（谱系比对）：由lead执行。lead维护全局视野，判断矛盾是否需要上浮给编排者。
- teammates之间的challenge如果暴露了实现层分歧，teammates自行解决。如果暴露了概念层矛盾，上报lead，由lead判断是否上浮。
...
- **仪式执行：** 编排者做出决断后，将新定义写入 `.chanlun/definitions/`，谱系写入 `.chanlun/genealogy/`。由于所有teammates共享文件系统，写入后teammates在下次读取时自动获得最新定义。
- **谱系写入：** 把编排者的语义层判断翻译成标准格式写入谱系。
