...
description: >
  ...
  谱系四类型：域/溯源/元规则/纠偏。结构工位：溯源审计/谱系维护/元规则观察。
---
...
- 引用的定义是否是最新版本？
- 条件是否真的被满足？逐条检查。
- **穷尽搜索，不是抽样。** 当回溯涉及原始文献（如缠论原文、论文、规格文档），搜索全部相关材料，不是找几条支持性引用。确认性偏差（只找支持结论的证据）是质询的敌人。如果你引用了原文的某句话，你有义务检查该原文中是否有与你的引用方向矛盾的其他段落。
...
- **多条谱系产生张力时，先检查是否属于不同抽象层次。** 表面上矛盾的两个结论可能分别在不同层面上成立——一个说的是系统间的关系，另一个说的是单个系统内部的结构。这种情况下矛盾是表面的，解决方式是分层而不是选边。只有在确认两个结论处于同一层次后，矛盾才是真实的，才需要上浮。
...
**谱系优先于汇总。** 工位完成后，先写谱系，再写汇总或报告。顺序不可颠倒。汇总是对已知结论的压缩，谱系写入是对生成过程的再审视——先汇总后谱系会让谱系退化为汇总的附录。先写谱系意味着在交付结论之前，先强制回溯这个结论是怎么来的、否定了什么、遗留了什么。

> **注释：谱系是发现引擎，不只是记录工具。** 谱系的强制字段（否定了什么、为什么否定、遗留了什么张力）构成一个思考框架。填写这些字段的过程本身会产出新的洞察——被否定方案之间的张力碰撞、遗留问题与已知矛盾的意外关联。这不是副作用，是谱系的核心功能之一。如果你在写谱系时发现了新东西，这个发现本身也要写入谱系。

### 谱系的内容类型

谱系条目按**内容类型**分为四类。不同类型有不同的验证标准和下游用途。写入谱系时必须标注类型。

**域谱系** —— 关于领域定义和原则的变更。例如：005的定理半和命名半的分离、"对象否定对象"作为独立原则的确立。验证标准：定义之间的逻辑一致性 + 回测数据。下游用途：指导形式化和代码实现。

**溯源谱系** —— 追溯断言到原始文本的考证。例如："概率为你服务"不是缠师原话、"不测而测"在第68课出现两次、第9/19课的概率原则和第68课在不同维度上。验证标准：原文逐字比对，穷尽搜索。下游用途：防止错误归因污染域谱系。

**元规则谱系** —— 关于元编排自身规则的变更。例如：谱系优先于汇总、穷尽搜索而非抽样、三元区分的引入。验证标准：实践中是否改善了系统运转。下游用途：更新SKILL.md和工作流程。

**纠偏谱系** —— 记录系统性偏差及其修正模式。例如：LLM倾向于软化原文立场、把不同维度的论述强行叠成纵向架构。验证标准：偏差是否在不同场景中重复出现。下游用途：预防——同类偏差再出现时可以被识别和拦截。

> **注释：四种类型的交叉检验。** 溯源谱系是域谱系的看门人——域谱系里每一条引用原文的断言都要经过溯源谱系的核实。纠偏谱系是所有其他谱系的看门人——它记录的系统性偏差模式可以在任何类型的谱系写入中被检测到。元规则谱系从其他三种谱系的运作模式中提取规则。四种类型不是并列的四个仓库，是互相监督的关系。
...
Agent Teams的工位分化不应仅靠任务驱动。任务工位（K线预处理、模式识别、pivot检测、回测等）随任务变化而变化。但有些功能不随任务转移——它们的对象不是领域问题，是系统自身的运转质量。这些功能需要结构工位：常驻的、不以当前任务为转移的工位。

结构工位的存在理由：任务工位忙着干活时，没有人专门确保元编排的流程被完整执行。谱系写入、穷尽搜索、质询序列这些规则都写在SKILL.md里，但"应该做"不等于"有人的职责就是确保它被做了"。结构工位就是让这些事情有专人负责。

### 溯源审计工位

**职责：** 对所有涉及原始文献的断言做穷尽搜索和比对。不管当前任务是实现买卖点还是实现中枢检测，只要有agent引用了原始文本（缠论原文、论文、规格文档），溯源审计工位核实引用的准确性。

**触发条件：** 任何结果包中出现对原始文献的引用或归因。

**工作内容：**
- 核实引用是否准确（逐字比对）
- 核实是否存在同一文献中与引用方向矛盾的其他段落
- 核实归因是否正确（这句话是不是这个人说的）
- 发现问题时生成溯源谱系条目，退回给引用方修正

**不做什么：** 不判断引用的结论是否正确——那是域层面的事。只判断引用本身是否忠实于原文。

### 谱系维护工位

**职责：** 审查所有谱系条目的一致性、交叉引用、模式识别。

**触发条件：** 每次有新谱系条目写入。以及周期性扫描（由lead安排）。

**工作内容：**
- 检查新条目与已有条目的一致性（域谱系之间、域谱系与溯源谱系之间）
- 发现交叉引用——新条目是否与其他条目关联但未标注
- 模式识别——多个条目是否暗示了同一个未显式化的规则（线索提交给lead做语法观察判断）
- 维护纠偏谱系——当同类偏差在不同场景中重复出现时，提取偏差模式并记录

**不做什么：** 不做域层面的判断。只维护谱系本身的质量和一致性。

### 元规则观察工位

**职责：** 观察系统运转的模式，辨认元编排自身的不足。

**触发条件：** 持续的。不是某个事件触发，是对系统运转的持续观察。

**工作内容：**
- 监控元编排流程的执行情况——谱系是否在被写入？质询序列是否在被执行？结果包是否完整？
- 当流程被跳过或执行不完整时，分析原因——是规则不清晰、是任务压力、还是规则本身不合理？
- 如果是规则不合理，生成元规则谱系条目并上浮
- 观察agents是否在一致地遵循某些未写入的规则（语法观察的执行层面）

**与lead的语法观察职责的关系：** lead负责判断（这是不是一条语法规则），元规则观察工位负责收集证据（agents在实践中的行为模式）。lead做判断，工位做观察。

> **注释：任务工位和结构工位的关系。** 任务工位处理"缠论的买卖点怎么形式化"，结构工位处理"我们形式化的过程本身是否可靠"。这不是主次关系——结构工位不是辅助任务工位的。结构工位的对象是系统本身，任务工位的对象是领域问题。两类工位有各自独立的质询标准和谱系类型。在Agent Teams模式下，结构工位可以由lead指定专门的teammate承担，也可以由lead自己承担（小团队时），但不能省略——省略结构工位等于让元编排的规则变成没有人执行的空文。

## 资源路径与I/O规范

所有agent共享同一个文件系统。以下是完整的资源地图——谁从哪里读、往哪里写、用什么工具。

### 缠论原文（只读）

溯源审计工位的核心数据源。权威性分三级，溯源比对时先查一级，不足再扩展到二级。三级仅作辅助理解，不作为溯源证据。

| 权威级别 | 路径 | 内容 |
|---------|------|------|
| 一级 | `docs/chanlun/text/blog/` | 108课+序篇+课间博文+答疑（110个md） |
| 二级 | `docs/chanlun/text/chan99/` | 编纂版拆分（48个md） |
| 三级 | `docs/chanlun/text/mindmaps/` | 思维导图，辅助理解 |
| 索引 | `docs/chanlun/text/blog/INDEX.md` | 博文总目录 |

溯源审计工位用 Grep/Read 搜索这些路径，不用 Serena（Serena 是代码语义工具，原文是自然语言文本）。

### 定义仓库（读写）

| 路径 | 读 | 写 | 说明 |
|------|---|---|------|
| `.chanlun/definitions/` | 所有agent | lead执行仪式时 | 当前有效的缠论定义和元编排原则 |

写入只通过仪式（`/ritual`）——lead在编排者确认或推论自动结算后执行。任何agent不得绕过仪式直接修改definitions/。

### 谱系记录（读写）

```
.chanlun/genealogy/
  ├── pending/    # 生成态——矛盾已辨认，尚未结算
  └── settled/    # 已结算——正向或负向
       ├── 001-degenerate-segment.md
       ├── ...
       └── 013-swarm-structural-stations.md
```

**文件组织：** 扁平结构，不按内容类型分子目录。理由：谱系之间的张力跨域（007域知识 + 012元编排 → 碰撞出013），分子目录会割裂张力发现。数量级可控（当前14条，预计<100）。

**内容类型标注：** 在文件头用结构化标签区分（域谱系/溯源谱系/元规则谱系/纠偏谱系），不通过目录区分。谱系维护工位通过读取文件头标签来执行跨类型一致性检查。

| 操作 | 谁 | 说明 |
|------|---|------|
| 读取全部 | lead + 谱系维护工位 | 谱系比对、回溯扫描、一致性检查 |
| 读取纠偏谱系 | 所有agent | 识别已知偏差模式，防止复发 |
| 写入 pending/ | lead（收到矛盾报告后） | 新辨认的矛盾进入生成态 |
| 从 pending/ 移到 settled/ | lead（结算后） | 状态变更，附写结算记录 |
| 更新 settled/ 条目 | lead（回溯结算时） | 旧条目被新定义解决时更新状态 |

### 会话状态（读写）

| 路径 | 读 | 写 | 说明 |
|------|---|---|------|
| `.chanlun/sessions/` | lead（恢复会话时） | lead（会话结束时） | 跨会话状态持久化 |

### 项目配置（只读）

| 路径 | 读 | 说明 |
|------|---|------|
| `CLAUDE.md` | 所有agent（启动时） | 项目目标、规则、当前阶段 |
| 元编排 SKILL.md | lead + 元规则观察工位 | 元编排规则本身 |

### Serena MCP 工具使用规范

Serena 提供代码语义层面的操作能力。不同工位使用不同的 Serena 功能子集。

| 工位 | Serena 功能 | 用途 |
|------|------------|------|
| 任务工位（代码实现） | `find_symbol`, `replace_symbol_body`, `insert_after_symbol` | 代码编写、重构 |
| 谱系维护工位 | `find_referencing_symbols` | 检查 definitions/ 中定义在代码中的引用是否一致 |
| 质量守卫工位 | `search_for_pattern` | 扫描代码中的硬编码超时/阈值（违反005a） |
| 溯源审计工位 | 不用 Serena | 查文本用 Grep/Read，原文在 `docs/chanlun/text/` |
| 元编排进化工位 | `write_memory`, `edit_memory` | 更新 Serena 跨会话记忆 |
| Serena 同步工位 | `write_memory`, `read_memory` | commit 时同步状态到 Serena 记忆 |

> **注释：Serena 记忆与谱系的关系。** Serena 的 `write_memory`/`read_memory` 是跨会话的轻量状态（当前进度、上次停在哪里）。谱系是重量级的知识积累（矛盾如何产生和解决）。两者不替代——Serena 记忆回答"上次做到哪了"，谱系回答"为什么这么做"。

### 持久化策略

| 产物 | 是否持久化 | 位置 | 理由 |
|------|-----------|------|------|
| 定义 | 持久化 | `.chanlun/definitions/` | 是系统运转的基础 |
| 谱系 | 持久化 | `.chanlun/genealogy/` | 是知识积累的载体 |
| 会话状态 | 持久化 | `.chanlun/sessions/` | 跨会话恢复需要 |
| 矛盾报告 | 不单独持久化 | — | 已通过谱系（pending/ → settled/）持久化 |
| 结果包 | 不持久化 | — | 过程产物，谱系已抽取关键结论 |
| Serena 记忆 | 持久化 | Serena 内部存储 | 跨会话轻量状态 |

## 开端（创世仪式）

蜂群启动时，lead执行创世仪式（`/ceremony`）。资源路径见上方"资源路径与I/O规范"。

1. 从知识仓库（`.chanlun/definitions/`）拉取当前定义
2. 拉取项目目标描述（`CLAUDE.md`）
3. 拉取已有谱系（`.chanlun/genealogy/`，如果有）
4. 加载纠偏谱系条目——提取所有文件头标注为"纠偏谱系"的条目，作为本次会话的偏差检测清单
5. 检查会话状态（`.chanlun/sessions/`）——如果有上次未完成的会话，恢复进度
6. 读取 Serena 记忆（`read_memory`）——恢复跨会话轻量状态
7. Lead确认对关键概念的理解，向编排者输出摘要
8. 编排者确认后，spawn teammates并分配任务——**系统开始自转**
...
- **结构工位不可省略。** 溯源审计、谱系维护、元规则观察——这些功能不随任务变化。省略结构工位等于让元编排的规则变成没有人执行的空文。
- **矛盾的最终裁判是数据，不是编排者。**
- **元编排自身可进化。** 谱系不仅生产域知识（缠论的定义和原则），也生产元知识（元编排自身的规则）。当谱系驱动的发现暴露了元编排规则的不足时，这些规则应该被更新。v3→v3.1→v3.2的演化本身就是这个原则的实例。元编排的规则和缠论的定义一样，受实践检验，受谱系驱动的发现所修正。
...
从原文考古实践中诞生。对缠师概率论述的审计暴露了三个问题：错误归因（"概率为你服务"非缠师原话）、不同维度的论述被强行叠成纵向架构（第9/19课的系统组合概率原则 ≠ 第68课的走势理论内部预测方法）、LLM系统性软化原文立场的偏差。

这些问题分别属于不同的内容类型，但当时谱系只按结算流程分类（谁结算的），没有按内容类型分类（记录的是什么性质的东西）。

v3.2续新增：

谱系的四种内容类型：域谱系（领域定义变更）、溯源谱系（原文考证）、元规则谱系（元编排规则变更）、纠偏谱系（系统性偏差记录）。四种类型有不同的验证标准，且互相监督——溯源谱系是域谱系的看门人，纠偏谱系是所有谱系的看门人。

结构工位：溯源审计工位（核实原文引用）、谱系维护工位（审查谱系一致性）、元规则观察工位（监控元编排流程执行）。结构工位不随任务变化——它们的对象是系统运转质量，不是领域问题。结构工位的存在理由是确保元编排的规则不沦为空文。

同时写入的元编排进化规则：谱系优先于汇总（先写谱系再汇总）、谱系是发现引擎（强制字段构成思考框架）、穷尽搜索而非抽样（质询中的反确认偏差）、分层解决表面矛盾（检查张力是否属于不同层次）、元编排自身可进化（谱系驱动的发现可以更新元编排规则本身）。

### v3.2续：资源路径与I/O规范

从实践问题诞生：Agent Teams不按元编排全部流程走。诊断：SKILL.md写了抽象规则但没写具体的读写路径和工具——agent不知道从哪里读、往哪里写、用什么工具，流程自然不被执行。

新增"资源路径与I/O规范"section。写入内容：

缠论原文三级权威体系（`docs/chanlun/text/`下 blog/chan99/mindmaps 三个目录，权威性递降）。溯源审计工位用Grep/Read搜索，不用Serena。

Serena MCP工具按工位分配：任务工位用 `find_symbol`/`replace_symbol_body`/`insert_after_symbol`，谱系维护工位用 `find_referencing_symbols`，质量守卫工位用 `search_for_pattern`，元编排进化工位和Serena同步工位用 `write_memory`/`read_memory`/`edit_memory`。溯源审计工位不用Serena。

谱系目录扁平结构确认：`pending/` + `settled/`，四种内容类型用文件头标签区分不分子目录。理由：跨域张力发现需要扁平结构，数量级可控。

持久化策略：定义、谱系、会话状态持久化。矛盾报告通过谱系持久化不单独存储。结果包不持久化。Serena记忆跨会话持久化。

创世仪式步骤更新：增加纠偏谱系加载（偏差检测清单）、会话状态恢复、Serena记忆读取三步。
