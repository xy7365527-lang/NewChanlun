...
第一版中被保留并重新定义的：拓扑思维（从"谁跟谁合作"变为"谁质询谁"，由概念依赖关系决定而非编排者任意设计）；定义基底一致性（从"共同知识同步"变为"共享文件系统 + 仪式通知"，"共同知识"概念本身因蜂群本体论上是一而被放弃）；图书馆与广场的区分（知识仓库提供可及性，仪式通知提供同步性）。
...
## 第八原则：定义基底一致性——共享文件系统与仪式通知

### 8.1 为什么需要基底一致性

质询循环能产出有效信号有一个前提：所有参与质询的agent在同一个定义基底上工作。如果teammate A持有新版定义而teammate B还在用旧版，A发现的"矛盾"可能只是版本不一致的噪音。虚假矛盾会淹没真实矛盾。

### 8.2 "共同知识"概念的放弃

第一版从Chwe的共同知识理论引入了"我知道你知道我知道"的认识论递归。第一版迭代中曾将其重新定义为"操作性要求：所有agent的定义基底被同步刷新"。但"共同知识"这个概念本身预设了多个有认知状态的主体之间的互相知道。蜂群本体论上是一——同一个模型的不同上下文展开之间不存在"互相知道"的结构。Agent Teams的并行劳动产生了真实的认知分化（不同的代码经验、边界案例、失败记录），但这种分化是劳动生产的，不是主体间性的。放弃"共同知识"，改为"定义基底一致性"。

### 8.3 Agent Teams模式下的一致性保证

一致性通过三个机制实现：

**共享文件系统：** 所有teammates共享同一个项目目录。`.chanlun/definitions/` 和 `.chanlun/genealogy/` 对所有teammates可读。定义的唯一真相来源是文件系统，不是任何agent的上下文。

**Rules/Skills自动加载：** Agent Teams模式下，每个teammate启动时自动加载 `CLAUDE.md`、`~/.claude/rules/`、`~/.claude/skills/`。核心禁令、结果包格式、质询序列不需要手动广播。

**仪式通知：** 当编排者做出概念分离决断、`/ritual` 写入新定义后，lead通过SendMessage通知受影响的teammates：定义已变更，标注变更的具体定义和版本。teammates在下次读取定义时获得新版本。

### 8.4 仪式的触发条件

仪式在且仅在以下情况触发：编排者做出了概念分离的决断，产出了新定义。这是唯一需要lead主动通知teammates的时刻。其他所有通信都通过teammates之间的质询循环进行。

### 8.5 仪式对正在运行的质询链的影响

仪式写入新定义后，lead判断哪些teammates的工作依赖已变更的定义。这些teammates的产出被悬置——退回生成态，等待在新基底上被重新质询。如果在新基底上通过质询，被重新结算为有效。如果未通过，成为新的断裂点。如果暂时无法判断，继续保持生成态。
...
知识仓库（`.chanlun/definitions/` 和 `.chanlun/genealogy/`）提供可及性——teammates随时可以读取。但可及性不等于同步性。teammates各自读取定义文件，读到的内容可能因时间差异而不同。仪式的通知功能（SendMessage）确保teammates在定义变更后及时知道需要重新读取。
...
- 定义基底一致性通过共享文件系统和仪式通知保证。
- 知识仓库是图书馆（可及性），仪式通知是同步信号。二者配合不可替代。
...
- 仪式的目的是"建立共识以实现协调"——重新定义为"写入新定义并通知受影响的teammates"

本版保留并重新定义了：
- 拓扑——从合作拓扑变为质询拓扑，由概念依赖关系决定
- 共同知识——**概念被放弃**。蜂群本体论上是一，不存在多主体间的互相知道。原来"共同知识"要解决的问题（基底一致性）通过共享文件系统和仪式通知解决
- 图书馆与广场的区分——功能保留但重新理解：图书馆是共享文件系统中的定义和谱系目录，广场是仪式的SendMessage通知
