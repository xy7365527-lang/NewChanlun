...
**日期**：2026-09-04
**裁定人**：编排者，走 [[grilling] 完整生产程序声明边界 #1321](https://github.com/xy7365527-lang/NewChanlun/issues/1321)
...
现有代码分出三条执行线：π 批量生产回测、organic 赋格交易线，以及 paper 的行情、订单和对账线。它们尚未形成一套有唯一 owner 的完整生产程序。若直接把任一现有入口扶正，结构事实、持久操作状态和交易决策仍可能各有第二套生产实现。
...
### 二、Classification 是唯一只读域间接缝

Classification 是结构判定域的唯一输出，也是结构判定域向重操作程序交付结构事实的唯一域间接缝。该契约只读且版本化；重操作程序不复制结构判定，也不生成第二份 Classification。

`NestChainGate` 归重操作程序 owner 内的准入消费职责。它只读 Classification，不回写 Classification 或结构主干，也不构成第二个 owner。

资金、成交、风控、UI 状态和任何操作状态都不能回写结构主干。该边只有 `结构判定域 → Classification → 重操作程序` 一个方向。
...
控制动作必须经正式 command 和 RiskGate 边界进入生产程序。command、事件、RiskGate 和 ReadModel 的接口形状留给 #1322 扫描后的设计裁定。
...
4. 从 π、organic 赋格和 paper 现状迁入唯一 owner 的切片顺序；
...
| 方案 | 判定 | 理由 |
|---|---|---|
| **分域单-owner** | 采纳 | 结构事实与跨交易持久状态的寿命不同。两个互斥 owner 能保持 Classification 唯一，并阻断资金、成交和 UI 状态反向污染结构。 |
| **π 合并 owner** | 拒绝 | π 会同时拥有结构判定和重操作状态，使不同寿命的职责重新混合。 |
| **π 和 organic 赋格并行双 owner** | 拒绝 | 两条生产路径会重复拥有交易决策或状态，并允许失败时切换判据。 |

分域单-owner 仍有迁移代价。现有 π 和 organic 赋格资产都不能按目录整体扶正。#1322 必须逐条辨认哪些逻辑收编、哪些入口退役；在该扫描完成前，本 ADR 不宣称接线已经实现。
