...
锁定八条纯确定性场景：3 selector / 2 个基础模型所有者桶成功、4 输入含 1 个精确重复并
去重为 3 个成功、同厂 OpenAI 仍只有 1 桶而不可用、跨两个所有者桶时同 family 的不同
版本与推理档均合法、同 selector 元数据冲突不可用、跨厂启动失败，以及 admission 已成功
但缺真实 child 结果。运行：
