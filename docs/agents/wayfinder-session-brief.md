...
2. **grilling 一次一问** —— 不批量抛题，agent 不得替人答。（2026-07-30 曾裁定改 batch 节奏，当场被 [#779](https://github.com/xy7365527-lang/NewChanlun/issues/779) 推翻并作废。）
...
- **要不要开图**：一个问题对话能 settle → 单会话 `/grilling`，不开图；路已见底只差写清楚 → 直接走 `/to-spec` 链；与在飞决策不挂钩的文本欠账 → triage 挂 `debt`，不进图。
