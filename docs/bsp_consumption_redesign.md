...
### 9.10 区间套递归落码（编排者裁决：多重赋格=区间套递归应用的自然结果）+ BTC L3（2026-06-21）

编排者裁决（替代 §9.9 "所有级别并行"）：「区间套是操作方法，多重赋格是区间套递归应用的自然结果。不是 N 个独立引擎并行消费 BSP，是一个递归过程：T 实例在操作时诞生（reduce 创造子级别仓位），recover 时归还父级。只检查当前活跃仓位的级别，不遍历所有级别。」

**嵌套重构**（撤销 §9.9 扁平 `level_short`）：`TRoot.level_short[k]` → `TInstance.child` 嵌套链 + `TInstance.level`（仓位骑的绝对级别）。`sink(parent_slot,c,bar)`：父减 quota + 在**次级别**（父级别−1）建子 T（ε 对称反向）。`recover(parent_slot,c,bar)`：父的子平仓归还。driver `reconcile_recursive`：沿活跃链（核心→子→孙）下钻，每仓位检查**自己级别**的反向/同向 BSP（反向 sink、同向 recover），不遍历所有级别。

**BTC L3 三模式（diag 兜底，但 §8.1 未触发）**：

| 指标 | 值（三模式相近） |
|------|------|
| strat | **−238%（穿仓，final_nav 负）** |
| sink / recover | 2096 / 1797（在场短差 299）|
| sink 级别分布 | L1:248 / L2:1791 / L3:57 |
| recover 级别分布 | L2:1739 / L3:58 / **L1:0** |
| 核心方向 / **净敞口** | long 99.9% / **净 long 27.3% short 72.6%** |
| §8.1 free 不足 | **0**（区间套递归解决扁平版现金流耦合 ✓）|

**诊断**：(1) §8.1 消失——区间套递归（嵌套链，子归还父）不再有扁平版的多级别现金流耦合。(2) 但**穿仓 −238%**，根因 = **做空晚平**：`reconcile_recursive` 中子（持空）归还父由**父级别 BSP**（core 持子时检查 type1_buy@core_level）触发；spawn（16 次）把核心升到高级别 → 高级别买点罕见 → 子做空长期不平 → 牛市**净空头 72.6%** → 穿仓。精确重现 [[project_t_short_close_level_mismatch]]「做空晚平=平空级别错配」。对比 §9.9 扁平版用同级别买点（频繁）平得快，net 不空（+30.5%）。
