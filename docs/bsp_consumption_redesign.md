...
- **C1（中心冲突）本级别翻转语义**：近期裁决（本 session）"本级别 type1_sell → 整仓 Z₂ 翻空（long→short）"；但早期结晶（[[emergent-direction]] 2026-06-17 + [[session5-spiral-reinterpretation]]）"**翻转应被扬弃——先卖后买是四步有时序循环**：本级别卖点→**平多（long→flat）** / 次级别做空 / 次级别买点→平空 / 本级别买点→做多"，且"sell_source fire ≠ 翻空（=平多，下一步取决于涌现新走势方向）"。**两者矛盾**：核心是 Z₂(多↔空) 还是 long↔flat（做空全在次级别）？§9.1 当前把"平多=Z₂翻转"与"开空=次级别"并列=内部不自洽（若本级别已 Z₂ 翻空，次级别为何还开空？）。这是 §11 读法A/B 的根问题，须裁。
