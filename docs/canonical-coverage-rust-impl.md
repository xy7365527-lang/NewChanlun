...
**#327 真覆盖见证锁（2026-07-26 落码，GOLDEN 先例登记）**：#323 影子评审查证出「#321 复跑的四把 BTC
见证锁在其窗口（BTC OOS 前 16000 bar）内 `zd==zg` 命中 **0** 次」——**零翻动是零覆盖的结果，不构成
安全证据**（`.chanlun/review-results/center-strict-single-point-impl-20260726.md` §3.2/§3.3）。本票补上
两把真覆盖锁，均在 `rust/src/theta_v0/backtest/wverify_run.rs`，均 `#[ignore]`（需 BTC 数据）：

| 锁 | 数据窗 | 真覆盖（`zd==zg` 命中） | 级联重排（对称差） |
|---|---|---|---|
| `center_strict_zd_eq_zg_change_block_witness` | 2022-01-01…2022-02-28（#323 点名的 2022-02 变动块） | L0 **2**（均在 2022-02-01，单点核心 3830000000000） | L0 6 / L1 2 / L2 0 / L3 0 |
| `center_strict_zd_eq_zg_full_btc_cascade_witness` | BTC 全量 4 613 599 bar | L0 **23** / L1 **2**（合计 25） | L0 26 / L1 18 / L2 8 / L3 3 / L4 2 |

- **反事实内建**：两把锁都跑双臂（生产严格判据 + `#321` 前旧弱口径本地副本），逐命中点断言
  「严格臂 `None` ∧ 弱臂 `Some` ∧ 核心恰为该单点」——生产判据若退回弱口径立刻红，锁不靠记忆数字。
- **复刻忠实性守恒**：双臂级联复刻 `classifier::classify_impl` 的中枢级联，严格臂逐级中枢序列与生产
  `classify` **逐字段相等**为机检前提（`assert_cascade_faithful`），六级全量实测通过。
- **#323 §2.1 对账成立**：该表原为「转录自实施方自旗读数，未独立重跑复现」（§2.4）；本锁独立复算
