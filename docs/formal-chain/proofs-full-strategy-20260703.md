...
**显式前提（时序同步，阶段三双审补）**：AncOK 直接保证的是**集合成员资格**闭包（祖先必须在场）；由它推出**生命期区间**包含 `[λ_e,ρ_e]⊆[λ_a,ρ_a]` 还需一条连接前提——元素移出活动集 D_t 的时刻与其生命期右端点 ρ 严格同步（不早不晚）。若去激活滞后（结构上已过 ρ 仍留 A_t 一拍），集合闭包依然通过而区间保证静默失效——这是 Q4 修复史「提前剪」（LiveDetached 误判 Stale）的互补方向「该剪未剪」。该前提此前隐含于假设 7，现显式声明；「该剪未剪」方向的机器证明缺口经 persistent.rs I1-I5 测试清单核实为真缺（登记后续加固清单第 4 项）。
...
四项均为审计确认的前瞻性加固空间（当前代码路径下前提成立且有配套机器测试覆盖当前行为），登记不阻塞 q3 验收：
...
4. **C.2 时序同步前提的「该剪未剪」方向机器证明缺口**（gemini CONCERN 2 核实结论）：`strategy/persistent.rs` I1-I5 测试清单已逐项 Read 核实——`test_held_detached_not_stale`（`persistent.rs:435`，Q4「提前剪」核心修复看守）与 `test_explicit_close_exits_live`（`persistent.rs:497`，显式平仓退出）覆盖在案，但**无 property test 断言「结构上已过 ρ 的元素不得滞留活动集」**（去激活滞后方向）。加固方向：补该方向 property test，使 C.2 显式前提的两个失效方向（提前剪/该剪未剪）均有机器看守。
