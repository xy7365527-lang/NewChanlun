...
> T_cv = 0.01。这不是精确调参的结果，是真实数据中涌现的**自然断裂**——正例 CV ∈ [0.043, 0.078]，反例 CV = 0.00048，两个数量级的无人区使得阈值放在 0.001–0.04 之间的任何位置都给出相同的分类结果。预筛不是裁判，是成本优化：避免对显然退化的序列跑完整管线。
...
| C-2 双层判定 | [新缠论] | CV 预筛 + 结构退化检测（让语法判定自身输出） |
