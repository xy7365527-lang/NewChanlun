...
| **盘整** | 包含恰好 1 个走势中枢 |
| **趋势** | 包含 2+ 个依次同向走势中枢 |
| **上涨趋势** | C2.ZD > C1.ZG（后枢底 严格高于 前枢顶） |
| **下跌趋势** | C2.ZG < C1.ZD（后枢顶 严格低于 前枢底） |
| **区间重叠** | 不满足上涨/下跌条件 → 截断为不同 move |
...
2. 若空 → 返回 `[]`
3. 贪心向右扫描：
   - `_is_ascending(c1, c2)`: `c2.zd > c1.zg` → 上涨延续
   - `_is_descending(c1, c2)`: `c2.zg < c1.zd` → 下跌延续
   - 同向且方向一致 → 归入当前 group
   - 否则 → 截断，开始新 group
4. 每个 group → 一个 Move：
