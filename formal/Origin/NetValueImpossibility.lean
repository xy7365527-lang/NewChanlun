...
════════════════════════════════════════════════════════════════════════
## ★有效域声明（#892 S8-b，2026-08-17）

**本文件六条定理全部仅在「组合净值坐标」下成立。** 成本轨迹口径（ADR 0015）下：
父子两腿按 T₃₃ 是**同一物理量的两个视图**（`Tlayers/Accounting/Ledger.lean:78`），
「抵消」不成立——净值坐标里 (+Q)+(−Q)=0 的「净账户吃不到」结论，在成本坐标里是
「成本轨迹按持有物理单位演进」的不同陈述。**不得把本文件六条定理引作成本口径的
否定性依据**；其否定性范围 = 净值 Sharpe 验收口径，不覆盖成本轨迹验收口径。

**成本口径版（C22 成本坐标不抵消）已另证**（#893 S8-c，2026-08-18）：落点
`Origin/TotalWealth.lean` §1.6——闭合腿 `closeShareLeg profit`（profit>0）使 `cumNetCash`
（cost_basis 符号载体）严格上升，未平仓腿不产生已实现盈亏 ⟹ 双开在成本坐标下**不抵消**。
本文件六定理只证净值坐标的抵消，与成本坐标版并置、互不越界。
