/-
  ...
  **全文件 L0**（纯结构/合一/代数，零数据依赖）。`lake env lean` 通过 = 「三个已证 L0 顶层
  ...
  ★★★C42 顶点是 **L0 结构定理（分账本声部级全元素覆盖）**，**NOT L2 实盘每笔盈利**——这与
    全窗 L3（完整 v1 实盘择时 8 品种 8/8 否证、无 alpha，**净值口径产物**）**不矛盾**：
    - C42 证的是「每元素有**头寸腿覆盖**、毛收益 G^sep>0 的**代数恒等**」（方向 ε_e·ΔP_e>0 是
      元素方向语义 W7，非市场预测；s_e>0 ⟹ s_e·ε_e·ΔP_e>0 是 Int 正乘正）——**纯语法层覆盖**。
    - 全窗 L3 否证的是「这套 v1 完整择时在 1min 尺度这些品种实盘扣成本盈利」（L2/L3 经验域，净值口径下）。
    - 两者有效域不同：C42 = 分账本声部级毛收益覆盖（L0 定义域内代数成立）；L3 = 净账户实盘
      扣成本盈利（L2 经验域，净值口径下被否证）。C40/W4 净收益不可能定理是**防火墙**——它封死了把 L0
      声部级覆盖膨胀为 L2 实盘每笔盈利的有效域越界（双开净额退化 G_net=0）。
  ★#893 S8-c（2026-08-18）：上段引作对照的「全窗 L3 8/8 否证」本身是**净值口径**的产物（旧验收
    尺子）。换成本轨迹口径（ADR 0015）后，该 8 品种 L3 测量**未重跑** ⟹ C42 在成本口径下的经验
    对照是「**未测**」，不得继续拿净值口径的 8/8 否证当成本口径的边界。补这一格须重跑 8 品种
    成本口径测量（读数归 #853 参数，本票不产）。
  ...
  ★有效域诚实声明：三结论合一仅在**覆盖可行域 X^cover_Θ** 内成立（W11 `targetMem`：q̄∈K_Θ，
    目标腿全部允许存在）。
  ★#892 S8-b（2026-08-17）：本文件整体建在**净值坐标**上——C40 防火墙（`hedged_position_net_zero`）
  与 C22/C23 的「不可能」结论，其否定性只对净值 Sharpe 验收口径有效；成本轨迹口径（ADR 0015）
  下父子双开是「成本按持有物理单位演进」，本文件的净值退化论证不构成对成本口径的否定。若 q̄∉K_Θ（保证金/杠杆/资本规则不允许目标腿），风险投影改变目标腿
...
/--
  ...
  ★★★C42 结论数学名称 = **「分账本声部级全元素覆盖」**，**NOT「净资产级每笔盈利」**：
    ...
    - 与全窗 L3（v1 实盘 8/8 否证，**净值口径**）**不矛盾**：C42 = L0 分账本声部级毛收益覆盖
      （定义域内代数成立）；L3 = L2 净账户实盘扣成本盈利（经验域，净值口径下被否证）——两个
      有效域（成本口径下的 L3 对照未测，见文件头 #893 S8-c）。
  ...
  **「分账本声部级全元素覆盖」**（covered domain 内，三结论合一）。
  ...
  ★**没有** `NetProfitGuaranteed` / `NetAccountEveryStroke` / `CoverageEverywhere` 构造子——
    ...
    (2) 「全覆盖 ⟹ 实盘盈利」（覆盖是 L0 语法层；实盘盈利是 L2 经验域，全窗 L3 v1 8/8 否证
        = 净值口径产物——C42 与 L3 不矛盾，两个有效域；成本口径 L3 对照未测）；
...
/-! ════════════════════════════════════════════════════════════════════════
  ...
  - ✗ 全覆盖 ⟹ 实盘盈利（L0 语法层；实盘盈利是 L2，全窗 L3 v1 8/8 否证 = 净值口径产物——
    C42 与 L3 不矛盾，两个有效域：C42=L0 分账本声部级覆盖、L3=L2 净账户实盘扣成本盈利；
    成本口径下的 L3 对照未测）。
