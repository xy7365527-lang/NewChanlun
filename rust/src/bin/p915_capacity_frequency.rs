...
//!
//! ### 日波动率与日成交额（#1013 起走 gated loader 净化序列）
//! #1013 MED-1（影子评审坐实）前本相自带原始 JSON 读取 + 旧判据（无 #922 坏 tick 闸）——
//! 实测 BRN `2024-02-19` 的 L=0.080 坍缩 bar 漏进日聚合、σ_GK 被毒化成 **491.6%**（复跑
//! rms 与毒化值逐位吻合）。现改消费 `load_by_symbol` 净化序列：坏 bar 已被前收填充并标
//! `untradable`（缺 OHLC / 越界 / 非正价 / `volume<=0` / #922 坏 tick 闸），本相跳过
//! `untradable` bar 不计入日聚合（计数上报），统计层自此不见坏值。价格 = 量化整数 ×
//! `tick_size`（与生产同口径）；`volume` 取 `i64→f64`（#919 落主线前 volume 仍 i64，
//! 探针聚合可承受该截断）。
//!
