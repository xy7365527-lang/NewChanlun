...
//!
//! ## 耗时特征（#411 实测曲线，2026-08-18，release，M 系本机；窗口列带 THETA=0.0，全量行无窗口）
//!
//! | 数据范围 | bar 数 | 耗时 |
//! |---|---|---|
//! | 1 天 | 631 | 0.08s |
...
//! | 全量 | 343,282 | ~250s |
//!
//! 口径对照：#411 原始票面（单日 ~1440 bars 0.09s、24 天 ~34500 bars 0.3s）是 #412 修复前
//! 的估算/旧数据口径；本表为修复后实测（数据版本与 #923 一致）。
//!
//! **超线性**（46K→343K bar 约 ×7.4，耗时 ×57）：前缀因果塔逐 bar 分类为主因，
//! 非参数解析问题（#412 修复后窗口化已恢复正常）。本 bin 属研究性诊断工具——
//! **建议切窗口跑（≤90 天秒级），全量 ~4 分钟仅用于全史因果回放类重型验证**；
//! 若需优化超线性来源，另开票定位（#411 票面「先测再决定」，本处采文档化）。
