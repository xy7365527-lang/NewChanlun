//! GUARD-ROLE: toplevel-loose-files-adjunct——名分：**对照件**（非五态自动归批，#764
//! C7-E5 单列核定；不随 `lib.rs` 头部 GUARD-ROLE 块「现役」批标）。判据：全仓零非测试
//! 调用者（仅 `orchestrator.rs:470/477` 注释点名，非代码引用）；本文件自身 4 个
//! `#[test]`（2 长测试运行期跳过、2 常规跑），机械套「深零调用者⟹deprecated/孤儿」判据
//! 会与其被生产代码注释显式点名「保留为差分对照」的事实矛盾，故单列「对照件」——沿用
//! 名分表原判语义（本票复核未推翻，仅原判「顶层散件整体降对照件」的批处置不适用于本文件）。
//! 处置：留档零删除，零移入 `legacy/`。
//!
