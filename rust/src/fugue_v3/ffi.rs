...
impl PyFugueV3Stream {
    ...
    fn new(floor_ladder: usize) -> PyResult<Self> {
        let core = FugueEngineCore::new(floor_ladder).map_err(pyo3::exceptions::PyValueError::new_err)?;
        Ok(Self { core })
    }
...
pub fn run_fugue_v3(py: Python<'_>, bars: Vec<BarTuple>, floor_ladder: usize) -> PyResult<PyObject> {
    let mut core = FugueEngineCore::new(floor_ladder).map_err(pyo3::exceptions::PyValueError::new_err)?;
