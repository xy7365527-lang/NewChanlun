...
pub use types::{
    Direction, PerfectionMode, RecursiveTree, TLevelOutput, TrendKind, TrendType, Unit, Zhongshu,
    BSPKind, BSP,
};
...
pub fn iterate(a0: Vec<Unit>, mode: PerfectionMode) -> RecursiveTree {
    ...
    loop {
        let out = apply_t(&current, k, mode);
...
mod tests {
    ...
    fn iterate_空输入不panic() {
        let tree = iterate(Vec::new(), PerfectionMode::Structural);
    ...
    fn judge_divergence_空中枢不panic() {
        ...
        assert_eq!(
            judge_divergence(&bad, PerfectionMode::Structural),
            None,
            "空中枢应返回 None 不 panic"
        );
    }
