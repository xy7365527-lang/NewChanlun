...
//!
//! - **名分**：**现役**——本文件的 `apply_t` 在 `rec_stream.rs:644,688` 生产路径
//!   被直接调用（`super::apply_t(&current, k, self.mode)`），是同目录 recursive_t/(b)
//!   T 引擎（12062 行，E3 处置范围）的编译期硬依赖，详细证据/理由见 `center.rs`
//!   头部同名 GUARD-ROLE 块（避免重复，此处不复述）。
