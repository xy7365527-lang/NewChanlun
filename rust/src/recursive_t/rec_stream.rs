        ...
        if sc > self.last_stroke_n {
            ...
            if trig != self.last_trigger {
                ...
                let (
                    ...
                    //   d_top[k] = 级别 k 走势真顶/真底（区间套链向下钻取，停于成本门 ∧ 塔底先到者——塔底 =
                    //   a₀ = L0，成本门据探针 #907 不咬现役级别 ⟹ 先到塔底 ⟹ 阶段一下钻到 0 = 走势完成真顶）。
                        ...
                        for k in 0..MAX_LEVEL {
                                ...
                                tree.levels.get(k).and_then(|lvl| lvl.trends.last())
                            {
                                dt[k] = crate::recursive_t::divergence::d_top(
                                    ...
                                    // 下钻停止级 = 成本门 ∧ 塔底先到者（#817 N-2 判据 + 参数分离）：
                                    // 塔底 = a₀ = 0；成本门据探针 #907 不咬现役级别 ⟹ 先到的是塔底 ⟹ 阶段一 = 0。
                                    0,
                                );
                            }
                        }
