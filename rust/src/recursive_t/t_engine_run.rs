...
fn t_engine_8x3() {
    ...
    for (sym, file) in SYMBOLS {
        ...
        eprintln!(
            "[{sym}] bars={n_bars} ({:.1}s) → 三模式操作层引擎回测…",
            ...
            let lvl_pnl: Vec<String> = (BASE_LADDER..MAX_LADDER)
                ...
                .map(|k| {
                    format!(
                        "L{}={:+.0}",
                        k - BASE_LADDER,
                        res.mobile_realized_pnl_by_ladder[k]
                    )
                })
                .collect();
            if !lvl_pnl.is_empty() {
...
fn t_level_structure_dump() {
    ...
    eprintln!(
        "[{sym}] confirmed&&settled 段(a0)={} 单元，iterate(structural)…",
        a0.len()
    );
...
fn t_downtrend_diagnosis() {
    ...
    for lvl_idx in [3usize, 2, 1] {
        ...
        for (i, t) in lvl.trends.iter().enumerate() {
            ...
            if nc < 2 {
                eprintln!(
                    "  #{i} {:?} 中枢={nc} → 【条件1 FAIL: 中枢<2】实际={actual:?}",
                    t.kind
                );
                continue;
            }
...
fn t_bsp_op_diagnosis() {
    ...
    for (sym, file) in SYMBOLS {
        ...
        let tot =
            (d.n_vacuum_bars + d.n_long_only_bars + d.n_short_only_bars + d.n_both_bars).max(1);
        let pp = |x: u64| 100.0 * x as f64 / tot as f64;
        eprintln!("\n╔══════ 真空期/敞口诊断（绝对值敞口? 零真空?）══════╗");
        eprintln!(
            "  完全无敞口(真空)   : {:>9} bar ({:>5.2}%)  ★绝对值敞口要求=0",
            d.n_vacuum_bars,
            pp(d.n_vacuum_bars)
        );
        eprintln!(
            "  仅多头敞口         : {:>9} bar ({:>5.2}%)",
            d.n_long_only_bars,
            pp(d.n_long_only_bars)
        );
        eprintln!(
            "  仅空头敞口         : {:>9} bar ({:>5.2}%)",
            d.n_short_only_bars,
            pp(d.n_short_only_bars)
        );
        eprintln!(
            "  多空同时敞口       : {:>9} bar ({:>5.2}%)  ←sink短差(核心多+子级空对冲)",
            d.n_both_bars,
            pp(d.n_both_bars)
        );
        let span = if d.first_active_bar >= 0 && d.last_active_bar >= 0 {
            d.last_active_bar - d.first_active_bar + 1
        } else {
            0
        };
        eprintln!(
            "  首次建仓bar={} 末次有敞口bar={} 跨度={} bar",
            d.first_active_bar, d.last_active_bar, span
        );
        eprintln!(
            "  首仓后最长连续真空 = {} bar ({:.2}% 跨度)  {}",
            d.max_vacuum_gap,
            if span > 0 {
                100.0 * d.max_vacuum_gap as f64 / span as f64
            } else {
                0.0
            },
            if d.max_vacuum_gap > (span / 100).max(1) {
                "★存在长真空=执行断链"
            } else {
                "无显著真空"
            }
        ...
        eprintln!(
            "  sink   (子级卖点→父多真减仓1/3+下放做空=正确短差) : {:>7} ({:>5.1}%)",
            d.sell_sink,
            pct(d.sell_sink, sells)
        );
        eprintln!(
            "  drain  (子级卖点→减暴露1/3不翻转)              : {:>7} ({:>5.1}%)",
            d.sell_drain,
            pct(d.sell_drain, sells)
        );
        eprintln!(
            "  recover(子级卖点→父空平空头短差升回)          : {:>7} ({:>5.1}%)",
            d.sell_recover,
            pct(d.sell_recover, sells)
        );
        eprintln!(
            "  flip   (核心卖点→清全塔+翻空) ★过度减仓/翻空   : {:>7} ({:>5.1}%)",
            d.sell_flip,
            pct(d.sell_flip, sells)
        );
        eprintln!(
            "  ascend (核心持空,更高卖点骑乘)                : {:>7} ({:>5.1}%)",
            d.sell_ascend,
            pct(d.sell_ascend, sells)
        );
        eprintln!(
            "  enter  (全空卖点→首次建空)                    : {:>7} ({:>5.1}%)",
            d.sell_enter,
            pct(d.sell_enter, sells)
        );
        eprintln!(
            "  noop   (卖点不动)                            : {:>7} ({:>5.1}%)",
            d.sell_noop,
            pct(d.sell_noop, sells)
        );
        eprintln!("\n── Q2: 核心层(最高活跃ladder)units 变化 ──");
        eprintln!(
            "  sink 父级=核心层 (核心被短差直接减1/3) : {} 次, 累计减 {:.4} units",
            d.n_sink_on_core, d.sink_core_reduced_units
        );
        eprintln!(
            "  flip 翻空清掉核心多头 (清全塔)         : {} 次, 累计清 {:.4} 多头units",
            d.flip_from_long, d.flip_long_units_cleared
        );
        eprintln!(
            "  ⟹ 卖点对核心仓: 减1/3({}次sink-on-core) vs 全清({}次flip-from-long)",
            d.n_sink_on_core, d.flip_from_long
        );
        eprintln!("\n── Q3: 翻空是否在强牛中做空亏钱 ──");
        eprintln!(
            "  卖点翻空(原核心持多, 强牛清多翻空) : {} 次",
        ...
        eprintln!(
            "  短差/做空腿累计 realized pnl (short_leg_pnl) : {:+.0}  ⟸ <0 = 做空净亏",
            res.short_leg_pnl
        );
        eprintln!(
            "  物理在场 bar: 多头={} 空头={} (空/多比={:.2})",
            res.phys_long_bars,
            res.phys_short_bars,
            if res.phys_long_bars > 0 {
                res.phys_short_bars as f64 / res.phys_long_bars as f64
            } else {
                0.0
            }
        );
        eprintln!("\n── 对照: 买点信号({buys}个) ──");
        eprintln!(
            "  sink={} drain={} recover={} flip={} ascend={} enter={} noop={}",
            d.buy_sink,
            d.buy_drain,
            d.buy_recover,
            d.buy_flip,
        ...
        let avg_iv = if d.recover_interval_count > 0 {
            d.recover_interval_sum / d.recover_interval_count as f64
        } else {
            0.0
        };
        eprintln!(
            "  recover 次数={} 平均持空={:.0} bar 最长持空={} bar",
            d.recover_interval_count, avg_iv, d.recover_interval_max
        );

        eprintln!("\n── Q2: 信号层密度（买点是否缺失，per ladder）──");
        eprintln!(
            "  {:>4} {:>9} {:>9} {:>7} | {:>9} {:>9} {:>9}",
            "lad", "卖信号", "买信号", "买/卖", "sink开空", "recover平", "残留空"
        );
        for k in BASE_LADDER..MAX_LADDER {
        ...
        eprintln!("\n── Q3: 平空后有没有重新做多 ──");
        eprintln!(
            "  buy_noop_no_short (买点触发但无空可平=平了空就停/不做多回来) : {}",
            d.buy_noop_no_short
        );
        eprintln!(
            "  recover (平空头同时升回父级=做多回来, 全量) : 卖侧{} 买侧{} 合计{}",
            d.sell_recover,
            d.buy_recover,
            d.sell_recover + d.buy_recover
        );
        eprintln!(
            "  ⟹ sink开空合计{} vs recover平空合计{} ⟹ 平空率={:.1}% (缺口={}个空未买回)",
            ...
            if res.n_cycle_opens_by_ladder.iter().sum::<u64>() > 0 {
                100.0 * res.n_cycle_closes_by_ladder.iter().sum::<u64>() as f64
                    / res.n_cycle_opens_by_ladder.iter().sum::<u64>() as f64
            } else {
                0.0
            },
            res.n_cycle_opens_by_ladder.iter().sum::<u64>() as i64
                - res.n_cycle_closes_by_ladder.iter().sum::<u64>() as i64
        );

        eprintln!("\n── Q4: 核心仓 units 在「卖→空→平→多」周期后是否恢复 ──");
        eprintln!(
            "  核心被 sink 减 (累计)   : {:.2} units ({} 次)",
            d.sink_core_reduced_units, d.n_sink_on_core
        );
        eprintln!(
            "  核心被 recover 升回(累计): {:.2} units ({} 次)",
            d.recover_core_restored_units, d.n_recover_on_core
        );
        let recov_ratio = if d.sink_core_reduced_units > 1e-9 {
            100.0 * d.recover_core_restored_units / d.sink_core_reduced_units
        } else {
            0.0
        };
        eprintln!(
        ...
        {
            use crate::trading::types::Polarity;
            let shorts: Vec<&_> = res
                .trades
                .iter()
                .filter(|t| t.polarity == Polarity::Short)
                .collect();
            let spnl: Vec<f64> = shorts
                .iter()
                .map(|t| t.shares * (t.entry_price - t.exit_price))
                .collect();
            ...
            let n_higher = shorts
                .iter()
                .filter(|t| t.exit_price > t.entry_price + 1e-9)
                .count();
            ...
            eprintln!(
                "  ★平空价>开空价(价格涨回来才平=做空亏): {} 笔 ({:.1}%)",
                n_higher,
                100.0 * n_higher as f64 / shorts.len().max(1) as f64
            );
            for r in ["recover", "eod", "liq", "flip", "clear"] {
                let (n, p) = by_reason(r);
                if n > 0 {
                    eprintln!("    平仓原因 {:>8}: {:>5}笔 pnl={:+.0}", r, n, p);
                }
            }
            ...
            eprintln!(
                "  ── 亏损最大10笔: 开空@→平空@(涨跌%)|期间最低@|本可平@底盈亏|{}bar|原因 ──",
                "持仓"
            );
            for &i in idx.iter().take(10) {
                let t = shorts[i];
                let (eb, xb) = (t.entry_bar.max(0) as usize, t.exit_bar.max(0) as usize);
                let lo = if eb <= xb && xb < c.len() {
                    c[eb..=xb].iter().cloned().fold(f64::MAX, f64::min)
                } else {
                    f64::NAN
                };
                let chg = if t.entry_price > 0.0 {
                    (t.exit_price / t.entry_price - 1.0) * 100.0
                } else {
                    0.0
                };
        ...
        eprintln!(
            "  sink次数/级别(n_cycle_opens): {:?}",
            &res.n_cycle_opens_by_ladder[BASE_LADDER..]
        );
        eprintln!(
            "  recover次数/级别(n_cycle_closes): {:?}",
            &res.n_cycle_closes_by_ladder[BASE_LADDER..]
        );
        eprintln!(
            "  强平次数/级别(n_liquidations): {:?}",
            &res.n_liquidations_by_ladder[BASE_LADDER..]
        );
        eprintln!("================================================================\n");
    }
}
