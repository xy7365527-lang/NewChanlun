...
impl NestChainGate {
    ...
    pub(super) fn chain_lookup(
        ...
        // 仅未确认事件（② 谓词不成立，`unconf_exact > 0`）⟹ 自相矛盾态。判据方向 =
        // 区间套「从大级别往下，用逐级的背驰段收缩范围，把转折点定位准」
        // （qujiantao.md §2，`027:44` 定理原文，`[旧缠论]`）；「高级别背驰 ⟹ 低级别
        // 已背驰」的一般必要性读法 = `[新缠论:选择]`（beichi.md 已订正为非原文命题，
        // 见 #862 issue 5）。据此把本级升级为 Closed，消除「高级 closed + 本级未确认」
        // 的矛盾谱系。只升级 missing_cert（有存在但
