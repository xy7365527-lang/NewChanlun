...
/// **本值单调但非度量** —— 差值不是时间间隔（`23:59:59`→次日 `00:00:00` 差 **1**，
/// 而 `23:00:00`→`23:01:00` 差 **100**）。
///
/// **#935 裁定（2026-08-07，map #931）：结构层不引入时间。** 依据是缠论基础判据里
/// 一个时间量都没有（`018:24` 中枢＝纯价格区间 max/min、`065:24` 包含＝纯高低比较、
/// `077:38` 笔的间隔**数 K 根数**），以及 `075:137`【答疑】逐字「跳空不改变任何事情，
/// 如果这缺口在线段里，还是线段；在笔里，还是笔」。⟹ 结构判据**不消费本值做度量**，
/// 故本编码**无需改**，`qujiantao.md` 残余风险第 25 条的结构层那一半按此免测。
///
/// ⚠️ **若将来有代码要拿它算真实时间间隔，必须先改编码** —— 且本函数在仓内有
/// **31 份各自复制**（检索式 `grep -rn "fn date_to_timestamp" rust/ | grep -v target`，
/// 2026-08-07 实测：本处 1 份 + `rust/src/bin/` 下 30 份），**改一处不够**。
