...
//!   （引擎不区分标的，标的无特化纪律）。**触发前提已成立**（#303 裁定本仓 venue = spot，
//!   #340 补登）：本文件代码无需改（venue 分叉本已正确），但该 gate 自此是**待落实的部署层
//!   要求**而非假想分支；牵连面登记见
//!   [`CostModel`](super::super::strategy::risk::CostModel) 节头，落实归 #62 / 部署层清单。
