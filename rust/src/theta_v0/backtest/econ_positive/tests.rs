...
fn l2_btc_capturable_spread_diagnosis() {
    ...
    let _ = writeln!(
        rpt,
        "- β^div force fill-rate：{}/{}（一类信号 {} 条；断言=有一类则 force 非全 None，#115 (e)）",
        n_force_some, agg.n_signals, n_type1
    );
    let _ = writeln!(rpt);
    ...
    let _ = writeln!(
        rpt,
        "涌现最高级别 L={max_level}（全级别 0..{max_level} 均列；稀疏高级别照报不判硬墙，663）。"
    );
    let _ = writeln!(
        rpt,
...
fn acc_level0sell_oos() {
    ...
    let _ = writeln!(
        rpt,
        "| 1（前半） | {w1_ret:+.2}% | {w1_sell:.4e} | {w1_s_n} | {w1_buy:.4e} | {w1_b_n} | {} |",
        if sell_beats_buy_w1 { "是" } else { "否" }
    );
    let _ = writeln!(
        rpt,
        "| 2（后半） | {w2_ret:+.2}% | {w2_sell:.4e} | {w2_s_n} | {w2_buy:.4e} | {w2_b_n} | {} |",
        if sell_beats_buy_w2 { "是" } else { "否" }
    );
    let _ = writeln!(rpt);
    if asym_structural {
...
fn bucket_key_is_full_z_l1() {
    ...
    let mk =
        |bsp_class: u8, parent_dir: i8, position: PositionState, actual_pnl: f64| SignalDecomp {
            entry_bar: 0,
            exit_bar: 1,
            level: 0,
            delta: 1,
...
fn acc_classification_level_hole_dx() {
    ...
    let overlap_probe_enabled = std::env::var(crate::theta_v0::env_registry::ECON_C3_OVERLAP_PROBE)
        .ok()
        .as_deref()
        == Some("1");
    ...
    if n_l1 == 0 {
        ...
        let _ = writeln!(
            rpt,
            "- c3_new_center_exists={} ({:.2}%)：source_index~confirm_index 间存在新确认次级中枢",
            xzd_l1_c3_new_center_exists,
            pct(xzd_l1_c3_new_center_exists)
        );
        ...
        if overlap_probe_enabled {
            ...
            let _ = writeln!(
                probe_rpt,
                "- 存在 ≥1 个滑动重叠窗口 post-source 新中枢的信号数={overlap_new_center_signals}"
            );
    ...
    eprintln!(
        ...
        type1_forced,
        if type1_sigs > 0 {
            100.0 * type1_forced as f64 / type1_sigs as f64
        } else {
            0.0
        }
    );
...
fn acc_bottomup_nest_parity_probe() {
    ...
    let win_end = ds
        ...
        .unwrap_or_default();
    eprintln!(
        "[bottomup-parity] bars={n}（{win_start}→{win_end}，全量={n_full}），max_bars={max_bars}"
    );
            ...
            for p in ls.bsp.iter() {
                ...
                for c in &assemble_gamma_with_tower(&single, &tower_i) {
                    ...
                    if let (Some(pc), Some(bc)) = (&prod, &bu) {
                        ...
                        let struct_diff =
                            pc.base_interval() != bc.base_interval()
...
fn acc_nest_trigger_quality_probe() {
    ...
    eprintln!(
        "过滤后（仅 Type1 存活）= {t1_n}；被一票否决 = {vetoed_n}（Type23 {t23_n} + Xzd {xzd_n}）"
    );
...
fn h2_sample_exclusion_dx() {
    ...
    let win_end = ds
        ...
        .unwrap_or_default();
    eprintln!(
        "[h2-exclusion-dx] bars={n}（{win_start}→{win_end}，全量={n_full}），max_bars={max_bars}"
    );
...
fn l2_depth_distribution_dx() {
    ...
    let _ = writeln!(
        rpt,
        "- 诚实标注：生产门 descent 触发域=`!is_type1 && lvl>=1`，比 Type2/3 主群宽 {n_other} 条；"
    );
    ...
    eprintln!(
        "真封：Type2/3 主群={tot_t23}=base_none{tot_base_none}+小转大{tot_xzd}+有锚{tot_some}；\
...
// ═══════════════════════════════════════════════════════════════════════
// #846 探针：Type1 下钻逐级连续性（ADR 0013 裁定七实测）
// ═══════════════════════════════════════════════════════════════════════

/// [`descend_type1_anchor_depth`] 单步失败成因（#846 三分）。
///
...
impl StepFail {
    /// 三分桶序（0=A / 1=B / 2=C），用于计数数组下标。
    fn bucket(self) -> usize {
        match self {
            StepFail::EmptySubs => 0,
            StepFail::NoAlign => 1,
            StepFail::DivFalse(_) => 2,
        }
    }
}
...
fn legacy_div_cand_why(
    ...
    use super::super::super::classifier::cand_predicate::rmove_dir;
    use super::super::super::classifier::divergence::{is_divergence, segment_macd_area};
    ...
    let extreme_ok = match delta {
        Side::Long => s.rmove.lo() < s_prev.rmove.lo(),
        Side::Short => s.rmove.hi() > s_prev.rmove.hi(),
    };
    if !extreme_ok {
        return Some(3);
    }
...
/// **#846 探针：Type1 一类点区间套下钻的逐级连续性**（只读诊断，不改生产，`#[ignore]`）。
///
/// ## 验什么
///
/// ADR 0013（#839）裁定七：「某级别一类点 = 该级别走势的**结束** ⟹ 构成它的最后一个次级别
/// 走势也结束 ⟹ 那必然是次级别的一类点（趋势背驰）或类一类点（盘整背驰）。**逐级无空档。**」
///
/// 本仓从未测过。旧两组实测（`9bd0a518c9:.chanlun/review-results/l2-depth-distribution-20260702.md`
/// / `b06e5cf10d:.chanlun/review-results/l2-depth-raw-20260702.md`）只测 Type2/3（走势**中途**
/// 的位置），且只报「小转大 89.79%–91.85%」这个**合计数**——`None` 的三种成因未分。
///
/// ## 怎么测
///
/// 复用 `l2_depth_distribution_dx` 的 bit-exact classify 循环（同一批 bar / 同一 Γ 定向 / 同一
/// 塔），把它 `:7299-7301` 的 `if is_type1 { continue }` **反过来**选出 Type1 群，同时保留
...
///
/// 命令：`ECON_L2_MAX_BARS=100000000 cargo test --release type1_descend_continuity_dx -- --ignored --nocapture`
#[test]
#[ignore]
fn type1_descend_continuity_dx() {
    ...
    let win_end = ds
        ...
        .unwrap_or_default();
    eprintln!(
        "[t1-descend-dx] bars={n}（{win_start}→{win_end}，全量={n_full}），max_bars={max_bars}"
    );

    let closes: Vec<f64> = bars.iter().map(|b| b.close as f64 / tick as f64).collect();
    let macd_hist = compute_macd(&closes, &config.macd).hist;

    // ★LMIN=0（与 l2_depth_distribution_dx 的 LMIN=1 差异）：一类点在塔里落哪一级是**待测量**，
    //   不是已知——把 level0 也收进来，否则「Type1=0 条」分不清是「没有」还是「没收」。
    //   lvl≥1 各行仍与旧两组数据同口径可比。
    const LMIN: usize = 0;
    const LMAX: usize = 4;
    const DCAP: usize = 8;
    const NG: usize = 2; // 0 = Type1 群；1 = Type2/3 群（对照）
    const NR: usize = 3; // 终止成因三桶：0=A(EmptySubs) 1=B(NoAlign) 2=C(DivFalse)
    let gname = |g: usize| if g == 0 { "Type1" } else { "Type2/3" };
    let in_scope = |lvl: usize| (LMIN..=LMAX).contains(&lvl);

    // ── BSP bit 清单（Γ 定向**之前**的原始读数，逐级）：用于分辨「群为空」的成因 ──
    let mut raw_pts = [0usize; LMAX + 1]; // 去重后的 BSP 点数
    let mut raw_bit = [[0usize; 6]; LMAX + 1]; // buy1,buy2,buy3,sell1,sell2,sell3 各自置位点数
    let mut raw_t1_pts = [0usize; LMAX + 1]; // 带 buy1|sell1 的点数
    let mut gamma_none = [0usize; LMAX + 1]; // Γ 未产出任何非 Flat 候选的点数
    let mut t1_gamma_none = [0usize; LMAX + 1]; // 带 type1 bit 但 Γ 无方向
    let mut t1_dir_mismatch = [0usize; LMAX + 1]; // 带 type1 bit、Γ 有方向但该方向无 type1 bit
    ...
    let mut term_by_abs = [[[0usize; LMAX + 1]; NR]; NG]; // 链终止成因 × 终止绝对级别 L
    let mut step_ok = [[[0usize; LMAX + 1]; LMAX + 1]; NG]; // [群][信号级][L] L→L−1 成功步数
    let mut step_fail = [[[[0usize; NR]; LMAX + 1]; LMAX + 1]; NG]; // [群][信号级][L][成因]
    let mut cond_fail = [[0usize; 5]; NG]; // C 的条件号分布（下标 0..=4）
    let mut cond_fail_abs = [[[0usize; 5]; LMAX + 1]; NG]; // C 条件号 × 绝对级别
    ...
    // ── 断链现场取样 ──
    const PER_CELL_CAP: usize = 3; // 每 (群,成因,绝对级别) 最多留 3 例
    ...
    for i in 0..n {
        ...
        for (lvl, ls) in cls_i.levels.iter().enumerate() {
            ...
            for p in ls.bsp.iter() {
                ...
                // 预过滤：至少带一个 bsp bit（无 bit 信号两群皆非，省 Γ 组装）。
                // ★与 l2_depth_distribution_dx:7264 的差异：那里只收 type2/3 bit，这里放宽到
                //   含 type1 bit ⟹ Type1 群进入采集域；Type2/3 群的采集条件**未变**。
                if !(p.bits.buy1
                    || p.bits.sell1
                    || p.bits.buy2
                    || p.bits.sell2
                ...
                let has_t1_bit = p.bits.buy1 || p.bits.sell1;
                if has_t1_bit {
                    raw_t1_pts[lvl] += 1;
                }
                // Γ 组装（bit-exact 复制生产路径）取交易方向 δ。
                ...
                let n_dir = cands
                    .iter()
                    .filter(|c| !matches!(c.dir, VoiceSide::Flat))
                    .count();
                if n_dir == 0 {
                    gamma_none[lvl] += 1;
                    if has_t1_bit {
                    ...
                    && !cands.iter().any(|c| match c.dir {
                        VoiceSide::Long => p.bits.buy1,
                        VoiceSide::Short => p.bits.sell1,
                        VoiceSide::Flat => false,
                    })
                {
                    t1_dir_mismatch[lvl] += 1;
                }
                for c in &cands {
                    let delta = match c.dir {
                        VoiceSide::Long => Side::Long,
                    ...
                    // ★群定义：Type1 优先（与 bsp_cand_type 的 Type1>Type2>Type3 坍缩同序）。
                    let g = if is_type1 {
                        0
                    ...
                    // ── 展开 descend_type1_anchor_depth 的递归为可观测循环 ──
                            ...
                            None => match find_move_containing_index(subs, src) {
                                Some(tidx) => {
                                    ...
                                    if sample_f.len() < 54 {
                                        ...
                                        let kind_txt = if matches!(&sub.rmove, RM2::Segment { .. })
                                        {
                                            "Segment"
                                        } else {
                                            "Compose"
                    ...
                    let rb = terminal.bucket();
                    term_by_lvl[g][lvl][rb] += 1;
                    term_by_abs[g][rb][cur_level.min(LMAX)] += 1;
                    step_fail[g][lvl][cur_level.min(LMAX)][rb] += 1;
                    if depth == 0 {
                        top_none[g][lvl][rb] += 1;
                    }
                    max_depth[g] = max_depth[g].max(depth);
                    if depth > DCAP {
                        depth_overflow += 1;
                    }
                    depth_hist[g][lvl][depth.min(DCAP)] += 1;
                    if let StepFail::DivFalse(cond) = terminal {
                        cond_fail[g][cond as usize] += 1;
                        cond_fail_abs[g][cur_level.min(LMAX)][cond as usize] += 1;
                    }

                    // ── 断链现场取样（B / C）──
                    let want_sample = matches!(terminal, StepFail::NoAlign | StepFail::DivFalse(_))
                    ...
                    if want_sample {
                        ...
                        match terminal {
                            StepFail::NoAlign => {
                                // 该级实际结构：段数、各段 end_index、目标 src、最近端点差。
                                let ends: Vec<String> =
                                    subs.iter().map(|m| m.end_index.to_string()).collect();
                                let nearest = subs
                                    .iter()
                                    .map(|m| ((m.end_index as i64 - src as i64).abs(), m.end_index))
                                    .min();
                            ...
                            StepFail::EmptySubs => unreachable!("已由 want_sample 排除"),
                        }
                    }
                }
            }
    ...
    let _ = writeln!(rpt, "- 群定义（per-delta，Type1 优先坍缩）：Type1 = `buy1|sell1` 置位；Type2/3 = 非 Type1 且 `buy2|sell2|buy3|sell3` 置位");
    let _ = writeln!(rpt, "- 判据口径：`cand_predicate::div_cand` 四条件（1 方向 / 2 前序同向段 / 3 Extreme / 4 Weak=MACD 段面积严格减）");
    let _ = writeln!(rpt, "- 三分：**A**=`sub_moves` 空（递归底=L0，基底选择，**不算断链**，#520）；**B**=无 `end_index==src` 对齐段；**C**=对齐但 `div_cand` 假");
    let _ = writeln!(
        rpt,
    ...
    // 0. BSP bit 清单（Γ 之前）——分辨「群为空」的成因
    let _ = writeln!(rpt, "## 0. BSP bit 清单（Γ 定向**之前**的原始读数）");
    let _ = writeln!(rpt);
    let _ = writeln!(rpt, "| lvl | 去重BSP点数 | buy1 | buy2 | buy3 | sell1 | sell2 | sell3 | 带type1 bit | 其中Γ无方向 | 其中Γ方向对不上 |");
    let _ = writeln!(rpt, "|---|---|---|---|---|---|---|---|---|---|---|");
    for lvl in LMIN..=LMAX {
        let _ = writeln!(
            rpt,
    ...
    let _ = writeln!(
        rpt,
        "\n- Γ 完全无方向（全 Flat）的点数逐级：{:?}",
        &gamma_none[LMIN..=LMAX]
    );
    let _ = writeln!(rpt, "- 判读：`带type1 bit` = 0 ⟹ 该级根本不产一类点（塔/识别层事实）；\
            `其中Γ无方向`/`方向对不上` > 0 ⟹ 一类点存在但被 Γ 定向环节滤掉（探针域问题，非命题否证）。");
    let _ = writeln!(rpt);

    // 1. 逐级命中率
    let _ = writeln!(rpt, "## 1. 逐级命中率（每步 成功/A/B/C）");
    let _ = writeln!(rpt);
    for g in 0..NG {
        let _ = writeln!(rpt, "### 1.{} {} 群", g + 1, gname(g));
        let _ = writeln!(rpt, "| 信号级 lvl | 步 L→L−1 | 到达该步 | 成功 | A(空subs) | B(无对齐) | C(div假) | 成功% |");
        let _ = writeln!(rpt, "|---|---|---|---|---|---|---|---|");
        for lvl in LMIN..=LMAX {
            for l in (0..=lvl).rev() {
                let ok = step_ok[g][lvl][l];
                let fa = step_fail[g][lvl][l][0];
                let fb = step_fail[g][lvl][l][1];
                let fc = step_fail[g][lvl][l][2];
                let reach = ok + fa + fb + fc;
                if reach == 0 {
                    continue;
                }
                let _ = writeln!(
                    rpt,
                    "| {lvl} | {l}→{} | {reach} | {ok} | {fa} | {fb} | {fc} | {:.2}% |",
                    l.saturating_sub(1),
                    pct(ok, reach)
                );
            }
    ...
    for g in 0..NG {
        ...
        for lvl in LMIN..=LMAX {
            ...
            let none_tot = a + b + cc;
            let some_l = n_sig[g][lvl] - base_none[g][lvl] - none_tot;
            let _ = writeln!(
                rpt,
                "| {} | {lvl} | {} | {} | {none_tot} | {a} | {b} | {cc} | {some_l} | {:.2}% |",
                gname(g),
                n_sig[g][lvl],
                base_none[g][lvl],
                pct(none_tot, n_sig[g][lvl])
            );
            t_n += n_sig[g][lvl];
            t_bn += base_none[g][lvl];
            t_a += a;
            t_b += b;
            t_c += cc;
        }
        let t_none = t_a + t_b + t_c;
        let _ = writeln!(
                rpt,
                "| **{}合计** | — | **{t_n}** | **{t_bn}** | **{t_none}** | **{t_a}** | **{t_b}** | **{t_c}** | **{}** | **{:.2}%** |",
                gname(g),
                t_n - t_bn - t_none,
                pct(t_none, t_n)
            );
    ...
    let _ = writeln!(rpt);
    let _ = writeln!(rpt, "### 3.2 按绝对级别切（链在级 L 上终止）");
    let _ = writeln!(rpt, "| 群 | 绝对级 L | 终止=A | 终止=B | 终止=C |");
    let _ = writeln!(rpt, "|---|---|---|---|---|");
    for g in 0..NG {
        for l in 0..=LMAX {
            let (a, b, cc) = (
            ...
            let _ = writeln!(rpt, "| {} | {l} | {a} | {b} | {cc} |", gname(g));
        }
    }
    let _ = writeln!(rpt);
    let _ = writeln!(
    ...
    // 4. 深度分布
    let _ = writeln!(rpt, "## 4. 深度分布（成功钻了几层；depth==0 即顶层 None）");
    let _ = writeln!(rpt);
    let _ = write!(rpt, "| 群 | lvl ");
    for d in 0..=DCAP {
        let _ = write!(rpt, "| d={d} ");
    }
    let _ = writeln!(rpt, "|");
    ...
    for g in 0..NG {
        for lvl in LMIN..=LMAX {
            let _ = write!(rpt, "| {} | {lvl} ", gname(g));
            for d in 0..=DCAP {
                let _ = write!(rpt, "| {} ", depth_hist[g][lvl][d]);
            }
            let _ = writeln!(rpt, "|");
        }
    }
    let _ = writeln!(
        rpt,
        "\n- max_depth：Type1={}，Type2/3={}；depth>DCAP({DCAP}) 溢出={depth_overflow}",
    ...
    if sample_b.is_empty() {
        ...
        let _ = writeln!(
            rpt,
            "| 群 | 信号级 | src | δ | 步 | 该级段数 | 各段 end_index | 最近 end | 差 | 父段区间 |"
        );
        let _ = writeln!(rpt, "|---|---|---|---|---|---|---|---|---|---|");
        for l in &sample_b {
            let _ = writeln!(rpt, "{l}");
    ...
    if sample_c.is_empty() {
        ...
        let _ = writeln!(rpt, "| 群 | 信号级 | src | δ | 步 | 失败条件 | 该级结构 | 目标段方向 | 目标段 lo/hi/area | 前序同向段 |");
        let _ = writeln!(rpt, "|---|---|---|---|---|---|---|---|---|---|");
        for l in &sample_c {
            let _ = writeln!(rpt, "{l}");
        }
...
口径：legacy = #846 旧判据镜像（趋势形状取段 + MACD 面积 Weak，诊断对照臂）；生产 = #883 后判据（#814 D-3 统一取段 + config.divergence_gauge 力度档，默认 ForceL）。「legacy假∧生产真」即 C 桶里本该算类一类点的下钻步数（盘整入口救回）；「legacy真∧生产假」是取段收紧（震荡段除名/无中枢不判）与 ForceL 换档的反向差，照实并列。"
    ...
    let out = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("rust/ 父目录 = 项目根")
        .join(".chanlun/review-results/issue846-type1-descend-continuity-raw.md");
    std::fs::write(&out, &rpt).unwrap_or_else(|e| panic!("写报告失败：{e}"));
    eprintln!("\n原始数据已落盘：{out:?}");
    ...
    for g in 0..NG {
        for lvl in LMIN..=LMAX {
            ...
            let term_sum: usize = term_by_lvl[g][lvl].iter().sum();
            assert_eq!(
                term_sum,
                n_sig[g][lvl] - base_none[g][lvl],
                "{} lvl{lvl} 终止成因穷举：ΣA/B/C({term_sum}) 应 = 启动下钻的链数",
                gname(g)
            );
        }
    }
    ...
    for g in 0..NG {
        for lvl in LMIN..=LMAX {
            assert_eq!(
                both_pass[g][lvl] + both_fail[g][lvl] + pan_rescue[g][lvl] + reverse_diff[g][lvl],
                n_eval[g][lvl],
                ...
                for c in &cands {
                        ...
                        && sample.len() < SAMPLE_CAP
                    {
                        ...
                        let fmt_arm = |a: usize| match targets[a] {
                            Some(tidx) => {
                                format!("idx={tidx} dir={}", fmt_dir(rmove_dir(&subs[tidx].rmove)))
                            }
                            None => "（无对齐）".to_string(),
                        };
                        sample.push(format!(
            ...
            for p in ls.bsp.iter() {
                ...
                for c in &cands {
                    ...
                    let (s_crosses, prev_info, prev_idx, prev_lo, prev_hi, prev_area) =
                        match (s_dir, parent_center) {
                            (Some(dir), Some(c)) => {
                                ...
                                let prev = subs[..tidx].iter().enumerate().rev().find(|(_, m)| {
                                    rmove_dir(&m.rmove) == Some(dir)
                                        && (crosses(m, dir, c) || m.end_index <= c.start_index)
                                });
                                match prev {
                                    Some((idx, m)) => {
                    ...
                    let lb = prev_idx.and_then(|idx| {
                        segment_force_l(&l0_i.strokes, subs[idx].start_index, subs[idx].end_index)
                    });
                    ...
                    // md 取样。
                    if bucket_used[cond as usize] < PER_BUCKET_CAP && sample_md.len() < TOTAL_CAP {
                ...
                for c in &cands {
                        ...
                        .or_else(|| find_move_containing_index(subs, src))
                    {
                        let why = prod_div_cand_why(
                            ...
                            delta,
                            &l0_i.strokes,
                            super::super::super::classifier::cand_predicate::parent_last_center(s),
                            config.divergence_gauge,
                        );
                        match why {
                            None => {
...
// ═══════════════════════════════════════════════════════════════════════
// #851 探针：#848 漏数的那一层——「次级别二/三类 bit」∧「同坐标盘整背驰」的**交集**
// ═══════════════════════════════════════════════════════════════════════

/// 某级 `pan_div` 在坐标 `src` 上的**分侧**命中：`(Long 命中, Short 命中)`。
///
/// 与 [`pan_div_at_coord`] 的区别：那个把结果折成「任意侧 / δ 同侧」两个 bool，本函数保留
/// 原始两侧——#851 明确要求分清「大 buy1 ↔ 小**同侧**盘背」与「大 buy1 ↔ 小**反侧**盘背」，
/// 折叠口径会把这两件事混数。
fn pan_div_sides_at_coord(certs: &[PanDivCert], src: usize) -> (bool, bool) {
    let mut long_hit = false;
    let mut short_hit = false;
    for c in certs {
        if c.source_index == src {
            match c.side {
                Side::Long => long_hit = true,
                Side::Short => short_hit = true,
            }
        }
    }
    (long_hit, short_hit)
...
/// **#851 探针：交集口径重数 #848 的 34 例**（只读诊断，不改生产，不改判据，`#[ignore]`）。
///
/// ## 为什么要重数
///
/// #848 §4 统计「大级别 Type1 信号坐标上，次级别标了什么」，得 34 例中一类 bit **0 个**，
/// 实际 buy2×16 / buy3×6 / sell2×10 / sell3×4，据此判「归类半句不成立」。但 #848 把
/// **BSP 层一类 bit**（§1，窄口径恒 0）与 **`pan_div` 通道**（§2，宽口径 9.09%/13.57%）
/// **分开数**，从未数过二者的**交集**。
///
/// ADR 0013 裁定七的命题写的是「次级别一类点**或类一类点（盘整背驰）**」，原文承重：
/// - `101-第101课.md:32`「第二类买点跌破第一类买点……这里**一般都构成盘整背驰**」
/// - `060-第60课.md:50`「严格来说，**盘整背驰无所谓第一类买点，只是这样来类比**」
/// - `027-第27课.md:66`「这因为盘整背驰而形成的**类第一类买点**」
///
/// ⟹ 命题要的口径是「次级别二类 bit **∧** 同坐标盘整背驰」这个**交集**，不是两个分离计数。
///
/// ## 本探针测什么（四件）
///
/// 1. **交集交叉表**：34 例逐例给出次级别 BSP bit × 同坐标 `pan_div` 命中/同侧，至少四格
///    （标二类∧有盘背 / 标二类∧无 / 标三类∧有 / 标三类∧无）。
/// 2. **同侧反侧分开数**：逐例标明大级别 δ、次级别哪个 bit、同侧还是反侧。
///    「大 buy1 ↔ 小 sell2」与「大 buy1 ↔ 小 buy2」是两件事。
/// 3. 二类点的 `Type1Anchor` 归属锚（`make_second_point` 载 `OwnerRef::Type1Anchor`）——
///    答「大级别一类点坐标落在次级别二类点上时，那个二类点自己的一类锚在哪」。
/// 4. 全 66 例（含 32 例坐标无次级别条目）的 `pan_div` 覆盖，防「只看 34 例」的选择偏差。
///
/// ## 口径（与 #848 严格同批，否则两次读数不可比）
///
/// 同一份 BTC 数据、同一 `IncrementalClassifier` 逐 bar 循环、同一 `seen` 去重键
/// `(lvl, source_index, bsp_disc)`、同一 `LMIN=0`/`LMAX=4`、同一 Γ 定向、同一群定义。
/// §0 的 `n_sig` 逐格与 `issue848-bsp-crosscheck-raw-b1200000.md` §0 对照即自检。
///
/// ## 不裁教义
///
/// 「盘整背驰算不算类一类点、几段起算」归 #817，**本探针不裁**；只测现有两个载体在同一
/// 坐标上的共现。**不改判据、不碰生产代码。**
///
/// **认识论 L2**（真实 BTC 逐信号 + 确定性判据，可产否定性计数）。
///
/// 命令：`ECON_L2_MAX_BARS=1200000 cargo test --release type1_quasi_first_intersection_dx -- --ignored --nocapture`
#[test]
#[ignore]
fn type1_quasi_first_intersection_dx() {
    use super::super::super::classifier::bsp::OwnerRef;
    use super::super::data;
    use std::fmt::Write as _;
    ...
    let win_end = ds
        ...
        .unwrap_or_default();
    eprintln!(
        "[t1-quasi-inter] bars={n}（{win_start}→{win_end}，全量={n_full}），max_bars={max_bars}"
    );
    ...
    // ── 坐标级 4×3 交叉表：[群][类别][pd 列] ──
    // 类别 0=带二类bit（任意侧） 1=不带二类但带三类bit 2=带一类bit 3=mask==0（纯结构候选）
    // pd 列 0=同侧 pan_div  1=仅反侧 pan_div  2=无 pan_div
    let mut cross_cat = [[[0usize; 3]; 4]; NG];
    // ── 同侧/反侧细分（指示计数，同坐标可多格并计）：[群][4 种][pd 列] ──
    // 0=同侧二类 1=反侧二类 2=同侧三类 3=反侧三类
    let mut cross_side = [[[0usize; 3]; 4]; NG];
    // ── 六 bit × pd 列（bit 级，与 #848 §4 的 bit 计数同口径）──
    let mut bit_x_pd = [[[0usize; 3]; 6]; NG];
    // 分母：坐标存在次级别条目的例数 / 全部 lvl≥1 例数
    let mut n_lvlge1 = [0usize; NG];
    let mut n_present = [0usize; NG];
    // 全 66 例口径：坐标**无**次级别条目时的 pan_div 覆盖
    let mut absent_pd = [[0usize; 3]; NG];
    // 终局快照复查（防「同期查不到」被写成「不存在」）
    let mut f_present = [0usize; NG];
    let mut f_pd_same = [0usize; NG];
    let mut f_pd_any = [0usize; NG];
        ...
        for (lvl, ls) in cls_i.levels.iter().enumerate() {
            ...
            for p in ls.bsp.iter() {
                ...
                for c in &cands {
                    ...
                    let at = bsp_at_coord(&tls.bsp, src);
                    let (pd_long, pd_short) = pan_div_sides_at_coord(&tls.pan_div, src);
                    let (pd_same, pd_opp) = match delta {
                        Side::Long => (pd_long, pd_short),
                        Side::Short => (pd_short, pd_long),
                    };
                    // pd 列：0=同侧 1=仅反侧 2=无
                    let pdc = if pd_same {
                        0
                    ...
                    // 坐标级类别
                    let has_t2_any = at.mask & 0b01_0010 != 0; // buy2 | sell2
                    let has_t3_any = at.mask & 0b10_0100 != 0; // buy3 | sell3
                    let has_t1_any = at.t1_any();
                    let cat = if has_t2_any {
                        0
                    } else if has_t3_any {
                        1
                    } else if has_t1_any {
                    ...
                    cross_cat[g][cat][pdc] += 1;

                    // 同侧/反侧细分（指示计数）
                    let (b2_same, b2_opp, b3_same, b3_opp) = match delta {
                        Side::Long => (
                            at.mask & 0b00_0010 != 0, // buy2
                            at.mask & 0b01_0000 != 0, // sell2
                            at.mask & 0b00_0100 != 0, // buy3
                            at.mask & 0b10_0000 != 0, // sell3
                        ),
                        Side::Short => (
                            at.mask & 0b01_0000 != 0, // sell2
                            at.mask & 0b00_0010 != 0, // buy2
                    ...
                    for (k, on) in [b2_same, b2_opp, b3_same, b3_opp].into_iter().enumerate() {
                        if on {
                            cross_side[g][k][pdc] += 1;
                        }
                    }
                    for b in 0..6 {
                        if at.mask & (1 << b) != 0 {
                    ...
                    if g == 0 {
                        // 二类 bit 的归属锚（`make_second_point` 载 OwnerRef::Type1Anchor）
                        let mut anchors = Vec::new();
                        for q in tls.bsp.iter() {
                            if q.source_index != src || !(q.bits.buy2 || q.bits.sell2) {
                                continue;
                            }
                            if let Some(OwnerRef::Type1Anchor(a)) = q.center {
    ...
    for &(g, lvl, src, delta) in &sigs {
        let tl = lvl - 1;
        if let Some(bl) = final_bsp.get(tl) {
            if bsp_at_coord(bl, src).present {
                f_present[g] += 1;
            }
        }
        if let Some(pl) = final_pd.get(tl) {
            let (pl_long, pl_short) = pan_div_sides_at_coord(pl, src);
    ...
    let bit_name = |b: usize| ["buy1", "buy2", "buy3", "sell1", "sell2", "sell3"][b];
    let mut rpt = String::new();
    let _ = writeln!(
        rpt,
    ...
    for lvl in LMIN..=LMAX {
        let _ = writeln!(
            rpt,
            "| {lvl} | {} | {} | {} |",
            raw_t1_pts[lvl], n_sig[0][lvl], n_sig[1][lvl]
        );
    }
    let _ = writeln!(rpt);
    for g in 0..NG {
        let _ = writeln!(
            rpt,
            "- {}：lvl≥1 共 **{}** 例，其中坐标存在次级别 BSP 条目 **{}** 例（#848 §4 的分母）",
            gname(g),
            n_lvlge1[g],
            n_present[g]
        );
    ...
    for g in 0..NG {
        let _ = writeln!(rpt, "### 1.{} {}", g + 1, gname(g));
        let _ = writeln!(rpt);
        let _ = writeln!(rpt, "| 次级别标了什么 | **∧ 同侧盘背** | ∧ 仅反侧盘背 | ∧ 无盘背 | 合计 | 同侧盘背% | 任意侧盘背% |");
        let _ = writeln!(rpt, "|---|---|---|---|---|---|---|");
        for cat in 0..4 {
            let r = &cross_cat[g][cat];
            let tot = r[0] + r[1] + r[2];
            if tot == 0 {
                continue;
            }
        ...
        for cat in 0..4 {
            for k in 0..3 {
                s[k] += cross_cat[g][cat][k];
            }
        }
        let stot = s[0] + s[1] + s[2];
        let _ = writeln!(
            rpt,
            "| **合计** | **{}** | {} | {} | {stot} | {:.2}% | {:.2}% |",
            s[0],
        ...
        let _ = writeln!(rpt);
        let _ = writeln!(rpt, "**票面点名的四格**（合并盘背侧向）：");
        let _ = writeln!(rpt);
        let _ = writeln!(rpt, "| | 有盘整背驰 | 无盘整背驰 |");
        let _ = writeln!(rpt, "|---|---|---|");
        let _ = writeln!(
            rpt,
            "| 标二类 | {} | {} |",
            cross_cat[g][0][0] + cross_cat[g][0][1],
            cross_cat[g][0][2]
        );
        let _ = writeln!(
            rpt,
            "| 标三类 | {} | {} |",
            cross_cat[g][1][0] + cross_cat[g][1][1],
            cross_cat[g][1][2]
        );
        let _ = writeln!(rpt);
    }
    ...
    // §3 全 lvl≥1 口径（含坐标无次级别条目的例）
    let _ = writeln!(rpt, "## 3. 全 lvl≥1 口径（防「只看有条目那批」的选择偏差）");
    let _ = writeln!(rpt);
    let _ = writeln!(rpt, "| 群 | lvl≥1 例数 | 有次级别条目 | 无条目 | 无条目∧同侧盘背 | 无条目∧仅反侧 | 无条目∧无盘背 |");
    let _ = writeln!(rpt, "|---|---|---|---|---|---|---|");
    for g in 0..NG {
        let a = &absent_pd[g];
        let _ = writeln!(
    ...
    for (k, r) in rows.iter().enumerate() {
        ...
        for b in 0..6 {
            ...
            let bit_is_long = b < 3; // buy* = 买侧 = Long
            let same = (r.delta == Side::Long) == bit_is_long;
            bits_s.push(format!(
                "{}({})",
                bit_name(b),
                if same { "同" } else { "**反**" }
        ...
        let (pd_same, pd_opp) = match r.delta {
            Side::Long => (r.pd_long, r.pd_short),
            Side::Short => (r.pd_short, r.pd_long),
        };
        let anchors_s = if r.anchors.is_empty() {
            "—".to_string()
        } else {
            r.anchors
                .iter()
        ...
        let dist_s = if r.anchors.is_empty() {
            "—".to_string()
        } else {
            r.anchors
                .iter()
    ...
    let out = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("rust/ 父目录 = 项目根")
        .join(format!(
            ".chanlun/review-results/issue851-quasi-first-intersection-raw-b{n}.md"
        ));
    std::fs::write(&out, &rpt).unwrap_or_else(|e| panic!("写报告失败：{e}"));
    eprintln!("\n原始数据已落盘：{out:?}");
    ...
    for g in 0..NG {
        ...
        let absent_sum: usize = absent_pd[g].iter().sum();
        assert_eq!(
            absent_sum + n_present[g],
            n_lvlge1[g],
            "{} 分母穷举：无条目({absent_sum})+有条目({}) 应 = lvl≥1 例数({})",
            gname(g),
            n_present[g],
            n_lvlge1[g]
        );
    ...
    assert_eq!(
        rows.len(),
        n_present[0],
        "Type1 逐例 dump 行数({}) 应 = 有条目例数({})",
        rows.len(),
        n_present[0]
    );
    eprintln!(
        "真封：Type1 lvl≥1={} 有条目={}；Type2/3 lvl≥1={} 有条目={}",
...
fn nesting_descent_and_type1_reach_dx() {
    ...
    use super::super::super::classifier::divergence::compute_macd;
    use super::super::super::classifier::recursive_tower::{find_move_by_end_index, LeveledMove};
    use super::super::data;
    use std::fmt::Write as _;
    ...
    for g in 0..NG {
        let _ = writeln!(
            ...
            b_pop[g],
            b_hit_n[g],
            pct(b_hit_n[g], b_pop[g]),
            if b_hit_n[g] == 0 {
                f64::NAN
            } else {
                b_steps_n[g] as f64 / b_hit_n[g] as f64
            },
            b_miss_bottom_n[g],
            b_hit_w[g],
            pct(b_hit_w[g], b_pop[g]),
            if b_hit_w[g] == 0 {
                f64::NAN
            } else {
                b_steps_w[g] as f64 / b_hit_w[g] as f64
            },
            b_miss_bottom_w[g]
        );
    }
