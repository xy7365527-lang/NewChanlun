...
fn beta_drift_suspect(
    stats: &BTreeMap<(u32, u8, i8, i8), CellStat>,
    key: (u32, u8, i8, i8),
) -> bool {
    let (lv, bc, d, pd) = key;
    let Some(cell) = stats.get(&key) else {
        return false;
    };
...
fn render_slice(
    ...
    for (&key, c) in stats {
        ...
        let base_state = base
            ...
            .unwrap_or_else(|| "—".into());
        let changed = if base_state == format!("{:?}", c.state) {
            "同"
        } else {
            "**变**"
        };
...
fn highlow_full() {
    ...
    let l0: Vec<ResidualTrade> = records
        .iter()
        .filter(|r| r.class.level == 0)
        .copied()
        .collect();
    ...
    for v in [1i8, -1, 0] {
        let slice: Vec<ResidualTrade> = l0
            .iter()
            .filter(|r| r.class.sigma_higher == Some(v))
            .copied()
            .collect();
        if slice.is_empty() {
            report.push_str(&format!(
        ...
        let (rows, alerts) =
            render_slice(&format!("σ^H={v:+}（n={}）", slice.len()), &stats, &base);
    ...
    let anchored: Vec<ResidualTrade> = l0
        .iter()
        .filter(|r| r.class.nest_depth.is_some())
        .copied()
        .collect();
    let unanchored: Vec<ResidualTrade> = l0
        .iter()
        .filter(|r| r.class.nest_depth.is_none())
        .copied()
        .collect();
    report.push_str(&format!(
...
mod self_check {
    ...
    fn slice_and_probe_logic() {
        ...
        let plus: Vec<ResidualTrade> = rs
            .iter()
            .filter(|r| r.class.sigma_higher == Some(1))
            .copied()
            .collect();
        let minus: Vec<ResidualTrade> = rs
            .iter()
            .filter(|r| r.class.sigma_higher == Some(-1))
            .copied()
            .collect();
        ...
        assert!(
            !beta_drift_suspect(&stats, kbuy),
            "两 δ mean 异号不应标 suspect"
        );
        ...
        assert!(
            beta_drift_suspect(&dstats, kbuy),
            "两 δ mean 同号必须标 suspect"
        );
    }
}
