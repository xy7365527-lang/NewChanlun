...
//! - **精确否证对象（645号，定理类——231 caveat 的必然细化）**：本全窗否证的对象是
//!   **π_Θ^cov（覆盖净额投影，v1 走 `extract_elements`/T_i 路径）**，**不是 π_Θ^bsp
//!   （买卖点择时子声部，从未在全窗测，645号）**。两投影正交：π_Θ^cov 否证不传导至
//!   π_Θ^bsp。⟹ 「缠论择时无 alpha」严格读作「π_Θ^cov 无 alpha」，π_Θ^bsp 的有效域空白。
...
fn l3_fullwindow_multi_symbol_significance() {
    ...
    for w in PREREG_WINDOWS {
        ...
        //   `config = ThetaConfig::default()`（本文件 :67），且**无 datum 注入通道**（无 env 钩子、
    ...
    eprintln!(
         ...
         **仍未补的退化维度（诚实标注，有效域 caveat）**：\n  \
         ...
         - 精确否证对象（645号）：否证对象 = π_Θ^cov（覆盖净额投影，v1 走 extract_elements/T_i 路径），\n    \
           **非 π_Θ^bsp（买卖点择时子声部，从未在全窗测）**；两投影正交，π_Θ^cov 否证不传导至 π_Θ^bsp。\n  \
