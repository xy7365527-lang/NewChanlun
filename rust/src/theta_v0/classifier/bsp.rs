...
pub fn bsp_bit_at(bsp: &[BspPoint], source_index: usize, pred: impl Fn(&BspBits) -> bool) -> bool {
    bsp.iter().any(|p| p.source_index == source_index && pred(&p.bits))
}
...
pub fn bind_turn(bsp: &[BspPoint], turn_source: usize, side: Side) -> Option<&BspPoint> {
    bsp.iter().find(|p| p.source_index == turn_source && p.bits.confirm_side(side))
}
...
mod tests {
    ...
    fn bsp_at_finds_first_by_source_index_only() {
        let pts = vec![
            minimal_point(5, BspBits { sell1: true, ..Default::default() }),
            minimal_point(7, BspBits { buy1: true, ..Default::default() }),
        ];
    ...
    fn bsp_bit_at_checks_existence_not_first_match() {
        let pts = vec![
            minimal_point(3, BspBits { buy1: true, ..Default::default() }),
            minimal_point(3, BspBits { buy2: true, ..Default::default() }),
        ];
    ...
    fn bind_turn_requires_confirm_side() {
        let pts = vec![minimal_point(7, BspBits { buy1: true, ..Default::default() })];
    ...
    fn bsp_all_at_collects_every_match() {
        let pts = vec![
            minimal_point(3, BspBits { buy1: true, ..Default::default() }),
            minimal_point(3, BspBits { buy2: true, ..Default::default() }),
            minimal_point(4, BspBits { sell1: true, ..Default::default() }),
        ];
    ...
    fn no_stray_bsp_binding_rewrites_outside_registry() {
        ...
        const WHITELIST: &[&str] = &[
            ...
            "theta_v0/parser/fractal.rs:80",
            // #[ignore] 死探针（已被 runner::newly_confirmed_step 取代，issue #747 条目6 登记不入
            // 18 处枚举）。
            "theta_v0/backtest/l3_pi_probe.rs:492",
            // ADR-0005 GUARD-ROLE 对照臂原型语句本体（单列登记，不改调，见函数头注释订正）。
            "theta_v0/classifier/nest.rs:690",
            // `CrossLevelConfirmationQuery::entry_at` 单源函数体（作用于 `TripleAnchorEntry`，
            // 与本族 `BspPoint` 三件并行不同型，见 projection.rs 函数头注释）。
            "theta_v0/classifier/projection.rs:212",
            // #[cfg(test)] mod 内断言用字面量 source_index 过滤——测试夹具，非生产绑定规则。
            "theta_v0/classifier/signal.rs:2268",
            "theta_v0/classifier/signal.rs:2438",
            "theta_v0/classifier/signal.rs:2760",
            "theta_v0/classifier/signal.rs:2791",
            "theta_v0/classifier/signal.rs:3013",
            "theta_v0/classifier/signal.rs:3059",
            // 「时刻分组键」过滤（spec §12 语义，合法契约非泄漏，非身份 join，不入本族）。
            "theta_v0/strategy/mod.rs:543",
            "theta_v0/strategy/mod.rs:759",
        ];
        ...
        for line in String::from_utf8_lossy(&output.stdout).lines() {
            let Some(rest) = line.strip_prefix(&src_dir_prefix) else { continue };
            let Some((path, tail)) = rest.split_once(':') else { continue };
            let Some((lineno, _content)) = tail.split_once(':') else { continue };
        ...
        let stray: Vec<&String> = hits.iter().filter(|h| !whitelist.contains(h.as_str())).collect();
        assert!(
            stray.is_empty(),
