...
//! 证明本身是在旧弱口径 `ZD_B≤ZG_B` 下完成的**，见下方⚠注）。故 B 口径下 `ThirdSpansCore` 被
...
//! 随 B 收窄。
//!
//! ⚠**#321 从严后本恒等式尚欠严格版重述（已立票 #328，本文件不做）**：上段"接受/拒绝集严格相等"
//! 的证明基于**弱**口径 `ZD_B≤ZG_B`，靠段不变量 `d₃≤g₃`（对任意段恒真）补齐第 9 个交叉不等式。
//! #321 把成立口径改严为 `ZD_B<ZG_B` 后，该证明**未随之重做**——严格版重述需要补上「每段
//! `lo<hi`」这一前提的新论证（弱口径下允许 `d₃==g₃` 单点段仍能补齐第 9 个不等式，严格版下
//! 这一步是否仍然永真待重新论证，不能想当然沿用旧证明）。下方 `third_spans_core` 函数 doc
//! （:130/:133 一带）引用的仍是这个**弱口径版**等价式原文，同样未更新，是本作用域内已知的一致
//! 未证明缺口，不在 #323 返工范围——严格版重述已另立票 **#328**（third_spans_core（637）等价式
//! 严格版重述：补「每段 lo<hi」前提的新论证，#323 评审浮出，OPEN）跟进。
//!
...
///
/// ⚠**本段两处 `ZD_B≤ZG_B` 是 637号原等价式的弱口径版本，未随 #321 更新**：`center_from_segments`
/// 本身已于 #321（2026-07-26）从严为 `ZD_B<ZG_B`，但上面这条 A/B 接受集等价证明是在旧弱口径下
/// 完成的，其严格版重述（需补「每段 `lo<hi`」前提的新论证）尚未做——`third_spans_core` 本函数的
/// 判据逻辑（`c.lo <= zg2 && zd2 <= c.hi`，闭区间）同样未改，仍是旧口径的派生见证。严格版跟进见
/// **#328**（本文件不做）。
