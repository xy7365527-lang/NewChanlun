...
pub fn cand_delta_tower_cached(
    ...
    if std::env::var(env_registry::DIAG_CANDCACHE).is_ok() {
...
pub fn cand_delta_entry_tower(
    classification: &Classification,
    legacy_events: &[Vec<CandDeltaEvent>],
) -> Vec<Vec<CandDeltaEntryEvent>> {
    classification
        .levels
        .iter()
        .enumerate()
...
/// 全量版（bit-exact）。
pub(crate) fn cache_series_ok(cache: &TowerCache, n: usize) -> bool {
