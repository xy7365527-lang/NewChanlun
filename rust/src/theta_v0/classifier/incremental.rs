...
pub fn classify_with_tower_incremental(
    ...
    // hist/dif 借 `cache.macd_{hist,dif}`（disjoint field 借用——循环内的 `&mut cache.levels[i]`
    // 借的是另一字段路径，见 §1.2）。force DIF 峰 proxy 输入，bit-exact 等价全量。
...
fn sync_cached_units(lc: &mut LevelCache, units: &[UnitRange], dirty_from: usize) {
...
fn pop_frontier_window(lc: &mut LevelCache) -> PoppedFrontier {
...
fn advance_confirmed_watermark(lc: &mut LevelCache, cascade_reset: bool) {
    let w_nat = lc
        .upper_moves
        .len()
...
fn debug_assert_level_alignment(lc: &LevelCache) {
    debug_assert!(
...
fn derive_units_anchors(parent_blocks: &[MoveBlock], units_len: usize) -> Vec<Option<Direction>> {
    (0..units_len)
