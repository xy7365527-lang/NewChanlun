//! #1087/#1080 Rust↔Lean 提取对拍共享面（fixture 写盘 + Lean 复核；cfg issue1087_parity）。
//!
//! 自 `rust/tests/issue1087_scan_lean_parity.rs` 上移：strict_nest_check（3b 镜像对拍执行器，
//! #1060/ADR 0026）与重型签收测试共用同一 fixture 生成与 Lean 复核口径——单一对拍面，避免
//! 第二份渲染器漂移（#799 收敛通则）。
...
use crate::theta_v0::types::{Bar, Direction, Side};
...
pub fn issue1087_config() -> ThetaConfig {
...
/// 镜像对拍窗口（fixture 校验标签用；symbol 无 `'static` 约束——bin 可从 env 读入）。
#[derive(Debug, Clone)]
...
/// 完整级别阶梯校验（L0 连续唯一至 reached；require_l2=true 时 reached ≥ 2——
/// #1087 签收窗口径；false 时允许浅前缀只到 L0/L1，仍逐级全比对）。
pub fn assert_complete_level_ladder(
    window: &MirrorWindow,
    stage: &str,
    raw_levels: &[u32],
    require_l2: bool,
    ...
    if raw_levels.is_empty() {
        return Err(format!("{window:?} {stage} 至少 L0"));
    }
    let mut levels = raw_levels.to_vec();
    levels.sort_unstable();
    let reached = *levels.last().expect("非空已断言");
    if require_l2 && reached < 2 {
        return Err(format!(
            "{window:?} {stage} 必须至少达到 L2，实际 L{reached}"
        ));
    }
    if reached > 8 {
        return Err(format!("{window:?} {stage} l_max=8 不得越界"));
    }
    if levels != (0_u32..=reached).collect::<Vec<_>>() {
        return Err(format!(
...
fn actual_center(value: &crate::theta_v0::types::Center) -> String {
    format!(
        "{{ zd := {}, zg := {}, dd := {}, gg := {}, startIndex := {}, endIndex := {} }}",
        value.zd, value.zg, value.dd, value.gg, value.start_index, value.end_index
    )
...
fn actual_segment(value: &crate::theta_v0::types::Segment) -> String {
    format!(
        "{{ direction := {}, startIndex := {}, endIndex := {}, startPrice := {}, endPrice := {} }}",
        actual_direction(value.direction),
        value.start_index,
...
pub fn write_fixture(path: &Path, records: &[issue1087_parity::ScanParityRecord]) {
...
pub fn parse_trade(line: &str, path: &Path, line_no: usize) -> (i64, i64, f64) {
...
pub fn load_equs_bars(root: &Path, symbol: &str, required: usize) -> Vec<Bar> {
...
/// 数制核验：Databento nanodollar 价 → 美分 tick 往返（EQUS.MINI 刻度 0.01）。
pub fn databento_nanodollars_price_uses_cents_tick_roundtrip_check() {
    let raw_price = 123_450_000_000_i64; // $123.45 in Databento nanodollars.
    let tick = databento_nanodollars_to_tick(raw_price);
    assert_eq!(tick, 12_345);
    assert_eq!(tick_to_price(tick, EQUS_MINI_TICK_SIZE), 123.45);
...
/// EQUS.MINI 数据根（env 直读留在调用方——theta_v0 内禁止裸 env 字面量，见
/// env_registry 完备性锁；测试侧经 `EQUS_MINI_DIR` 覆盖后传入）。
pub fn equs_mini_root(env_dir_override: Option<&Path>) -> PathBuf {
    if let Some(root) = env_dir_override {
...
pub fn cand_case_is_accepted(case: &issue1087_parity::WireCandDeltaCase) -> bool {
    match case.kind {
...
pub fn stage_event_domain_counts(records: &[issue1087_parity::ScanParityRecord]) -> (usize, usize) {
...
/// `coverage_preconditions`：签收窗覆盖前置（raw/事件/rows/centers/λ_C 非空）是否强制——
/// #1087 签收测试 true；strict_nest_check 镜像门 checkpoint false（浅前缀逐级全比对照跑，
/// 覆盖前置只在末根终态/签收窗内评估）。
pub fn verify_records_in_lean(
    formal: &Path,
    window: MirrorWindow,
    stage: &str,
    records: &[issue1087_parity::ScanParityRecord],
    require_l2: bool,
    coverage_preconditions: bool,
) -> Result<(), String> {
    if records.is_empty() {
        return Err(format!("{window:?} {stage} 不得无快照"));
    }
    let mut levels: Vec<_> = records.iter().map(|record| record.level).collect();
    levels.sort_unstable();
    levels.dedup();
    if levels.len() != records.len() {
        return Err(format!("{window:?} {stage} 每级必须恰一份快照"));
    }
    assert_complete_level_ladder(&window, stage, &levels, require_l2)?;
    let (accepted_raw, production_events) = stage_event_domain_counts(records);
    if coverage_preconditions {
        if accepted_raw == 0 {
            return Err(format!(
        ...
        for record in records {
            if record.prelude_input.rows.is_empty() {
                return Err(format!("{window:?} {stage} L{} rows 空", record.level));
            }
            if record.level <= 2 {
                if record.prelude_input.centers.is_empty() {
                    return Err(format!("{window:?} {stage} L{} centers 空", record.level));
                }
                if record.episode_cases.is_empty() {
                    return Err(format!("{window:?} {stage} L{} λ_C 空", record.level));
                }
            }
        }
    }
    ...
    let lean = Command::new("lake")
        ...
        .arg(&fixture)
        .output()
        .map_err(|error| format!("无法执行 lake env lean: {error}"))?;
...
fixture={}",
    ...
    fs::remove_file(&fixture).map_err(|error| format!("fixture 清理失败: {error}"))?;
    Ok(())
}
