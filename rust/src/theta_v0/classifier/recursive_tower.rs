...
// ── #1180（D01）三块迁出后的原路径保持（纯移动零行为，消费面零改）──────────────────────
// cp recall 审计类型族 → `diag/cp_recall_audit.rs`；CandDeltaEvent 投影族 → `cand_event/delta.rs`；
// 证书 view 族 → `chain_cert/`。此处重导出保 `recursive_tower::X` 原路径逐字不变。
pub use super::cand_event::{
    relaxed_cand_delta_entries, CandDeltaCpEdge, CandDeltaEntryEvent, CandDeltaEntryOrigin,
    CandDeltaEvent,
};
pub use super::chain_cert::{
    cp_certificate_at_divergence, cp_terminal_certificate, CompletedTrendDecomposition,
    CpCertificateView, FullTrendCQualified, FullTrendQualificationEvidence,
};
pub(crate) use super::diag::cp_recall_audit::cp_scan_ownership;
pub use super::diag::cp_recall_audit::{
    audit_cp_recall_upper_bound, CpLifecycleStatus, CpRecallAtom, CpRecallAuditCase,
    CpScanOwnership, WinMeta, WindowScanCursor,
};
...
// ── #1180（D01）：CandDeltaEvent 载体层已拆分——CandDeltaEvent 投影族迁至 `cand_event/delta.rs`，
//    证书 view 族迁至 `chain_cert/`。此处保留 c_p 生命周期侧（identity structs +
//    advance_cp_lifecycles / full_trend_c_qualification*）。
...
pub(crate) fn cp_unit_to_segment(unit: &UnitRange) -> Segment {
