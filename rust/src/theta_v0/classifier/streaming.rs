...
use super::{classify_incremental, ClassifyOutput, TowerCache};
...
///
/// **bit-exact 契约**：`append_bar(bar)` 逐次调用的返回值序列的 `classification`/`tower` == 全量
/// `classify(parse_layer(&bars[..=i]), config, &[])`（`backtest::incremental::tests::owned_bit_exact_*`
...
impl OwnedIncrementalClassifier {
    ...
    /// **per-bar 增量重分类（自持缓冲区）**：追加单根 bar，返回统一输出模块
    /// [`ClassifyOutput`]（`classification`/`tower` == `classify(parse_layer(&bars[..=i]), config,
    /// &[])`，bit-exact，`i` = 累计追加次数-1；候选事件流随 `TowerCache` 跨 bar 持久）。
    ///
    /// 与借用变体 `classify_at` 的唯一差异：吃 owned `Bar` 而非索引进外部切片——调用方
    /// （如 `ThetaCore::bars.push(new_bar)` 后）直接把新 bar 传入本方法，无需持有一个生命
    /// 周期内稳定不变的完整切片。
    pub fn append_bar(&mut self, bar: Bar) -> ClassifyOutput {
        let l0 = self.append_parse(bar);
        classify_incremental(&l0, &self.config, &mut self.tower_cache, &[])
    }
