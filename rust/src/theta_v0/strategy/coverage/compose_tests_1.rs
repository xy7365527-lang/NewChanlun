...
fn pi_theta_step_traced_opened_and_bitexact() {
    ...
    assert_eq!(
        trace.opened[0].1.id, na_t[0].id,
        "opened 腿 = next_active 中的新腿"
    );
...
fn pi_theta_step_traced_reverse_close_attribution() {
    ...
    assert_eq!(
        trig.bsp_class, 1,
        "触发归因 = 一类卖候选（reverse_exit_type ⟹ CloseRoot）"
    );
    assert!(
        !next_active.iter().any(|l| l.id == held.id),
        "被关腿不入 next_active"
    );
...
fn t1_target_zero_assertion_buy_batch_checks_short_side_only() {
    ...
    let (na, _ps, _o, trace) = pi_theta_step_traced(
        work,
        &gamma,
        &[held_short, held_long, held_parent_a],
        600.0,
        11,
        1000.0,
        &r,
...
fn pi_theta_step_traced_typed_close_reverse_open_overrides_trigger() {
    ...
    assert_eq!(
        trig.bsp_class, 1,
        "触发类=1（若非 ReverseOpen 会派 CloseRoot）"
    );
    assert_eq!(
        *exit_type,
