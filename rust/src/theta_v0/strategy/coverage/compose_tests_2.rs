...
fn pi_theta_step_traced_p1_force_flat_risk_exits_all() {
    ...
    assert!(
        ...
        "P1 强平 ⟹ next_active=∅（open 不入账 = 幽灵腿堵口）"
    );
    assert_eq!(
        trace.risk_exits.len(),
        1,
        "prev_active 全部 RiskExit（无触发候选）"
    );
...
fn pi_theta_step_traced_p2_close_overlay() {
    ...
    assert!(
        !next_active.iter().any(|l| l.id == sd_leg.id),
        "重叠腿不入 next_active"
    );
    assert!(
        next_active.iter().any(|l| l.id == root_leg.id),
        "非重叠根腿保留"
    );
    assert!(
        trace.opened.is_empty(),
        "P2 消耗当步裁决 ⟹ 买候选不开仓（屏蔽 P8）"
    );
    ...
    assert_ne!(
        order_p2, order_none,
        "P2 CloseOverlay 真改同 bar 订单（非仅 trace 差异）"
    );
}
...
fn pi_theta_step_traced_p3_withdraw_consumes_step() {
    ...
    assert!(
        trace.opened.is_empty(),
        "P3 消耗当步裁决 ⟹ 买候选不开仓（屏蔽 P5..P10）"
    );
    assert!(
        next_active.is_empty(),
        "无持仓腿 ⟹ next_active 空（无开仓）"
    );
...
fn pi_theta_step_traced_p1_masks_tw_predicates() {
    ...
    assert!(
        trace.overlay_closes.is_empty() && trace.tw_event.is_none(),
        "P2/P3/P4 被 P1 屏蔽"
    );
}
