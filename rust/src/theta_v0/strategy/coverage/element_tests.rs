...
fn attach_inherits_host_compose_parent_dir() {
    ...
    assert_eq!(
        parent,
        Some(0),
        "hostOf(a1) 的真 Compose 父 = compose_a(idx0)"
    );
...
fn attach_right_endpoint_归产出段_not_next_start() {
    ...
    assert_eq!(
        (pb, sb),
        (Some(4), Some(VoiceSide::Short)),
        "16=b0.ρ ⟹ host=b0,父 compose_b=Short"
    );
}
...
fn attach_level_context_disambiguates_shared_rho() {
    ...
    assert_eq!(
        attach_bsp_to_tree(&tree, 0, 12),
        (Some(0), Some(VoiceSide::Long))
    );
...
fn attach_ordinal_identity_not_rmove_struct_equal() {
    ...
    assert_eq!(
        attach_bsp_to_tree(&tree, 0, 16).1,
        Some(VoiceSide::Short),
        "ρ=16 → b0 真父 Short"
    );
    assert_eq!(
        attach_bsp_to_tree(&tree, 0, 12).1,
        Some(VoiceSide::Long),
        "ρ=12 → a2 真父 Long"
    );
}
...
fn carrier_forest_all_levels_dedup_unique_id() {
    ...
    assert_eq!(
        ids.len(),
        n_before,
        "K_i dedup 后无重复 ElementId（codex NO#2）"
    );
    ...
    assert!(
        kidx.contains_key(&(0, 8)),
        "L0 子 ρ=8 在 K_i（endpoint-complete）"
    );
...
fn carrier_forest_dedup_triggers_when_l0_nonempty() {
    ...
    assert_eq!(
        l0_child.parent_id,
        Some(eid(1, 0)),
        "dedup 保留带真 parent_id 的出现（codex NO#2）"
    );
...
fn a12_dual_view_consistency_rejects_forgery() {
    ...
    dup.push(k_i[0]);
    assert!(
        dual_view_consistency(&t_i, &dup).is_err(),
        "K_i id 重复必拒"
    );
    // T_i 元素缺失（K_i 少一个 T_i 元素）。
    let missing: Vec<CoverageElement> = k_i.iter().filter(|e| e.id != t_i[0].id).copied().collect();
    assert!(
        dual_view_consistency(&t_i, &missing).is_err(),
        "T_i⊆K_i 破裂必拒"
    );
    // 同 id 字段分叉。
    ...
    forked[pos].rho += 1;
    assert!(
        dual_view_consistency(&t_i, &forked).is_err(),
        "同 id 字段分叉必拒"
    );
}
...
fn attach_host_not_found_is_ambient() {
    let tree = extract_elements(&two_parent_tower());
    assert_eq!(
        attach_bsp_to_tree(&tree, 0, 99),
        (None, None),
        "无本级 host ⟹ 无真父 ⟹ Ambient"
    );
}
...
fn attach_guard_no_compose_level_is_ambient() {
    ...
    assert_eq!(
        attach_bsp_to_tree(&tree, 0, 4),
        (None, None),
        "缺塔（len<2）⟹ host 是根 ⟹ Ambient"
    );
}
...
fn from_classification_levels_flat_root_coverage() {
    ...
    assert!(
        elements.iter().all(|e| e.parent.is_none()),
        "扁平入口全根级（无真父子）"
    );
...
fn element_id_deterministic_full_vs_incremental() {
    ...
    let (mut prefix_c, mut prefix_upper, _, _m6, cursor6) =
        compose_level_resume(&units[..6], &moves[..6], true, 1, 0, 0, &[]);
    if cursor6.resume_from < cursor6.consumed {
        prefix_upper.pop();
        prefix_c.pop(); // #897：前缀中枢与上级走势 1:1 同步 pop（frontier 协议）
    }
    let (_tc2, tail_upper2, _, _mt2, _) = compose_level_resume(
        &units,
        &moves,
        true,
        1,
