...
use crate::theta_v0::types::{BspBits, Direction};
...
fn open_candidate_parent_injected_from_registry_admits_depth_child() {
    ...
    assert_eq!(
        ids.len(),
        n,
        "next_active 含重复 ElementId（restore 未复用现有 idx）⟹ p̃ 双计；实得 {active2:?}"
    );
}
...
fn restore_reuses_existing_work_idx_no_duplicate_id() {
    ...
    assert_eq!(
        work.len(),
        1,
        "restore 不得 push 重复 id 元素（应复用 work[0]，overlay 空）"
    );
...
fn restore_rebuilds_parent_link_and_attached_dir() {
    ...
    assert_eq!(
        work[0].parent,
        Some(1),
        "恢复元素 parent 索引须由 parent_id 重建（#247 缺口二）"
    );
    assert_eq!(
        work[0].attached_dir,
    ...
    assert_eq!(
        work[1].parent, None,
        "真 ∂ 根（parent_id=None）保持 None——正确语义非兜底"
    );
    assert_eq!(
        work[1].attached_dir, None,
        "真 ∂ 根 σ_p=0 ⟹ Ambient（正确）"
    );
    let flat = work.as_contiguous().into_owned();
    assert_eq!(
        vertical_relation(&flat, 0),
        Vertical::ReverseOpen,
        "恢复元素不再恒定落 V=Ambient（δ=Long vs σ_p=Short ⟹ ReverseOpen）"
    );
    assert_eq!(
        grade_relation(&flat, 0),
        GradeRel::SubLevel,
...
fn restore_role_rebuild_changes_p_tilde_leg_set_unchanged() {
    ...
    assert!(
        (pre_p - (-600.0)).abs() < 1e-9,
        "口径①改前（全 Ambient）p̃ 复现 = −600；实得 {pre_p}"
    );
    ...
    let half_p = net_target_units(&strategy_target_legs(
        &half_view,
        &[0, 1, 2],
        1000.0,
        &cfg(),
...
fn restore_deep_chain_depth_ge3_weight_zeroed() {
    ...
    assert_eq!(
        active.len(),
        4,
        "四级链全部恢复入 A_{{t+1}}（身份不变，限定 risk=None）"
    );
    assert!(
        (p - (-1000.0)).abs() < 1e-9,
    ...
    let deep = vec![
        CoverageElement {
            ...
            level: 0,
            parent: Some(1),
            attached_dir: Some(VoiceSide::Short),
            id: child,
            parent_id: Some(p1),
        },
        CoverageElement {
    ...
    assert!(
        (pre_p - (-1200.0)).abs() < 1e-9,
        "深链改前 p̃ 复现 = −1200；实得 {pre_p}"
    );
}
...
fn element_depth_cycle_fails_loudly_with_probe() {
    ...
    let cyclic = vec![
        ...
        CoverageElement {
            ...
            level: 1,
            parent: Some(0),
            attached_dir: Some(VoiceSide::Long),
            id: b,
            parent_id: Some(a),
        },
    ];
    ...
    assert_eq!(
        ancok_probe_snapshot().element_depth_fuel_exhausted,
        1,
    ...
    ancok_probe_reset();
    assert_eq!(
        element_depth(&ElementView::new(&acyclic), 0),
        1,
        "无环链正常返回 depth=1"
    );
    assert_eq!(
...
fn held_leg_reregister_reuses_restore_pushed_idx_no_duplicate_id() {
    ...
    assert!(
        ...
        "子腿借 restore 恢复的父链 AncOK 准入；实得 {active:?}"
    );
...
fn open_candidates_same_carrier_id_reverse_pair_annihilates() {
    ...
    assert!(
        ...
        "父 carrier 经 open 父注入 restore 恢复；实得 {active:?}"
    );
...
fn opened_restore_leg_not_externalized_for_same_carrier_candidate_pair() {
    ...
    let leg_child = ActiveLeg {
        ...
        lambda: 20,
        id: child,
        parent_id: Some(carrier),
        is_boundary_root: false,
        op_parent: Some(carrier),
    };
    ...
    let (na, _ps, _o, trace) = pi_theta_step_traced(
        ElementView::from_parts(&tree, candidates),
        &gamma,
        &[leg_child],
        0.0,
        5,
    ...
    assert_eq!(
        carrier_legs.len(),
        1,
        "restore 复用树 idx 注入 carrier 恰一条在场；实得 {na:?}"
    );
    assert_eq!(
        carrier_legs[0].dir,
        VoiceSide::Long,
        "restore 腿方向取树/registry，非候选方向"
    );
...
fn held_leg_id_hits_candidate_copy_keeps_held_identity() {
    ...
    let reg =
        super::super::super::persistent::PersistentRegistry::new().merge(&elements, &[leg_held]);
    let (active, _p) = coverage_step_from_buckets(
        view_split(&elements, 0),
        &[leg_held],
        &buckets,
    ...
    assert_eq!(
        carrier_legs.len(),
        1,
        "held 腿与候选拷贝同 id ⟹ 恰一腿（#216 唯一性）；实得 {active:?}"
    );
    assert_eq!(
        carrier_legs[0].dir,
        VoiceSide::Long,
    ...
    assert_eq!(
        carrier_legs[0].source_index, 40,
        "持仓身份优先：坐标取腿自身（非候选点元素）"
    );
    assert_eq!(
        carrier_legs[0].op_parent,
        Some(parent),
        "op_parent 持久（I4），非候选 parent_id 改写"
    );
}
...
fn restore_does_not_reuse_candidate_segment_copy_for_ancestor() {
    ...
    assert!(
        !raw.contains(&0),
        "raw 不得含候选段 idx 0（候选拷贝非持久身份）；实得 raw={raw:?}"
    );
    assert_eq!(
        raw.len(),
        2,
        "沿持久 structural_parent_id 链上溯 ancestor→grandparent 恰两级；实得 raw={raw:?}"
    );
    let new_ancestor_idx = raw[0];
    assert_eq!(
        work[new_ancestor_idx].eps,
        VoiceSide::Long,
        "新 push 元素须取 registry 持久方向 Long，不得沿用候选拷贝的信号方向 Short"
    );
    assert_eq!(
        work[new_ancestor_idx].lambda, 10,
        "新 push 元素须取 registry 持久坐标，非候选 λ==ρ 点元素"
    );
    assert_eq!(
        work[new_ancestor_idx].parent_id,
        Some(grandparent),
        "新 push 元素须取 registry structural_parent_id，不得沿用候选伪 parent_id"
    );
    assert_eq!(
        work[raw[1]].id, grandparent,
        "上溯第二环须抵达真持久 grandparent（候选伪 parent_id 不会指向它）"
    );
}
