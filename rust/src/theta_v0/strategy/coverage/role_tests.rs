...
fn role_toplevel_is_first_ambient_not_root() {
    ...
    assert_eq!(
        r.v,
        Vertical::Ambient,
        "去根化：顶层父=胚元 σ=0 → Ambient（非 RootRole）"
    );
...
fn operation_role_split_matches_merged_lcg() {
    ...
    for _ in 0..200 {
        ...
        let elems: Vec<CoverageElement> = (0..n)
            .map(|i| {
                let parent = if tree_n > 0 && next() % 2 == 0 {
                    Some(next() % tree_n.max(1))
                } else {
                    None
                };
                CoverageElement {
        ...
        let mut cand_sib: std::collections::HashMap<(Option<usize>, u32), Vec<usize>> =
            std::collections::HashMap::new();
        for ci in cstart..n {
            cand_sib
                .entry((elems[ci].parent, elems[ci].level))
                .or_default()
                .push(ci);
        }
