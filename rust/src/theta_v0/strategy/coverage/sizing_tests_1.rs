...
fn feasible_candidates_lot_aligned_within_cap() {
    ...
    for i in 1..cands.len() {
        assert!(
            cands[i].1 > cands[i - 1].1 && cands[i].0 > cands[i - 1].0,
            "升序单射"
        );
    }
}
...
fn pi_theta_position_tracks_ptilde_within_cap() {
    let r = rcfg();
    let p_star = pi_theta_position(
        600.0,
        0.0,
        1000.0,
        &r,
        PiThetaWeights::from_risk(&r),
        KThetaRiskGate::open(),
    );
...
fn pi_theta_position_clamps_over_cap() {
    ...
    assert!(
        (p_star - 1000.0).abs() < 1e-9,
        "p* = +hi = cap = 1000（杠杆/资本 cap binding）"
    );
}
...
fn stop_long_caps_ptilde_regardless_of_leg_eps_direction() {
    ...
    let gate_stop_long = KThetaRiskGate {
        stop_long: true,
        ..KThetaRiskGate::open()
    };
    ...
    assert!(
        (p_star_open - 600.0).abs() < 1e-9,
        "无 gate：p*=p̃=600（对照组）"
    );
    assert!(
        p_star_stopped <= 1e-9,
        "stop_long ⟹ hi_cap=0 ⟹ p*≤0（禁净多，与触发腿的 eps 无关）"
    );
    assert!(
...
fn cap_binding_attribution_records_hi_and_lo_events() {
    ...
    let gate_hi = KThetaRiskGate {
        stop_long: true,
        ..KThetaRiskGate::open()
    };
    let gate_lo = KThetaRiskGate {
        stop_short: true,
        ..KThetaRiskGate::open()
    };
    ...
    assert!(
        hi_ev
            .p_star_cf_hi
            .is_some_and(|cf| (cf - 600.0).abs() < 1e-9),
        "反事实=去掉 stop_long 后应恢复 p̃"
    );
    let lo_ev = events
        .iter()
        .find(|e| e.lo_triggered)
        .expect("lo 事件在表中");
    ...
    assert!(
        lo_ev
            .p_star_cf_lo
            .is_some_and(|cf| (cf + 600.0).abs() < 1e-9),
        "反事实=去掉 stop_short 后应恢复 p̃"
    );

    cap_binding_attribution_reset();
    assert!(
        cap_binding_attribution_snapshot().is_empty(),
        "reset 后表清空"
    );
}
...
fn pi_theta_step_buy_point_entry_gap5() {
    ...
    assert_eq!(
        active[0].source_index, 4,
        "GAP-5：入场源=买卖点 source_index"
    );
    assert_eq!(active[0].dir, VoiceSide::Long);
    assert!((p_star - 600.0).abs() < 1e-9, "p̃=600 cap 内 ⟹ p*=600");
    assert_eq!(
        (order.action, order.qty, order.exec_index),
        (StrictAction::Buy, 600, 5)
    );
    assert_eq!(
        protocol_event,
...
fn p23_channel_closes_every_reverse_hit_leg_multi_leg() {
    ...
    assert!(
        !na.iter()
            .any(|l| l.id == held_root.id || l.id == held_cascade.id),
        "被关两腿均不入 next_active"
    );
    ...
    assert!(
        trace
            .verdicts
            .iter()
            .all(|v| v.exit == interp::ExitType::ReduceCore),
    ...
    for (held, ev) in [
        (held_root, Vertical::Ambient),
        (held_cascade, Vertical::FollowParent),
    ] {
...
fn p23_channel_type1_multi_leg_bit_exact_with_fold() {
    ...
    assert_eq!(
        trace.closed.len(),
        2,
        "一类多腿：channel 判据与 #209 全平同效（bit-exact 域）"
    );
    assert!(
        trace
            .closed
            .iter()
            .all(|(_, trig, exit)| { trig.bsp_class == 1 && *exit == interp::ExitType::CloseRoot }),
...
fn t2_cross_level_confirmed_certificate_holds_l1_position_end_to_end() {
    ...
    let held_idx = tree
        .iter()
        .position(|e| e.id == held.id)
        .expect("L1 compose_a 在塔中");
    ...
    assert!(
        ...
        "L0 证书不得进入 L1 的 close/typed-exit 通道"
    );
    assert!(
        trace.opened.is_empty(),
        "未持有 compose_b 父声部，L0 子候选被 §13 AncOK 拒绝"
    );
...
fn t2_unconfirmed_same_level_signal_records_without_exit_decision() {
    ...
    assert!(
        close_triggers.is_empty(),
        "未确认信号不得成为 typed exit 原料"
    );
    assert!(
        buckets.open.is_empty(),
        "未成立证书不得借规则3反向开同 carrier"
    );
    assert_eq!(
        buckets.record.len(),
        1,
    ...
    assert!(
        trace.closed.is_empty(),
        "StepTrace 不得生成 CloseRoot/ReduceCore/CloseReverseOpen"
    );
...
fn a10_c5_eta_correction_gates_p4_enter_earning() {
    ...
    assert_eq!(
        ...
        "correction=0 ⟹ 派（bit-exact 回归锁）"
    );
    // 边界：correction=tw()=100 ⟹ η_corrected=0=η⋆ ⟹ 仍派（≥ 含等号）。
    assert_eq!(
        run(100),
        Some(TwEvent::EnterEarning),
        "η_corrected=0=η⋆ ⟹ 边界仍派"
    );
    ...
    assert_eq!(
        run(101),
        None,
        "η_corrected=−1<η⋆ ⟹ 不派（G1：修正判据变严=安全侧）"
    );
}
