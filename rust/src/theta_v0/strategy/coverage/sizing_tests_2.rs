...
fn protocol_event_does_not_reorder_p1_p10_order_track_bitexact() {
    ...
    for (event, maturity) in events.into_iter().zip(expected_maturity) {
        ...
        assert_eq!(
            *baseline_order.get_or_insert(order),
            order,
            "协议轨不得改 P8 订单"
        );
    ...
    assert_eq!(
        (p1_order.action, p1_order.qty, p1_order.exec_index),
        (StrictAction::Close, 600, 5)
    );
}
...
fn order_and_protocol_product_total() {
    ...
    assert_eq!(
        (order.action, order.qty, order.exec_index),
        (StrictAction::Wait, 0, 17)
    );
    assert_eq!(
        event,
        ProtocolEvent::hold(0),
        "Hold 是显式协议像，不以 Option/缺省表达"
    );
}
...
fn zero_qty_protocol_event_remains_observable() {
    use super::super::super::protocol::{
        CompletedCenterRef, EvidenceRef, PostTrendKey, ProtocolMaturity,
    };
    ...
    assert!(event
        .reasons()
        .contains(super::super::super::protocol::ReasonSet::COMPLETED_RETRACE));
}
...
fn pi_theta_position_force_flat_gate_clamps_to_zero() {
    ...
    assert_eq!(
        p_star, 0.0,
        "force_flat ⟹ 𝒦_Θ={{0}} ⟹ p*=0（持仓 600 → Close）"
    );
}
...
fn pi_theta_position_no_increase_cap_forbids_growth() {
    ...
    let gate = KThetaRiskGate {
        force_flat: false,
        stop_long: false,
        stop_short: false,
        no_increase_cap: Some(300.0),
    };
    let capped = pi_theta_position(800.0, 300.0, 1000.0, &r, w, gate);
    assert!(
        capped <= 300.0 + 1e-9,
        "no_increase_cap=300 ⟹ p*≤300（禁增仓），实得 {capped}"
    );
    assert!(
        capped < grow,
        "接 no_increase_cap 后订单流确改变（{capped} < {grow}）"
    );
}
...
fn pi_theta_position_stop_long_gate_forbids_net_long() {
    ...
    assert!(
        p_star <= 1e-9,
        "stop_long ⟹ 禁净多 ⟹ p*≤0（止损平多走 𝒦_Θ，非第二出口），实得 {p_star}"
    );
}
...
fn pi_theta_step_reverse_open_from_parent_container_not_position() {
    ...
    assert!(
        a.is_empty(),
        "未持父 ⟹ ReverseOpen 子腿 AncOK 剪枝 ⟹ A_{{t+1}} 空"
    );
    assert_eq!(p_star, 0.0, "无活动腿 ⟹ p*=0（不开仓）");
    assert_eq!(
        order.action,
        StrictAction::Wait,
        "未持父 ⟹ ReverseOpen 剔除 ⟹ Wait（639(c)）"
    );
...
fn bit_exact_cached_indices_vs_fallback() {
    ...
    for i in 0..n {
        ...
        let (next_without, p_without) = coverage_step_prebuilt(
            work_without,
            &gamma,
            &prev_without,
            1000.0,
        ...
        reg_with.merge_in_place(
            ElementView::from_parts(&tree, Vec::new())
                .as_contiguous()
                .as_ref(),
            &next_with,
        );
    ...
    eprintln!(
        "bit_exact_cached_indices_vs_fallback: {n} bars 全部 with==without，{hits} bars 缓存命中"
    );
}
...
fn level_cap_decomposition_never_exceeds_account_cap() {
    ...
    assert!(
        (sum_level_caps - account_cap).abs() < 1e-6,
        "Σw_ℓ=1 ⟹ 分解恰好覆盖总 cap"
    );
}
...
fn clamp_levels_to_weighted_cap_clips_only_binding_levels() {
    ...
    assert_eq!(
        clamped,
        vec![(0, 50), (1, 3), (2, 0)],
        "level0 超帽裁到 50，level1 未超帽原样，level2 零项保留"
    );
    // 负向对称裁剪。
    let gated_neg = vec![(0u32, -80i64)];
    assert_eq!(
        clamp_levels_to_weighted_cap(&gated_neg, base_units, &risk),
        vec![(0, -50)]
    );
}
