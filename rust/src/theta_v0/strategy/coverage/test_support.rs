...
pub(crate) fn nested_l1(si: usize, ei: usize, sub_dirs: [Direction; 3]) -> LeveledMove {
    let s0 = LeveledMove::from_unit(
        &unit(si, si + 4, sub_dirs[0], 0, 10),
        ElementId {
            level: 0,
            ordinal: 0,
        },
    );
    let s1 = LeveledMove::from_unit(
        &unit(si + 4, si + 8, sub_dirs[1], 3, 12),
        ElementId {
            level: 0,
            ordinal: 1,
        },
    );
    let s2 = LeveledMove::from_unit(
        &unit(si + 8, ei, sub_dirs[2], 5, 15),
        ElementId {
            level: 0,
            ordinal: 2,
        },
...
pub(crate) fn view_split(elements: &[CoverageElement], cstart: usize) -> ElementView {
...
pub(crate) fn sell_bsp(si: usize) -> BspPoint {
    BspPoint {
        ...
        pivot_low: 0,
        pivot_high: 210,
        center: Some(crate::theta_v0::classifier::bsp::OwnerRef::Center(ctr(
            0, si,
        ))),
...
pub(crate) fn sell_classification_at(level: u32, source_index: usize, class: u8) -> Classification {
...
pub(crate) fn sell_classification(class: u8) -> Classification {
...
pub(crate) fn buy_classification_at(level: u32, source_index: usize, class: u8) -> Classification {
    let bits = match class {
        1 => BspBits {
            buy1: true,
            ..Default::default()
...
pub(crate) fn held_l1_compose_a() -> ActiveLeg {
    ActiveLeg {
        level: 1,
        dir: VoiceSide::Long,
        source_index: 12,
