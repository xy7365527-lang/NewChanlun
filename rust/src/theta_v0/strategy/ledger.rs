...
//!
//! #282（#280 裁定）：#149 `SplitLegLedger`/`SplitLegEvent`/`SplitLegError`/`SplitLegView`/
//! `SplitLegNetView` 全套（父腿+反翻短差子腿数量账本、#135 T6 毛暴露递归额度、nest 套娃）
//! 已删除——S6 开空腿账面形态（修1 废止）的模型层实装，git 历史可回溯。
...
mod tests {
    ...
    // ──────────────────────────────────────────────────────────────────────
    //  #282（#280 裁定）：#149 split-leg 账本测试与 #135 T6 毛暴露递归约束测试
    //  随 SplitLegLedger 机删除一并删除（S6 开空腿账面形态废止）。
    // ──────────────────────────────────────────────────────────────────────
