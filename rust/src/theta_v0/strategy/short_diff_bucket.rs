...
//!   [`TwEvent::Realize`]）。
//!
//! `Replenish` 不变：`ShortDiff(-units·price)`（建仓侧成交价本身即新成本基，无「已实现」
//! 概念可言，`avg_cost` 入参对该分支无效果）。
//!
//! [`ShortDiffBucket::realized_cash`] 保留为报告层读数——语义不变（Σ 净现金：`Reduce` 记
//! `+units·price`，`Replenish` 记 `-units·price`），但不再是单一 TwEvent 的同源值：新禁双写
//! 口径是**桶净现金增量 = 本次调用产出全部 TwEvent 金额之和**（`Reduce`：`ShortDiff`+`Realize`
//! 两份相加＝`units·price`；`Replenish`：仅 `ShortDiff` 一份）。
//!
...
/// `record_action` 单次产出的 TW 事件序列（P1 修复 #293：`Reduce` 拆两笔，`Replenish` 单笔）。
///
/// - `short_diff`：成本基划转（[`TwEvent::ShortDiff`]，恒不漂移 TW，`ledger.rs::tw_step_preserves_tw`
///   覆盖的守恒构造子之一）。
/// - `realize`：已实现盈亏（[`TwEvent::Realize`]），仅 `Reduce` 产出；`Replenish` 恒 `None`——
///   建仓侧无「已实现」盈亏概念，成交价即新成本基。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ShortDiffAccount {
    ...
    /// - `Replenish`（回补买入）单笔：`ShortDiff(-units·price)`（free→holding，建仓侧成交价
    ///   本身即新成本基，无「已实现」概念，`avg_cost` 对该分支无效）。
    ///
    /// `avg_cost`：持仓均价，caller 显式入参——同 `units`/`price` 一样不臆造默认值（本模块
    /// 「范围边界」既有原则）。真实生产来源 = 持仓账本接口（本仓成本基/units 均价读数，见
    /// [`CoreCostBasisSnapshot`]）；本模块只读消费，不推导/不重算。生产侧尚未实例化真实账本
    /// 接线——这与 units 的真实 sizing 口径是**同一个接口缺口**（#294 前置票落地时一并接入）。
    ///
    ...
    pub fn record_action(
        ...
        let events = match action {
            ...
            CenterOscillationAction::Replenish => {
                ...
                ShortDiffEvents { short_diff: TwEvent::ShortDiff(-(units * price)), realize: None }
            }
        };
...
mod tests {
    ...
    /// `Replenish` 保持单笔 `ShortDiff(-units·price)`（建仓侧成交价即新成本基），`avg_cost`
    /// 入参对该分支无效果，无 `Realize` 产出。
    #[test]
    fn replenish_stays_single_short_diff_no_realize() {
        let mut acct = ShortDiffAccount::new(snapshot(0, 0));
        acct.record_action(CenterOscillationAction::Reduce, 10, 12, 9).unwrap();
        let events = acct.record_action(CenterOscillationAction::Replenish, 10, 9, 0).unwrap();
        assert_eq!(events.short_diff, TwEvent::ShortDiff(-90), "Replenish(10@9) ⟹ d_cash=-90（买回）");
        assert_eq!(events.realize, None, "Replenish 无已实现盈亏概念，avg_cost 入参无效果");
    }
    ...
    fn full_round_trip_conserves_units_and_realizes_cash() {
        ...
        acct.record_action(CenterOscillationAction::Replenish, 10, 9, 0).unwrap(); // 买10@9
    ...
    fn multi_round_trips_each_conserve_independently() {
        ...
        acct.record_action(CenterOscillationAction::Replenish, 5, 18, 0).unwrap();
        assert!(acct.assert_conserved().is_ok());
        acct.record_action(CenterOscillationAction::Reduce, 7, 22, 19).unwrap();
        acct.record_action(CenterOscillationAction::Replenish, 7, 19, 0).unwrap();
        assert!(acct.assert_conserved().is_ok());
    ...
    fn over_replenish_is_explicit_violation_and_state_unchanged() {
        ...
        let before = acct;
        let result = acct.record_action(CenterOscillationAction::Replenish, 8, 9, 0); // 试图买8
        assert_eq!(
            result,
    ...
    fn n_to_one_mismatch_is_not_immediately_illegal_but_fails_boundary_assertion() {
        ...
        let partial = acct.record_action(CenterOscillationAction::Replenish, 1, 18, 0); // 只买1
    ...
    fn cost_basis_untouched_across_round_trips_bucket_listed_separately() {
        ...
        for _ in 0..3 {
            ...
            acct.record_action(CenterOscillationAction::Replenish, 10, 11, 0).unwrap();
        }
    ...
    fn cost_basis_untouched_even_when_short_diff_call_is_rejected() {
        ...
        let _ = acct.record_action(CenterOscillationAction::Replenish, 99, 9, 0); // 拒绝
    ...
    fn tw_drift_equals_sum_of_realize_through_short_diff_round_trip() {
        ...
        // Replenish(20@6) ⟹ ShortDiff(-20·6=-120)（守恒，无 Realize）。
        let events2 = acct.record_action(CenterOscillationAction::Replenish, 20, 6, 0).unwrap();
        assert_eq!(events2.realize, None);
        let tw2 = events2.apply(&tw1);
        assert_eq!(tw2.tw(), tw1.tw(), "Replenish 无 Realize ⟹ 本轮 TW 不再漂移（ShortDiff 恒守恒）");

        assert_eq!(tw2.tw() - tw0_total, 40, "全程 TW 总漂移=Σ Realize=40（唯一一次 Realize），非 0");
    ...
    /// ⟹ 正确落成 `ShortDiffEvents`（Reduce 两笔/Replenish 单笔）并推进 TW 账本；往返闭合后
    /// 恒仓断言过、成本基不动；TW 漂移=本轮 Realize（P1 修复口径，非旧「TW 全程不变」）。
    #[test]
    fn center_oscillation_action_from_book_bridges_into_tw_ledger() {
        ...
        let tw2 = acct.record_and_apply(&tw1, cover_action, 15, id.zd, 0).unwrap();
        assert_eq!(tw2.tw(), tw1.tw(), "回补无 Realize ⟹ 本轮 TW 不再漂移（ShortDiff 恒守恒）");
        ...
        assert_eq!(
            tw2.tw() - tw0.tw(),
            15 * (id.zg - avg_cost),
            "全程 TW 总漂移=Σ Realize，恰等于本轮已实现盈亏"
        );
