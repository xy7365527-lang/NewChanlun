                ...
                *n_base -= shortfall;
                ...
                if excess > 0.0 {
                    if at_point && parent.dir == Polarity::Long {
                        ...
                        // basis 加权更新（视图锚与物理一致）。
                        let dq = excess / c;
                        parent.basis = (parent.basis * parent.units + excess) / (parent.units + dq);
                        parent.units += dq;
                        *n_base += dq;
...
mod tests {
    ...
    const V4: PolarityMode = PolarityMode::NestedRecursive {
    ...
    fn parse_and_guards() {
        ...
        assert!(run_positional(&t3, 2, CT)
            .unwrap_err()
            .contains("trend_flips"));
    }
    ...
    fn root_enters_full_position_at_top() {
        ...
        assert!(
            (t.shares - INITIAL_CAPITAL / 100.0).abs() < 1e-9,
            "满仓：units = 全部资金/价"
        );
    ...
    fn negation_kills_child_with_shrink_rebase() {
        ...
        let root = r.trades.iter().find(|t| t.exit_reason == "eod").unwrap();
        assert!(
            (root.shares - (750.0 + expect_back)).abs() < 1e-9,
            "根 = 750 + 缩水回补"
        );
    ...
    fn liquidation_only_at_top_emergence_perfection() {
        ...
        assert!(
            (r.final_nav - 103_250.0).abs() < 1e-6,
            "final={}",
            r.final_nav
        );
    }
