...
/// 停削时钟的层级作用域——anc 祖先趋势豁免（预注册：
/// `analysis/slow_bull_vs_bh_research.md` §4/§6）。
///
/// 三锚逐字：
/// 026:80【缠师补记】"除了日线的单边上扬走势，短线必须坚持"——日线单边上扬
/// 是短线纪律的例外；
/// 044:48【正文】"你不可能按30分钟操作，而一见到1分钟顶背驰就全部扔掉，
/// 这就变成按1分钟级别操作了"——操作级别配套（级别纪律）；
/// 086:22【正文】"在一个大级别的中枢上移中，一个小级别的第三类卖点，唯一
/// 注意的，就是这个卖点扩展出来的走势，是否会改变大级别中枢上移本身……
/// 关键是警戒的意义"——大级别趋势对小级别操作的意义。
///
/// 豁免判据是**祖先层**的走势结构读数：∃ j > k: kind(j)==Trend ∧
/// dir(j)==Up（17课趋势定义 ≥2 同向中枢 = 中枢序列单调上移的引擎直读）。
/// **工程推广（非原文字面，#1196 落法 a）**：026:80 字面限定"日线"单边
/// 上扬，本判据把"日线"推广为"祖先层（∃j>k Trend∧Up）"——级别的自然推广，
/// 属工程选择，非缠师原文字面。
...
pub enum PolarityMode {
    ...
    /// 反向词汇）；否定 = 背驰段被打破（结构判据，027:22，级联回收子树）；
