...
fn oddeven_holding_window_diagnosis() {
    ...
    let max_bars: usize = std::env::var("ECON_L2_MAX_BARS").ok().and_then(|s| s.parse().ok()).unwrap_or(usize::MAX);
    ...
    for d in &decomps {
        ...
        let e = buckets.entry((d.level, d.delta)).or_insert((0, 0.0, 0, 0.0, 0.0, 0));
        e.0 += 1; e.1 += d.actual_pnl; e.2 += hold_bars; e.3 += net_move; e.4 += d.actual_spread;
        if net_move > 0.0 { e.5 += 1; }
    }
    let market_net = px(n_bars - 1) - px(0);
    eprintln!("\n===== 持有窗诊断（BTC bars={n_bars}，全窗净涨跌={market_net:+.1}={}）=====",
        if market_net > 0.0 { "涨" } else { "跌" });
    eprintln!("{:>2} {:>3} {:>6} {:>12} {:>12} {:>14} {:>14}",
        "L", "δ", "n", "μ̂", "avg持有bar", "avg净涨跌(无向)", "avg(δ·净涨跌)");
    for ((lvl, dlt), (n, spnl, shold, snet, sspread, _)) in &buckets {
        let nn = *n as f64;
        eprintln!("{:>2} {:>+3} {:>6} {:>12.2} {:>12.0} {:>14.2} {:>14.2}",
            lvl, dlt, n, spnl / nn, *shold as f64 / nn, snet / nn, sspread / nn);
    }
    ...
    for ((lvl, dlt), (n, spnl, _, snet, _, _)) in &buckets {
        ...
        eprintln!("  L{lvl} δ{dlt:+}: μ̂={:+.1}, δ·avg净涨跌={:+.2} → {}",
            spnl / *n as f64, (*dlt as f64) * (snet / *n as f64),
            if consistent { "一致" } else { "★不一致(成本翻转)" });
    }
    ...
    for (lvl, (shold, n)) in &level_hold {
        ...
        let trend = match prev { Some(p) if avg < p => { monotone = false; "↓非单调" } Some(_) => "↑", None => "基准" };
...
fn bsp_disc(b: &BspBits) -> u8 {
    (b.buy1 as u8) | (b.buy2 as u8) << 1 | (b.buy3 as u8) << 2
        | (b.sell1 as u8) << 3 | (b.sell2 as u8) << 4 | (b.sell3 as u8) << 5
}
            ...
            for p in ls.bsp.iter() {
                ...
                for c in &assemble_gamma_with_tower(&single, &tower_i) {
                    let delta: i8 = match c.dir {
                        VoiceSide::Long => 1, VoiceSide::Short => -1, VoiceSide::Flat => continue,
                    };
...
fn ce_unit(px_in: f64, px_out: f64, fee: f64) -> f64 { (px_in + px_out) * fee }

/// 口径4（基准，现口径）：持有到下一反向新确认信号，价用确认bar close。
fn pnl_next_reverse(sig: &[Signal], px: &dyn Fn(usize) -> f64, fee: f64) -> BTreeMap<(u32, i8), (usize, f64)> {
    let mut out: BTreeMap<(u32, i8), (usize, f64)> = BTreeMap::new();
    for (idx, &(entry_bar, delta, _, level)) in sig.iter().enumerate() {
        let opp = -delta;
        let Some(&(exit_bar, _, _, _)) = sig[idx + 1..].iter().find(|(eb, d, _, _)| *eb > entry_bar && *d == opp) else { continue };
        if exit_bar <= entry_bar { continue; }
        let (pin, pout) = (px(entry_bar), px(exit_bar));
        if pin <= 0.0 || pout <= 0.0 { continue; }
        let e = out.entry((level, delta)).or_insert((0, 0.0));
        e.0 += 1; e.1 += delta as f64 * (pout - pin) - ce_unit(pin, pout, fee);
    }
    out
}
...
fn pnl_fixed_hold(sig: &[Signal], px: &dyn Fn(usize) -> f64, n_bars: usize, hold: usize, fee: f64) -> BTreeMap<(u32, i8), (usize, f64)> {
    let mut out: BTreeMap<(u32, i8), (usize, f64)> = BTreeMap::new();
    for &(entry_bar, delta, _, level) in sig {
        let exit_bar = entry_bar + hold;
        if exit_bar >= n_bars { continue; }
        let (pin, pout) = (px(entry_bar), px(exit_bar));
        if pin <= 0.0 || pout <= 0.0 { continue; }
        let e = out.entry((level, delta)).or_insert((0, 0.0));
        e.0 += 1; e.1 += delta as f64 * (pout - pin) - ce_unit(pin, pout, fee);
...
fn pnl_pivot(sig: &[Signal], px: &dyn Fn(usize) -> f64, fee: f64) -> BTreeMap<(u32, i8), (usize, f64)> {
    let mut out: BTreeMap<(u32, i8), (usize, f64)> = BTreeMap::new();
    for (idx, &(entry_bar, delta, lambda, level)) in sig.iter().enumerate() {
        let opp = -delta;
        let Some(&(_, _, rho, _)) = sig[idx + 1..].iter().find(|(eb, d, _, _)| *eb > entry_bar && *d == opp) else { continue };
...
fn parity_match_ratio(b: &BTreeMap<(u32, i8), (usize, f64)>) -> (usize, usize) {
    ...
    for (&(lvl, dlt), &(n, spnl)) in b {
        ...
        if ((spnl / n as f64) > 0.0) == (expected > 0.0) { matched += 1; }
    }
    (matched, total)
}
...
fn dump_buckets(name: &str, b: &BTreeMap<(u32, i8), (usize, f64)>) {
    ...
    for (&(lvl, dlt), &(n, spnl)) in b {
        if n == 0 { continue; }
        eprintln!("  L{lvl}({}) δ{dlt:+} n={n:>5} μ̂={:+.2}",
            if lvl % 2 == 0 { "偶" } else { "奇" }, spnl / n as f64);
    }
}
...
fn oddeven_causation_counterfactual() {
    ...
    let n_bars = ds.bars.len();
    let tick = config.tick.tick_size;
    let fee_rate = (config.exec.commission_bps + config.exec.slippage_bps + config.exec.tax_bps) / 10_000.0;

    let sig = collect_signals(&ds, &config);
    // px 借用 ds（非 move，ds 活到函数结束；collect 与 px 都只借用，无冲突）。
    let px = |i: usize| ds.bars.get(i).map(|b| b.close as f64 * tick).filter(|&p| p > 0.0).unwrap_or(0.0);
    eprintln!("\n===== 奇偶交替成因反事实（BTC bars={n_bars}，信号={}）=====", sig.len());
    ...
    let s_stat = |b: &BTreeMap<(u32, i8), (usize, f64)>| -> f64 {
        b.values().filter(|(n, _)| *n > 0).map(|(n, s)| (s / *n as f64).abs()).sum()
    };
    ...
    for _ in 0..n_perm {
        ...
        for k in (1..d.len()).rev() { let j = next(&mut seed, k + 1); d.swap(k, j); }
        for (s, &nd) in perm_sig.iter_mut().zip(d.iter()) { s.1 = nd; }
    ...
    eprintln!("  S_obs(真标签)={s_obs:.2}, S_perm均值={perm_mean:.2}, perm_p(#perm≥obs)={perm_p:.4}");
...
fn counterfactual_helpers_l1() {
    ...
    assert!((h.get(&(0, 1)).unwrap().1 - 5.0).abs() < 1e-9, "买固定持有 Σ=5");

    // 口径5 pivot：买@pivot0(10)→配对卖 rho=pivot2(14) δ+1·4=+4。
    let p = pnl_pivot(&sig, &px, 0.0);
    assert!((p.get(&(0, 1)).unwrap().1 - 4.0).abs() < 1e-9, "买 pivot pnl=+4");
