...
fn position_closed_core_boundary_matches_lean() {
    ...
    assert_eq!(
        classify_position(&c, fx.position.zd),
        RelativePosition::Within,
        "Lean: p=zd → within（闭区间）"
    );
    assert_eq!(
        classify_position(&c, fx.position.zg),
        RelativePosition::Within,
        "Lean: p=zg → within（闭区间）"
    );
}
...
fn position_total_and_exclusive_matches_lean() {
    ...
    for p in -20..=30 {
        ...
        let count = [lean_below, lean_within, lean_above]
            .iter()
            .filter(|&&b| b)
            .count();
...
fn type3_buy_rejected_when_reenter_matches_lean() {
    ...
    assert!(
        !bits.buy3,
        "Lean ¬IsType3Buy（回抽破ZG重入）⟺ rust buy3 不置位"
    );
}
...
fn type3_requires_left_center_matches_lean() {
    ...
    assert!(
        !bits.buy3,
        "Lean leftCenter=false ⟹ ¬IsType3Buy ⟺ rust buy3 不置位"
    );
}
...
fn type3_buy_lean_witness_scale_matches_fixture() {
    ...
    assert!(
        retrace > zg,
        "fixture 自洽前置：Lean eventType3 retrace 严格 > zg（不破 ZG）"
    );
    ...
    assert_eq!(
        points.len(),
        1,
        "Lean 见证尺度 retrace>zg ⟹ 一个第三类买点（rust 与 Lean IsType3Buy 一致）"
    );
    assert!(
        points[0].bits.buy3,
        "Lean eventType3 见证：rust 与 Lean IsType3Buy 同为真"
    );
    // （#218 面 A 载体形态机械适配：Center 变体读出。）
    assert_eq!(
        points[0].center.and_then(|o| match o {
            newchan_rust::theta_v0::classifier::bsp::OwnerRef::Center(c) => Some(c.zg),
            _ => None,
        }),
        Some(zg),
...
fn type3_buy_interior_point_matches_lean() {
    ...
    assert_eq!(
        points.len(),
        1,
        "离开+回试不破 ⟹ 一个第三类买点（rust 与 Lean 一致）"
    );
    assert!(
        points[0].bits.buy3,
        "内部安全点 retest>zg：rust 与 Lean IsType3Buy 同为真"
    );
    // （#218 面 A 载体形态机械适配：Center 变体读出。）
    assert_eq!(
        points[0].center.and_then(|o| match o {
...
fn boundary_retest_eq_zg_rust_aligned_to_lean_strict() {
    ...
    assert!(
        points.is_empty(),
        "rust 严格（>）：retest==zg 触及中枢=非第三类，对齐 Lean IsType3Buy 严格口径"
    );
}
...
fn boundary_retest_eq_zd_rust_aligned_to_lean_strict() {
    ...
    assert!(
        points.is_empty(),
        "rust 严格（<）：retest==zd 触及中枢=非第三类，对齐 Lean IsType3Sell 严格口径"
    );
}
...
fn signal_extraction_emits_no_second_class_only() {
    ...
    assert!(
        ...
        "产第一类（趋势背驰：C 段破最后中枢 ∧ C<A 面积）"
    );
    assert!(
        points.iter().any(|p| p.bits.buy3),
        "产第三类（离开后回试不破 ZG）"
    );
}
...
fn type2_buy_matches_lean_witness_second_point() {
    ...
    assert_eq!(
        points.len(),
        1,
        "Lean witness_secondTypeStructure 成立 ⟹ rust 产一个 B2"
    );
    assert!(
        points[0].bits.buy2,
        "Lean SecondTypeStructure Side.long ⟺ rust buy2 置位"
    );
    assert!(
    ...
    assert_eq!(
        ...
        "rust B2 止损源 pivot_low == Lean secondPointPrice m2Wit（-8）"
    );
}
...
fn type2_sell_mirror_matches_lean() {
    ...
    assert!(
        points[0].bits.sell2 && !points[0].bits.buy2,
        "Lean SecondTypeStructure Side.short ⟺ rust sell2 置位"
    );
    assert_eq!(
        points[0].pivot_high, 8,
        "rust S2 止损源 pivot_high == Lean secondPointPrice Side.short m2（m2.hi=8）"
    );
}
...
fn type2_empty_for_l0_segment_matches_lean() {
    ...
    assert!(
        points.is_empty(),
        "Lean segment_no_secondType ⟺ rust L0 线段无 B2（递归底诚实空）"
    );
}
