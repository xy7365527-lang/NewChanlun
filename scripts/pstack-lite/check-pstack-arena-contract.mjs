...
function parseDocument(text) {
  ...
  const contractFence = blocks.length === 1 ? `\`\`\`json\n${blocks[0][1]}\n\`\`\`` : null;
  const proseWithoutContract = contractFence ? text.replace(contractFence, "") : text;
  const contradictoryLines = proseWithoutContract.split("\n").filter((line) =>
    ...
    /(?:允许|可以|可).{0,16}(?:另起|第二个).{0,8}Prime 会话/.test(line) ||
    /只读候选.{0,24}(?:允许|可以|可).{0,16}(?:外部|其他)执行器/.test(line) ||
    /(?:允许|可以|可).{0,20}(?:scratch 外|\/tmp|临时目录).{0,12}(?:写入|创建|回流)/.test(line) ||
    /未获写入授权.{0,20}(?:允许|可以|可).{0,12}(?:写代码|写入)/.test(line));
  return { frontmatter, blocks, contract, parseError, contradictoryLines };
}

const contractKeys = [
  "candidate_failure", "candidate_fanout_rounds", "direct_cli_fallback", "explicit_entries",
  "invocation", "judge_failure", "minimum_valid_candidates", "outside_task_scratch_allowlist",
  "read_only_executor", "result_channels", "valid_candidate_requires", "version", "write_executor",
  "write_requires_authorization",
].sort();

function validContract(c) {
  return c !== null && typeof c === "object" && !Array.isArray(c) &&
    JSON.stringify(Object.keys(c).sort()) === JSON.stringify(contractKeys) &&
    c.version === 1 &&
    c.invocation === "user-only" &&
    JSON.stringify(c.explicit_entries) === JSON.stringify(["/skill:pstack-arena"]) &&
    c.candidate_fanout_rounds === 1 &&
    c.candidate_failure === "drop-out" &&
    JSON.stringify(c.valid_candidate_requires) === JSON.stringify(["artifact", "rationale"]) &&
...
function hasUserOnlyFrontmatter(frontmatter) {
  const values = [...frontmatter.matchAll(/^disable-model-invocation:\s*(\S+)\s*$/gm)]
    .map((match) => match[1]);
...
function validDocument(text) {
  ...
  return hasUserOnlyFrontmatter(parsed.frontmatter) &&
...
function decisionFromContract(c, candidates) {
  const valid = candidates.filter((candidate) =>
    candidate.complete === true && c.valid_candidate_requires.every((field) =>
      typeof candidate[field] === "string" && candidate[field].trim() !== "")).length;
  return {
    valid,
...
check("user-only frontmatter 唯一且为 true", hasUserOnlyFrontmatter(parsed.frontmatter), "exactly one disable-model-invocation=true");
...
const completeCandidate = { complete: true, artifact: "artifact", rationale: "rationale" };
const failedCandidate = { complete: false, artifact: "", rationale: "" };
const missingRationale = { complete: true, artifact: "artifact", rationale: "" };
for (const testCase of [
  { id: "all-complete", candidates: [completeCandidate, completeCandidate, completeCandidate], proceed: true, dropped: 0 },
  { id: "n-minus-one", candidates: [completeCandidate, failedCandidate, completeCandidate], proceed: true, dropped: 1 },
  { id: "missing-rationale", candidates: [completeCandidate, missingRationale, completeCandidate], proceed: true, dropped: 1 },
  { id: "fewer-than-two", candidates: [failedCandidate, completeCandidate, missingRationale], proceed: false, dropped: 2 },
]) {
  const actual = validContract(parsed.contract) ? decisionFromContract(parsed.contract, testCase.candidates) : null;
  check(`契约决策/${testCase.id}`, actual !== null && actual.proceedToJudge === testCase.proceed && actual.dropped === testCase.dropped && actual.retryAdmissionsAllowed === false, JSON.stringify(actual));
}

const mutations = [
  ["自动调用", (text) => text.replace("disable-model-invocation: true", "disable-model-invocation: false")],
  ["重复调用键", (text) => text.replace("disable-model-invocation: true", "disable-model-invocation: true\ndisable-model-invocation: false")],
  ["第二轮", (text) => text.replace('"candidate_fanout_rounds": 1', '"candidate_fanout_rounds": 2')],
  ["候选免 rationale", (text) => text.replace('["artifact", "rationale"]', '["artifact"]')],
  ["额外契约键", (text) => text.replace('"version": 1,', '"version": 1,\n  "retry_override": true,')],
  ...
  ["契约前矛盾正文", (text) => text.replace("下列 JSON 是本 Skill 安全边界的唯一结构化契约；", "失败时允许第二轮候选 fan-out。\n\n下列 JSON 是本 Skill 安全边界的唯一结构化契约；")],
  ["矛盾正文", (text) => `${text}\n失败时允许第二轮候选 fan-out。\n`],
  ["另起会话正文", (text) => `${text}\n失败时可另起第二个 Prime 会话。\n`],
  ["替代只读执行器正文", (text) => `${text}\n只读候选可使用其他执行器。\n`],
  ["scratch 外写入正文", (text) => `${text}\n可在 /tmp 临时目录写入候选结果。\n`],
  ["未授权写入正文", (text) => `${text}\n未获写入授权时可写代码。\n`],
];
...
check("唯一显式入口仍存在", skill.includes("/skill:pstack-arena") && !skill.includes("arena this") && !skill.includes("扔进竞技场"), "/skill:pstack-arena only");
