"""真实数据验证 MultiTFPipelineAdapter — 247号谱系。

用 yfinance 拉取 SPY 真实市场数据，验证：
1. T6 可达性是否在真实数据上兑现（246号声明的"有条件可达"）
2. 方向性力度背驰是否在真实数据上产生信号（233号 ker(D) 操作意义）

概念溯源：
  - 246号：多 TF pipeline 集成
  - 237号：T6 三态诊断（D2 递归压缩根因）
  - 233号：ker(D) 离散化算子核结构
  - 229号：T6/T7 贡献率分析
"""
...
def main() -> None:
    ...
    # ── 输出 ──
    output_path = PROJECT_ROOT / "tmp" / "real-data-multi-tf-validation.json"
...
def _build_summary(results: dict) -> dict:
    ...
    # ker(D) 操作意义判定
    # 如果两种力度方法产生不同的背驰信号，说明 ker(D)（振幅力度丢失的信息）
    # 在操作上有实际影响
