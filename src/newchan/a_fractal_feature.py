...
def _classify_feature_fractal(
    ...
    if (
        ...
        return FeatureFractal(idx=idx, kind="top", high=h_curr, low=l_curr)
    if (
        l_curr < l_prev and l_curr < l_next
        and h_curr < h_prev and h_curr < h_next
    ):
        return FeatureFractal(idx=idx, kind="bottom", high=h_curr, low=l_curr)
    return None
...
def fractals_from_feature(seq: list[FeatureBar]) -> list[FeatureFractal]:
    ...
    for i in range(1, n - 1):
        f = _classify_feature_fractal(
            seq[i - 1].high, seq[i].high, seq[i + 1].high,
            seq[i - 1].low, seq[i].low, seq[i + 1].low,
            idx=i,
        )
        if f is not None:
            result.append(f)
