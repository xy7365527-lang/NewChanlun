...
- 盘整 = 包含恰好 1 个走势中枢
- 趋势 = 包含 2+ 个依次同向走势中枢
  - 上涨：C2.DD > C1.GG（后枢波动区间下沿 严格高于 前枢波动区间上沿，中心定理二）
  - 下跌：C2.GG < C1.DD（后枢波动区间上沿 严格低于 前枢波动区间下沿，中心定理二）
...
def _is_ascending(c1: Zhongshu, c2: Zhongshu) -> bool:
    """后枢 DD 严格高于 前枢 GG → 上涨延续（波动区间递升，中心定理二）。"""
    return c2.dd > c1.gg


def _is_descending(c1: Zhongshu, c2: Zhongshu) -> bool:
    """后枢 GG 严格低于 前枢 DD → 下跌延续（波动区间递降，中心定理二）。"""
    return c2.gg < c1.dd
