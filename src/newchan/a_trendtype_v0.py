...
- 走势 = 一维 CW 复形上从起点 a 到终点 b 的有向离散路径（笔链的序列）
- 走势类型 = 路径的组合分类，由中枢的组合数据决定：
  - 趋势（≥2 同向中枢）= 穿越多个区间交集的路径类
  - 盘整（1 中枢）= 被单个区间交集约束的路径类
- 走势分解定理"任何走势 = 上涨/下跌/盘整的连接"= 有向图路径的分类定理
- centers_relation_by_gg_dd() = 相邻区间交集之间的连接关系判定
- 同向中枢分组 = 路径类的粗分类（方向不变 = 同一类）

### 映射的边界
走势不是环路（loop）——它有起点和终点，不闭合。因此 π₁（基本群）不直接适用。
分类由中枢的有限组合数据（数量、方向、区间关系）确定，是有限组合分类，
不需要连续拓扑空间的基本群机制。"路径空间"在拓扑学中特指连续映射空间 X^I，
此处的离散步进序列不构成该意义下的路径空间——准确描述是有向图上的游走分类。
"""
