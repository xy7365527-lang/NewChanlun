...
def _make_two_moves_v1():
    ...
    segs_up = [
        ...
        _seg(8, 8, 80, 90, "up", 34, 26),
        _seg(9, 9, 90, 100, "down", 34, 28),
        # B2（C 段越界极值）后窗口延伸到下一 settled 中枢（zs2，seg 11..13）：
        # 原 high=35 与 Move 1 起点 95 价格不连续，B2 会把 seg11 的 95 误判为
        # 上涨极值（合成数据缺陷，非定义缺陷）。补成 96 使顶点真实落在 Move 0
        # 离开段上（价格连续：96 冲顶 → 95 开始下跌），保持本 fixture
        # "两个 move 各有一个背驰" 的测试意图。
        _seg(10, 10, 100, 110, "up", 96, 33),
    ]
