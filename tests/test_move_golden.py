...
# =====================================================================
# 16) DD/GG 外缘分离区分趋势（#815 M-2 转正 + #898 落地）
# =====================================================================

class TestZDZGTrendDistinction:
    """趋势判断使用波动区间外缘 DD/GG 分离（#815 M-2 + #898，「后DD>前GG
    等价于上涨及其延续」020:58）。

    核心分离（ZD/ZG）但外缘仍重叠 = 中枢扩展、升一级，**不是**趋势；
    与 Rust `moves.rs`/`level.rs` 的 #898 测试锁同口径。
    """

    def test_dd_above_gg_is_trend(self):
        """C2.DD > C1.GG → 上涨趋势（外缘分离递升）。

        C1: zd=10, zg=18（外缘 dd=5, gg=25）
        C2: zd=27, zg=33（外缘 dd=26, gg=36）
        C2.dd=26 > C1.gg=25 → 上涨
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
            _zs(6, 8, 27.0, 33.0, break_direction="up", dd=26.0, gg=36.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 1
        assert moves[0].kind == "trend"
    ...
    def test_core_separation_with_envelope_overlap_is_not_trend(self):
        """核心分离但外缘重叠 → 中枢扩展（升一级），非趋势（#815 M-2 / #898）。

        C1: zd=10, zg=18（外缘 dd=5, gg=25）
        C2: zd=20, zg=28（外缘 dd=15, gg=35）
        C2.zd=20 > C1.zg=18（核心分离）但 C2.dd=15 <= C1.gg=25（外缘重叠）→ 非趋势
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
    ...
    def test_dd_not_above_gg_is_not_trend(self):
        """C2.DD <= C1.GG → 非趋势（外缘未分离）。

        C1: zd=10, zg=22（外缘 dd=5, gg=25）
        C2: zd=20, zg=28（外缘 dd=15, gg=35）
        C2.dd=15 < C1.gg=25 → 非趋势
        """
        zhongshus = [
    ...
    def test_dd_above_gg_ascending(self):
        ...
        C1: zd=10, zg=18（外缘 dd=5, gg=22）
        C2: zd=25, zg=33（外缘 dd=23, gg=38）
    ...
    def test_gg_below_dd_descending(self):
        """C2.GG < C1.DD → 下跌趋势（外缘分离递降）。

        C1: zd=20, zg=30（外缘 dd=18, gg=35）
        C2: zd=5, zg=15（外缘 dd=3, gg=17）
