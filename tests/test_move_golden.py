...
class TestGGDDTrendDistinction:
    ...
    def test_zdzg_separated_but_ddgg_overlapping_is_not_trend(self):
        """ZD/ZG 分离但 DD/GG 重叠 → 高级别中枢扩张，不是趋势。

        C1: zd=10, zg=18, dd=5,  gg=25
        C2: zd=20, zg=28, dd=15, gg=35
        C2.zd=20 > C1.zg=18，但 C2.dd=15 <= C1.gg=25。
        """
        zhongshus = [
    ...
    def test_dd_above_gg_is_trend(self):
        ...
        C1: zd=10, zg=18, dd=5,  gg=25
        C2: zd=30, zg=38, dd=26, gg=45
        C2.dd=26 > C1.gg=25 → 上涨
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
            _zs(6, 8, 30.0, 38.0, break_direction="up", dd=26.0, gg=45.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 1
        assert moves[0].kind == "trend"
    ...
    def test_dd_not_above_gg_is_not_trend(self):
        ...
        C1: zd=10, zg=22, dd=5,  gg=25
        C2: zd=26, zg=34, dd=24, gg=40
        C2.dd=24 <= C1.gg=25 → 非趋势
        """
        zhongshus = [
            _zs(0, 2, 10.0, 22.0, break_direction="up", dd=5.0, gg=25.0),
            _zs(6, 8, 26.0, 34.0, break_direction="up", dd=24.0, gg=40.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 2
        assert moves[0].kind == "consolidation"
    ...
    def test_dd_equal_gg_boundary_is_not_trend(self):
        """C2.DD == C1.GG → 非趋势（严格不满足递升）。"""
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
            _zs(6, 8, 26.0, 34.0, break_direction="up", dd=25.0, gg=40.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 2
        assert moves[0].kind == "consolidation"
