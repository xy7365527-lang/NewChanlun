...
class TestGGDDTrendDistinction:
    ...
    def test_dd_above_gg_is_trend(self):
        ...
        C1: zd=10, zg=18
        C2: zd=20, zg=28
        C1.gg=18, C2.dd=20 → 上涨
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=10.0, gg=18.0),
            _zs(6, 8, 20.0, 28.0, break_direction="up", dd=20.0, gg=28.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 1
        assert moves[0].kind == "trend"
    ...
    def test_fixed_interval_separation_with_overlapping_wave_interval_is_not_trend(self):
        """ZD/ZG 分离但 GG/DD 重叠 → 更高级别中枢/扩展，不是趋势。

        C1: zd=10, zg=22
        C2: zd=25, zg=33
        C2.zd=25 > C1.zg=22，但 C2.dd=18 <= C1.gg=30 → 非趋势
        """
        zhongshus = [
            _zs(0, 2, 10.0, 22.0, break_direction="up", dd=5.0, gg=30.0),
            _zs(6, 8, 25.0, 33.0, break_direction="up", dd=18.0, gg=40.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 2
        assert moves[0].kind == "consolidation"
