...
class TestWaveRangeTrendDistinction:
    ...
    def test_zd_above_zg_is_trend(self):
        ...
        C1: zd=10, zg=18
        C2: zd=20, zg=28
        C2.dd=30 > C1.gg=25 → 上涨
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
            _zs(6, 8, 20.0, 28.0, break_direction="up", dd=30.0, gg=35.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 1
        assert moves[0].kind == "trend"
    ...
    def test_fixed_range_separates_but_wave_range_overlaps_is_not_uptrend(self):
        """C2.ZD > C1.ZG 但 C2.DD <= C1.GG → 非上涨趋势。

        固定区间分离但波动区间仍重叠时，应升级/扩张处理，
        不能直接降格为同级别上涨趋势。
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
    ...
    def test_fixed_range_separates_but_wave_range_overlaps_is_not_downtrend(self):
        """C2.ZG < C1.ZD 但 C2.GG >= C1.DD → 非下跌趋势。"""
        zhongshus = [
            _zs(0, 2, 20.0, 30.0, break_direction="down", dd=18.0, gg=35.0),
