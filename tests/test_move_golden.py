...
class TestGGDDTrendDistinction:
    """趋势判断使用 GG/DD 波动区间（中心定理二）。"""

    def test_zd_above_zg_but_wave_overlap_is_not_trend(self):
        """C2.ZD > C1.ZG 但 C2.DD <= C1.GG → 非本级别趋势。

        C1: zd=10, zg=18
        C2: zd=20, zg=28
        固定区间分离，但 C2.dd=15 <= C1.gg=25 → 波动区间重叠。
        """
        zhongshus = [
    ...
    def test_wave_overlap_is_not_trend(self):
    ...
    def test_zg_below_zd_but_wave_overlap_is_not_trend(self):
        """C2.ZG < C1.ZD 但 C2.GG >= C1.DD → 非本级别趋势。"""
        zhongshus = [
            _zs(0, 2, 20.0, 30.0, break_direction="down", dd=18.0, gg=35.0),
            _zs(6, 8, 5.0, 15.0, break_direction="down", dd=3.0, gg=20.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 2
        assert moves[0].kind == "consolidation"
    ...
    def test_wave_descending(self):
        """C2.GG < C1.DD → 下跌趋势（中心定理二）。
