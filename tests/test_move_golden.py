...
class TestGGDDTrendDistinction:
    ...
    def test_zd_above_zg_but_dd_not_above_gg_is_not_trend(self):
        """后 ZD > 前 ZG 但后 DD <= 前 GG → 更高级别中枢，不是上涨趋势。
    ...
    def test_dd_above_gg_is_trend_even_when_zd_not_above_zg(self):
        """后 DD > 前 GG → 上涨趋势，即使固定区间 ZD/ZG 未完全分离。

        C1: zd=10, zg=22, gg=25
        C2: zd=20, zg=28, dd=26
        C2.zd=20 <= C1.zg=22，但 C2.dd=26 > C1.gg=25 → 上涨
        """
        zhongshus = [
