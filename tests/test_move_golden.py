...
class TestGGDDTrendDistinction:
    """趋势判断使用波动区间 GG/DD（含离开段极值），而非固定区间 ZD/ZG。"""

    def test_dd_above_gg_is_trend(self):
        """C2.DD > C1.GG → 上涨趋势（波动区间递升）。

        C1: zd=10, zg=18, dd=5,  gg=18
        C2: zd=20, zg=28, dd=19, gg=35
        C2.dd=19 > C1.gg=18 → 上涨
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=18.0),
            _zs(6, 8, 20.0, 28.0, break_direction="up", dd=19.0, gg=35.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 1
        assert moves[0].kind == "trend"
    ...
    def test_zd_zg_separated_but_gg_dd_overlaps_is_not_trend(self):
        """ZD/ZG 分离但 GG/DD 重叠 → 非趋势（更高级别中枢候选，非同向趋势）。

        C1: zd=10, zg=18, dd=5,  gg=25
        C2: zd=20, zg=28, dd=15, gg=35
        C2.zd=20 > C1.zg=18 但 C2.dd=15 <= C1.gg=25 → 非趋势
        """
        zhongshus = [
    ...
    def test_dd_equal_gg_is_not_trend(self):
        """C2.DD == C1.GG → 非趋势（严格递升）。"""
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=25.0),
            _zs(6, 8, 20.0, 28.0, break_direction="up", dd=25.0, gg=35.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 2
        assert moves[0].kind == "consolidation"
