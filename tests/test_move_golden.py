...
class TestGGDDTrendDistinction:
    """趋势判断使用波动区间 DD/GG，避免把高级别中枢误判为同级趋势。"""

    def test_zd_above_zg_but_gg_dd_overlap_is_not_trend(self):
        """ZD/ZG 不重叠但 GG/DD 重叠 → 非趋势（形成更高级别中枢）。

        C1: zd=10, zg=18, dd=5, gg=25
        C2: zd=20, zg=28, dd=15, gg=35
        ZD比较: C2.zd=20 > C1.zg=18
        GG/DD比较: C2.dd=15 <= C1.gg=25 → 波动区间重叠
        """
        zhongshus = [
    ...
    def test_zd_overlap_and_gg_dd_overlap_is_not_trend(self):
        """固定区间与波动区间都重叠 → 非趋势。

        C1: zd=10, zg=22, dd=5, gg=25
        C2: zd=20, zg=28, dd=15, gg=35
        C2.dd=15 <= C1.gg=25 → 波动区间重叠
        """
        zhongshus = [
