...
class TestGGDDTrendDistinction:
    ...
    def test_dd_above_gg_is_trend(self):
        ...
        C1: dd=5, gg=18
        C2: dd=20, gg=35
        C2.dd=20 > C1.gg=18 → 上涨
        """
        zhongshus = [
            _zs(0, 2, 10.0, 18.0, break_direction="up", dd=5.0, gg=18.0),
            _zs(6, 8, 20.0, 28.0, break_direction="up", dd=20.0, gg=35.0),
        ]
        moves = moves_from_zhongshus(zhongshus)
        assert len(moves) == 1
        assert moves[0].kind == "trend"
    ...
    def test_fixed_interval_separation_with_overlapping_wave_range_is_not_trend(self):
        """ZD/ZG 分离但 GG/DD 重叠 → 非趋势（级别扩张）。

        C1: zd/zg=10/18, dd/gg=5/25
        C2: zd/zg=20/28, dd/gg=15/35
