...
谬误）。不擅自发明 ⟹ 维持 Python 严格口径（#290 裁定 B）。

下游真空照实标注：单点中枢若成立，其下游（定理二分支/三类买卖点/破坏/092 监视器
Z 值）全落原文真空。**输入域实测（ZG==ZD 输入，落码口径）**：OKLO **0 次**、
BZ2024 **2 次**（均在 L1）；Python 两版（v0 `a_center_v0._try_init_center` /
v1 `a_zhongshu_v1._scan_zhongshu`）**均判不成立** ⟹ 单点中枢成立分支及其下游真空
**在 Python 链上不激活**。

⚠**不可推广到 Rust**：Rust θ v0 **生产链是弱口径**——
`rust/src/theta_v0/classifier/center.rs` 的 `center_from_segments`（`:162`）/
`center_from_window`（`:192`）两处均为 `if zd > zg { return None }`（单点
`zd==zg` 通过），同文件 `:152` doc 明写「`ZD_B=ZG_B` 单点核心合法」。故该分支在
Rust 生产链上**可达且判成立**，下游真空是**激活的**。此矛盾（Lean 弱 / Python 严格 /
Rust 弱）**超出 #290 裁定 B 的有效域**，已另立票 **#321**（成立口径三方矛盾，
grilling）。
