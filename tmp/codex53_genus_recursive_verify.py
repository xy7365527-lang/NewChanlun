#!/usr/bin/env python3
"""Codex 5.3 High: verify editor's genus construction + recursive topology."""
...
### 6. 与 Codex 之前设计的方案的关系
你之前设计了 zigzag PH + 层理论 + 离散 Morse 的方案（6个定理）。亏格构造与这些方案是互补的、冗余的、还是矛盾的？

## Gemini 3.1 Pro 同时在验证同一构造。给出独立的严格判断。

**如果构造有错，直接指出。如果某个声明需要额外条件才能成立，给出精确条件。
不允许"大致成立"——要么成立（给证明），要么不成立（给反例），要么需要条件（给条件）。**
"""

print("Calling Codex 5.3 High: genus construction + recursive topology verification...")
