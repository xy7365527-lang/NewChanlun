"""Atomically create 107/108/109 genealogy files and update dag.yaml.

106 is already taken by 'governance bias crystallization'.
Next available: 107=zhongshu, 108=beichi, 109=maimai.
"""
import yaml
...
- **创建**: `.chanlun/genealogy/settled/107-zhongshu-definition-choices.md`
- **不修改**任何定义文件或代码文件
...
- **创建**: `.chanlun/genealogy/settled/108-beichi-definition-choices.md`
- **不修改**任何定义文件或代码文件

## 谱系引用

- 004号溯源框架、005b号对象否定对象、107号中枢定义选择、068号范式转换
"""
...
109号#8（"不跌破"的 `>` vs `>=`）与 107号#4（延伸弱不等式）形成一组相关选择。若遵循同样逻辑（"不跌破"="不低于"=`>=`），则 #8 应改为 `>=`。值得在下一轮源头审计中解决。
...
- **创建**: `.chanlun/genealogy/settled/109-maimai-definition-choices.md`
- **不修改**任何定义文件或代码文件

## 谱系引用

- 004号溯源框架、005b号对象否定对象、107号中枢、108号背驰
"""
...
def main():
    ...
    (SETTLED / "107-zhongshu-definition-choices.md").write_text(FILE_107.strip() + "\n", encoding="utf-8")
    (SETTLED / "108-beichi-definition-choices.md").write_text(FILE_108.strip() + "\n", encoding="utf-8")
    (SETTLED / "109-maimai-definition-choices.md").write_text(FILE_109.strip() + "\n", encoding="utf-8")
    print("3 genealogy files written (107, 108, 109).")
    ...
    new_nodes = [
        {"id": "107", "title": "中枢定义选择谱系——定义到实现的溯源追踪", "status": "已结算", "type": "语法记录", "file": "settled/107-zhongshu-definition-choices.md"},
        {"id": "108", "title": "背驰定义选择谱系——定义到实现的溯源追踪", "status": "已结算", "type": "语法记录", "file": "settled/108-beichi-definition-choices.md"},
        {"id": "109", "title": "买卖点定义选择谱系——定义到实现的溯源追踪", "status": "已结算", "type": "语法记录", "file": "settled/109-maimai-definition-choices.md"},
    ]
    ...
    for n in new_nodes:
        if n["id"] not in existing_ids:
            ...
            print(f"Node {n['id']} already exists, skipping")
