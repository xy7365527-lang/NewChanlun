"""
Commodity节点折叠内在性检验（复制Au/$节点第三轮方法论）
==================================================
K4拓扑中Commodity节点连着三条边：
  e1 = CL=F / DXY  (Commodity/$)
  e2 = CL=F / GC=F (Commodity/Au)
  e3 = CL=F / SPX   (Commodity/Equity)

CL=F（原油期货）是Commodity节点的直接代理变量。

假说：如果Commodity是"普通节点"，应与Au一致——
  下降压缩(r<0) + 上升散开(r>0)。
如果与$节点一致——下降压缩 + 上升不散开——则$节点的不对称性不是$特有。
...
print("Downloading data for Commodity node (CL=F)...")
clf = _dl("CL=F")
dxy = _dl("DX-Y.NYB")
gold = _dl("GC=F")
spx = _dl("^GSPC")

df = pd.DataFrame({"clf": clf, "dxy": dxy, "gold": gold, "spx": spx}).dropna()

# === Special handling: 2020-04-20 negative oil price ===
# CL=F closed negative that day -> log undefined. Mark as NaN and drop.
neg_mask = df["clf"] <= 0
if neg_mask.any():
    neg_dates = df.index[neg_mask].tolist()
    print(f"WARNING: Negative/zero CL=F prices on {len(neg_dates)} days, dropping: "
          f"{[str(d.date()) for d in neg_dates]}")
    df = df[~neg_mask]

# Commodity node edges: ratios with CL=F as numerator
e1 = df["clf"] / df["dxy"]     # Commodity/$
e2 = df["clf"] / df["gold"]    # Commodity/Au
e3 = df["clf"] / df["spx"]     # Commodity/Equity

returns = pd.DataFrame({
    "e1_clf_dxy": np.log(e1).diff(),
    "e2_clf_gold": np.log(e2).diff(),
    "e3_clf_spx": np.log(e3).diff(),
}).dropna()

# Drop any rows with inf/nan from log of near-zero ratios
...
# ============================================================
# 4. CL=F Trend Identification (TWO METHODS)
# ============================================================
print("\n" + "=" * 70)
print("CL=F TREND IDENTIFICATION")
print("=" * 70)

# --- Method A: 252-day MA crossover (objective, automatic) ---
clf_aligned = df["clf"].reindex(eff_dim_series.index)
ma252 = clf_aligned.rolling(252).mean()
...
above_rising = (clf_aligned > ma252) & (ma252_slope > 0)
below_falling = (clf_aligned < ma252) & (ma252_slope < 0)
...
manual_trend_periods = [
    ("2001-01-01", "2008-07-15", "T", "上升趋势1(超级商品周期)"),
    ("2008-07-16", "2009-02-28", "T", "下降趋势1(金融危机暴跌)"),
    ("2009-03-01", "2011-04-30", "T", "上升趋势2(QE1反弹)"),
    ("2011-05-01", "2012-06-30", "N", "盘整1"),
    ("2012-07-01", "2016-02-15", "T", "下降趋势2(页岩革命+OPEC价格战)"),
    ("2016-02-16", "2018-10-15", "T", "上升趋势3(OPEC减产)"),
    ("2018-10-16", "2020-04-30", "T", "下降趋势3(COVID暴跌)"),
    ("2020-05-01", "2022-06-15", "T", "上升趋势4(疫后复苏+俄乌)"),
    ("2022-06-16", "2023-12-31", "T", "下降趋势4(需求放缓)"),
    ("2024-01-01", "2026-02-28", "N", "盘整/低位"),
]

# Separate rising and falling for asymmetry analysis
manual_rising_periods = [
    ("2001-01-01", "2008-07-15", "上升趋势1(超级商品周期)"),
    ("2009-03-01", "2011-04-30", "上升趋势2(QE1反弹)"),
    ("2016-02-16", "2018-10-15", "上升趋势3(OPEC减产)"),
    ("2020-05-01", "2022-06-15", "上升趋势4(疫后复苏+俄乌)"),
]
manual_falling_periods = [
    ("2008-07-16", "2009-02-28", "下降趋势1(金融危机暴跌)"),
    ("2012-07-01", "2016-02-15", "下降趋势2(页岩革命+OPEC价格战)"),
    ("2018-10-16", "2020-04-30", "下降趋势3(COVID暴跌)"),
    ("2022-06-16", "2023-12-31", "下降趋势4(需求放缓)"),
]
...
# ============================================================
# 5. PROPOSITION 1: eff.dim x CL=F trend state (BOTH METHODS)
# ============================================================
...
for start, end, state, name in manual_trend_periods:
    ...
    if len(seg) > 0:
        print(f"  {name:<40} ({state}): n={len(seg):>4}, median={seg.median():.3f}, "
              f"mean={seg.mean():.3f}, range=[{seg.min():.3f}, {seg.max():.3f}]")

# ============================================================
# 7. KEY TEST: Rising vs Falling CL=F trend asymmetry
# ============================================================
print("\n" + "=" * 70)
print("ASYMMETRY TEST: Rising vs Falling CL=F trends")
print("=" * 70)
...
print(f"\nRising CL=F (oil price up):    n={len(rising_data)}, median={rising_data.median():.3f}, "
      f"mean={rising_data.mean():.3f}, IQR=[{rising_data.quantile(0.25):.3f}, {rising_data.quantile(0.75):.3f}]")
print(f"Falling CL=F (oil price down): n={len(falling_data)}, median={falling_data.median():.3f}, "
...
if len(rising_data) >= 10 and len(falling_data) >= 10:
    ...
    # Comparison with Au and $ nodes:
    # Au: rising trend -> higher eff.dim (散开), falling trend -> lower eff.dim (压缩) -- SYMMETRIC
    # $:  falling DXY -> lower eff.dim (压缩), rising DXY -> NOT higher (不散开) -- ASYMMETRIC
    # Commodity prediction: if "普通节点" -> SAME_AS_AU; if $ pattern -> SAME_AS_DOLLAR
    if rf_p < 0.05:
        if rf_d > 0:
            asym_verdict = "SAME_AS_AU"
            asym_desc = "上升散开/下降压缩：与Au节点同向（对称）-> Commodity是普通节点"
        else:
            asym_verdict = "OPPOSITE_TO_AU"
            asym_desc = "上升压缩/下降散开：与Au节点反向 -> 节点特异性"
    else:
        # No significant difference -- could be like $ (asymmetric: compress but no spread)
        # Check if falling shows compression
        falling_time_indices = np.arange(len(falling_data))
        sp_fall_r, sp_fall_p = stats.spearmanr(falling_time_indices, falling_data.values)
        rising_time_indices = np.arange(len(rising_data))
        sp_rise_r, sp_rise_p = stats.spearmanr(rising_time_indices, rising_data.values)

        if sp_fall_r < -0.1 and sp_fall_p < 0.1 and abs(sp_rise_r) < 0.1:
            asym_verdict = "SAME_AS_DOLLAR"
            asym_desc = (f"下降压缩(fall Spearman r={sp_fall_r:.3f}) + 上升不散开(rise r={sp_rise_r:.3f})"
                        f" -> 与$节点同型（不对称）")
        else:
            asym_verdict = "NO_ASYMMETRY"
            asym_desc = (f"无显著不对称 (p={rf_p:.4f}, d={rf_d:.4f})"
                        f" -> 不对称可能是节点特异或效应量不足")
...
for start, end, state, name in manual_trend_periods:
    ...
    print(f"  {name:<40} ({state}): {seg_days:>4} days, {seg_ev_count} events, rate={rate:.2f}/1000d")
...
# Cross-node comparison: Au min=1.894 (6.92%), $ min=1.894 (6.22%)
print(f"\n--- Cross-node comparison (Au vs $ vs Commodity) ---")
print(f"  Au        node: min event eff.dim = 1.894, frac below = 6.92%")
print(f"  $         node: min event eff.dim = 1.894, frac below = 6.22%")
print(f"  Commodity node: min event eff.dim = {min_event_ed:.3f}, frac below = {frac_below*100:.2f}%")

# Convergence judgment
threshold_converge = abs(min_event_ed - 1.894) < 0.15
if threshold_converge:
    thresh_verdict = "CONVERGE_WITH_AU_DOLLAR"
    thresh_desc = f"三节点汇合 (Au=1.894, $=1.894, Commodity={min_event_ed:.3f})"
else:
    thresh_verdict = "DIVERGE"
    thresh_desc = f"Commodity阈值偏离 (Au=1.894, $=1.894, Commodity={min_event_ed:.3f}, delta={abs(min_event_ed-1.894):.3f})"
print(f"\nThreshold convergence verdict: {thresh_verdict}")
print(f"  {thresh_desc}")
...
results = {
    "node": "commodity",
    "proxy": "CL=F (WTI Crude Oil Futures)",
    "edges": ["CL=F/DXY (Commodity/$)", "CL=F/GC=F (Commodity/Au)", "CL=F/^GSPC (Commodity/Equity)"],
    "data_range": f"{returns.index[0].date()} to {returns.index[-1].date()}",
    "n_obs": len(returns),
    "negative_price_handling": "CL=F <= 0 days dropped (2020-04-20 negative oil price)",
    "eff_dim_summary": {
    ...
    "asymmetry": {
        ...
        "cross_node_comparison": {
            "au_pattern": "rising_gold=散开(higher eff.dim), falling_gold=压缩(lower eff.dim) -- SYMMETRIC",
            "dollar_pattern": "falling_dxy=压缩, rising_dxy=不散开 -- ASYMMETRIC",
            "commodity_pattern": f"rising_clf mean={rising_data.mean():.3f}, falling_clf mean={falling_data.mean():.3f}",
        },
    },
    "proposition_2": {
    ...
    "threshold_effect": {
        ...
        "cross_node_comparison": {
            ...
            "dollar_frac_below": 0.0622,
            "commodity_min_event_effdim": round(min(eds), 3),
            "commodity_frac_below": round(float(frac_below), 4),
        },
        "convergence_verdict": thresh_verdict,
        "convergence_description": thresh_desc,
    },
}
...
results["final_verdicts"] = {
    ...
    "threshold_convergence": thresh_verdict,
    "proposition_1": p1_combined,
    "proposition_2": p2v,
    "hypothesis_test": (
        "Commodity与Au同型（普通节点对称）" if asym_verdict == "SAME_AS_AU" else
        "Commodity与$同型（不对称）" if asym_verdict == "SAME_AS_DOLLAR" else
        "Commodity与Au反向" if asym_verdict == "OPPOSITE_TO_AU" else
        "证据不足"
    ),
}

output_path = OUTPUT_DIR / "r3_commodity_node_results.json"
...
print(f"\nAsymmetry (核心问题 - Commodity是普通节点还是特殊节点):")
print(f"  Verdict: {asym_verdict}")
print(f"  {asym_desc}")
print(f"\nThreshold (三节点汇合检验):")
print(f"  Au        node: min event eff.dim = 1.894")
print(f"  $         node: min event eff.dim = 1.894")
print(f"  Commodity node: min event eff.dim = {min_event_ed:.3f}")
print(f"  Convergence: {thresh_verdict}")
print(f"  {thresh_desc}")
print(f"\nHypothesis test: {results['final_verdicts']['hypothesis_test']}")
