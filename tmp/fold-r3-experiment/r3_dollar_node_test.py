"""
$节点折叠内在性检验（复制Au节点第三轮方法论）
==================================================
K4拓扑中$节点连着三条边：$/Au, $/equity(SPX), $/commodity(Cu)
DXY是$节点的直接代理变量。

问题1（不对称性来源 - 323号推论3）：下降压缩/上升散开是Au特殊性质还是K4一般性质？
问题2（阈值效应确认 - 323号推论2）：$节点是否存在类似的eff.dim下界？

认识论等级：L2（真实数据验证）
谱系引用：323号（折叠理论结算）、321号（第三轮判决）、292号（折叠本体论）
"""
...
print("Downloading data for $ node...")
dxy = _dl("DX-Y.NYB")
gold = _dl("GC=F")
spx = _dl("^GSPC")
copper = _dl("HG=F")

df = pd.DataFrame({"dxy": dxy, "gold": gold, "spx": spx, "copper": copper}).dropna()

# $ node edges: ratios with DXY as numerator
e1 = df["dxy"] / df["gold"]      # $/Au
e2 = df["dxy"] / df["spx"]       # $/equity
e3 = df["dxy"] / df["copper"]    # $/commodity

returns = pd.DataFrame({
    "e1_dxy_gold": np.log(e1).diff(),
    "e2_dxy_spx": np.log(e2).diff(),
    "e3_dxy_copper": np.log(e3).diff(),
}).dropna()
...
# ============================================================
# 4. DXY Trend Identification (TWO METHODS)
# ============================================================
print("\n" + "=" * 70)
print("DXY TREND IDENTIFICATION")
print("=" * 70)

# --- Method A: 252-day MA crossover (objective, automatic) ---
dxy_aligned = df["dxy"].reindex(eff_dim_series.index)
ma252 = dxy_aligned.rolling(252).mean()
...
above_rising = (dxy_aligned > ma252) & (ma252_slope > 0)
below_falling = (dxy_aligned < ma252) & (ma252_slope < 0)
...
manual_trend_periods = [
    ("2001-01-01", "2008-03-01", "T", "下降趋势1(美元走弱)"),
    ("2008-03-02", "2008-11-30", "T", "上升趋势1(金融危机避险)"),
    ("2008-12-01", "2011-04-30", "T", "下降趋势2(继续走弱)"),
    ("2011-05-01", "2014-06-30", "N", "盘整→启动"),
    ("2014-07-01", "2016-12-31", "T", "上升趋势2(强美元)"),
    ("2017-01-01", "2018-01-31", "T", "下降趋势3(回调)"),
    ("2018-02-01", "2020-02-28", "N", "盘整偏强"),
    ("2020-03-01", "2021-05-31", "T", "下降趋势4(QE)"),
    ("2021-06-01", "2022-09-30", "T", "上升趋势3(加息)"),
    ("2022-10-01", "2023-12-31", "T", "下降趋势5(见顶回落)"),
    ("2024-01-01", "2026-02-28", "N", "高位震荡"),
]

# Separate rising and falling for asymmetry analysis
manual_rising_periods = [
    ("2008-03-02", "2008-11-30", "上升趋势1"),
    ("2014-07-01", "2016-12-31", "上升趋势2"),
    ("2021-06-01", "2022-09-30", "上升趋势3"),
]
manual_falling_periods = [
    ("2001-01-01", "2008-03-01", "下降趋势1"),
    ("2008-12-01", "2011-04-30", "下降趋势2"),
    ("2017-01-01", "2018-01-31", "下降趋势3"),
    ("2020-03-01", "2021-05-31", "下降趋势4"),
    ("2022-10-01", "2023-12-31", "下降趋势5"),
]
...
# ============================================================
# 5. PROPOSITION 1: eff.dim × DXY trend state (BOTH METHODS)
# ============================================================
...
# ============================================================
print("\n" + "=" * 70)
print("ASYMMETRY TEST: Rising vs Falling DXY trends")
print("=" * 70)
...
print(f"\nRising DXY ($ strengthening):  n={len(rising_data)}, median={rising_data.median():.3f}, "
      f"mean={rising_data.mean():.3f}, IQR=[{rising_data.quantile(0.25):.3f}, {rising_data.quantile(0.75):.3f}]")
print(f"Falling DXY ($ weakening):     n={len(falling_data)}, median={falling_data.median():.3f}, "
...
if len(rising_data) >= 10 and len(falling_data) >= 10:
    ...
    # If $ node same pattern: rising DXY → higher eff.dim, falling DXY → lower eff.dim
    # If opposite: rising DXY → lower eff.dim, falling DXY → higher eff.dim
    if rf_p < 0.05:
        if rf_d > 0:
...
# ============================================================
# 8. PROPOSITION 2: eff.dim × fold depth (Mahalanobis)
# ============================================================
print("\n" + "=" * 70)
...
# Au comparison: min event eff.dim was 1.894, frac below = 6.92%
print(f"\n--- Cross-node comparison (Au vs $) ---")
print(f"  Au node: min event eff.dim = 1.894, frac below = 6.92%")
print(f"  $  node: min event eff.dim = {min_event_ed:.3f}, frac below = {frac_below*100:.2f}%")
...
results = {
    "node": "dollar",
    "edges": ["DXY/GC=F ($/Au)", "DXY/^GSPC ($/equity)", "DXY/HG=F ($/commodity)"],
    ...
    "asymmetry": {
        ...
        "au_comparison": {
            ...
            "dollar_pattern": f"rising_dxy mean={rising_data.mean():.3f}, falling_dxy mean={falling_data.mean():.3f}",
        },
    },
    "proposition_2": {
    ...
    "threshold_effect": {
        ...
        "au_comparison": {
            ...
            "dollar_min_event_effdim": round(min(eds), 3),
            "dollar_frac_below": round(float(frac_below), 4),
        },
    },
}
...
results["final_verdicts"] = {
    ...
    "threshold_convergence": (
        "TWO_NODE_CONVERGENCE"
        if frac_below > 0.03  # meaningful exclusion zone exists
        else "DOLLAR_NO_THRESHOLD"
    ),
    "question_1_answer": (
        "K4一般性质" if asym_verdict == "SAME_AS_AU" else
        "Au节点特异" if asym_verdict == "OPPOSITE_TO_AU" else
        "证据不足（无显著不对称或数据不足）"
    ),
    "question_2_answer": (
        "两节点汇合确认" if frac_below > 0.03 else
        "Au特有" if frac_below < 0.01 else
        "证据不足"
    ),
}

output_path = OUTPUT_DIR / "r3_dollar_node_results.json"
...
print(f"  {asym_desc}")
print(f"\nThreshold (Q2 - 阈值效应确认):")
print(f"  $ node min event eff.dim: {min_event_ed:.3f} (Au: 1.894)")
print(f"  $ node frac below: {frac_below*100:.2f}% (Au: 6.92%)")
