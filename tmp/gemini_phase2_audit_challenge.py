#!/usr/bin/env python
"""Gemini challenge: Phase 2 审计四议题异质质询。

四个议题分别调用 Gemini challenge 模式，结果写入 tmp/gemini-phase2-audit-challenge.md。
"""
import sys, os
...
def main():
    ...
    for key, subject in SUBJECTS.items():
        print(f"[{key}] Calling Gemini challenge...", flush=True)
        try:
    ...
    out = Path("tmp/gemini-phase2-audit-challenge.md")
    lines = ["# Gemini Phase 2 审计质询报告\n\n"]
    for key, resp in results.items():
        lines.append(f"## 议题：{key}\n\n{resp}\n\n---\n\n")
