...
def ingest_ceremony_texts(
    ...
    # 第一轮：收集所有待处理段落（去重检查 + 文件读取）
    all_paragraphs: list[str] = []
    processed_keys: list[tuple[str, str]] = []  # (rel_key, file_hash) 待更新 state
    for md_file in md_files:
        try:
            ...
            processed_keys.append((rel_key, file_hash))
            files_processed += 1
    ...
    # 第二轮：一次性批量摄入所有段落（白名单只构建一次）
    all_log_entries: list[dict] = []
    if all_paragraphs:
        source = f"ceremony_genealogy({files_processed} files)"
            ...
            for entry in all_log_entries:
                if entry.get("type") == "cooccurrence":
                    bt_writer.append_cooccurrence(
                        ...
                        weight=1.0,
                        corpus_source=f"ceremony_genealogy:batch",
    ...
    # 更新 state（所有已处理文件标记为已完成）
    for rel_key, file_hash in processed_keys:
        state[rel_key] = file_hash
