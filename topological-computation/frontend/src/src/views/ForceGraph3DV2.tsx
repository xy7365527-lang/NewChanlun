...
const haloFragGLSL = `
  ...
  void main() {
    ...
    float alpha = ring * (0.45 + 0.3 * pulse);
...
export function ForceGraph3DV2({
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(COL.bg, 0.0016);
    ...
    const pointA = new THREE.PointLight(0x4488ff, 1.8, 600);
    pointA.position.set(-150, 80, 100);
    scene.add(pointA);
    const pointB = new THREE.PointLight(0x22d68a, 1.2, 500);
    pointB.position.set(150, -60, -80);
    scene.add(pointB);
    const pointC = new THREE.PointLight(0xff4466, 0.8, 400);
    ...
    const negLineSegs = buildLineSegs(negLinks, COL.negEdge, 0.28);
    const depLineSegs = buildLineSegs(depLinks, COL.depEdge, 0.15);
    ...
    negLinks.forEach((l, li) => {
      ...
      const mat = new THREE.PointsMaterial({
        ...
        size: 3.5, transparent: true, opacity: 0.85,
    ...
    const spherical = new THREE.Spherical(camera.position.length(), Math.PI / 2, 0);
    let zoomVelocity = 0;
    ...
    function startFly(
      ...
      flyLookFrom = new THREE.Vector3(0, 0, 0);
    ...
    const onWheel = (e: WheelEvent) => { zoomVelocity += e.deltaY * 0.35; };

    const onDblClick = () => { startFly(new THREE.Vector3(0, 0, 480)); };
    ...
    const animate = () => {
      ...
      if (Math.abs(zoomVelocity) > 0.5) {
        spherical.radius = Math.max(40, Math.min(2200, spherical.radius + zoomVelocity));
        camera.position.setFromSpherical(spherical);
        camera.lookAt(0, 0, 0);
        zoomVelocity *= 0.88;
      }
      ...
      // 节点 InstancedMesh：呼吸 + hover 放大 + 邻域高亮
      for (let i = 0; i < N; i++) {
        ...
        nodeScales[i] = isHovered
          ? Math.min(nodeScales[i] + 0.06, 1.45)
          : Math.max(nodeScales[i] - 0.04, 1.0);
        scale *= nodeScales[i];
        ...
        if (hoveredIdx >= 0 && !isHovered && !isNeighbor) tmpColor.multiplyScalar(0.35);
        else if (isHovered) tmpColor.multiplyScalar(1.3);
      ...
      pulsePoints.forEach(({ point, srcIdx, tgtIdx, phase }) => {
        ...
        (point.material as THREE.PointsMaterial).opacity = 0.9 * (1 - distFrac * distFrac);
      });
      ...
      bubbleMeshes.forEach(({ mesh, src }) => {
        ...
        mat.opacity =
          activeCluster === null ? 0.04 :
          src === activeCluster ? 0.09 : 0.015;
      });
    ...
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {hoveredNode && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 10, color: "rgba(106,106,138,0.8)", fontSize: 9 }}>
            <span>f={hoveredNode.f_avg.toFixed(1)}</span>
            <span>deg={hoveredNode.degree}</span>
            <span style={{ color: "rgba(58,58,90,0.8)" }}>
