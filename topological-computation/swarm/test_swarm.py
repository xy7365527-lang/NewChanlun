...
IPFS shared layer, 200 steps each. Verify cross-instance block sharing and sync.
...
def run_test() -> str:
    ...
    lines: list[str] = []
    lines.append("=" * 70)
    lines.append("SWARM INTEGRATION TEST (IPFS backend)")
    lines.append("=" * 70)
    lines.append("")

    # IPFS client — 测试要求 IPFS daemon 在线
    ipfs = IPFSClient()
    if not ipfs.is_available():
        lines.append("[FAIL] IPFS daemon 不可用 — 测试无法执行")
        lines.append("启动 IPFS daemon: ipfs daemon")
        return "\n".join(lines)

    lines.append(f"IPFS API: {ipfs.api_url}")
    lines.append("")
    ...
    try:
        ...
        # Create two SwarmDaemon instances（共享同一个 IPFS）
        ...
        # Shared layer stats (via IPFS)
        shared = SharedLayer(ipfs)
        all_hashes = shared.all_block_hashes()
        lines.append("--- Shared Layer (IPFS) ---")
        lines.append(f"  Total blocks in IPFS: {len(all_hashes)}")
        lines.append("")
        ...
        checks.append(("Instance A wrote blocks to IPFS", a_wrote))
        checks.append(("Instance B wrote blocks to IPFS", b_wrote))

        # 2. Blocks exist in IPFS
        blocks_exist = len(all_hashes) > 0
        checks.append(("IPFS contains blocks", blocks_exist))
